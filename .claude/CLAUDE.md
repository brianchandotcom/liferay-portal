...
- `portal-web/` — The main web application: JSPs, tag libraries, WEB-INF configuration.
- `util-bridges/`, `util-java/`, `util-taglib/`, `util-slf4j/` — Shared utility libraries.

### OSGi Modules (gradle based)

- `modules/apps/` — Most of the OSGi modules. This is where most of development happens.
...
The default for most things: anything that touches services, persistence, or the portal runtime.
...
Last resort, reserved for complete UI flows that can't be covered at a lower level.
...
- **Poshi**
    ...
    ```bash
    cd <repo-root> && HOSTNAME=localhost ant -f build-test.xml run-selenium-test -Dtest.class=<TestClassName>
    ```
