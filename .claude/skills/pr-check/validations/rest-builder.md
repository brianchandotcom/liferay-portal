...
- OR (**Generator-Source Change**) the diff touches the REST Builder generator itself under `modules/util/portal-tools-rest-builder/` — a template or the generator Java. A generator change alters every REST Builder module's output, so the full regen below catches modules the diff never touched.
