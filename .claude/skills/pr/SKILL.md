...
If the commits do not include any tests, ask the user for a rationale. Never open a pull request that lacks both tests and a rationale. The only exceptions are pull requests with no code changes (e.g., language key updates) or pull requests that only contain autogenerated code or tool output (e.g., source formatting, build services, etc.).

When writing the pull request description and title, avoid being verbose. Use a direct, to-the-point style.
