...
run "should_cap_availability_zones_at_2_by_default" {
	...
	assert {
		...
		error_message="selected_azs must be capped at max_availability_zones (default 2) even when more AZs are available."
	}
...
run "should_compute_default_subnet_cidrs" {
	...
	assert {
		...
		error_message="default_private_subnets must be cidrsubnet(vpc_cidr, 8, i+1) for each AZ."
	}

	assert {
		condition=local.default_public_subnets == ["10.0.101.0/24", "10.0.102.0/24"]
		error_message="default_public_subnets must be cidrsubnet(vpc_cidr, 8, i+101) for each AZ."
	}
}
