...
run "should_configure_crossplane_functions_and_runtime_configs" {
	...
	assert {
		...
		error_message="Function runtime configs must run as non-root."
	}
...
run "should_name_the_provider_iam_roles_and_policies" {
	...
	assert {
		condition=aws_iam_role.provider_aws_rds_role.name == "liferay-test-eks-provider-aws-rds-role" && aws_iam_role.provider_aws_kms_role.name == "liferay-test-eks-provider-aws-kms-role"
