...
run "should_create_ecr_sync_resources_for_the_marketplace_chart" {
	...
	assert {
		condition=length(aws_iam_role.ecr_role) == 1 && aws_iam_role.ecr_role[0].name == "liferay-test-eks-gitops-ecr-credentials-sync"
		error_message="The marketplace chart must create the ECR sync IRSA role named after the cluster."
	}

	assert {
		condition=length(kubernetes_cron_job_v1.ecr_credentials_sync) == 1 && kubernetes_cron_job_v1.ecr_credentials_sync[0].spec[0].schedule == "0 */8 * * *"
