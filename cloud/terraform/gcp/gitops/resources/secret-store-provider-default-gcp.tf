resource "google_project_iam_member" "secrets_accessor_permissions" {
	condition {
		expression=join(" || ", concat(
			[
				"resource.name.startsWith('projects/_/secrets/${local.secret_prefixes.certificates}')",
				"resource.name.startsWith('projects/_/secrets/${local.secret_prefixes.licenses}')",
			],
			[
				for git_repo_auth_config in local.git_repo_auth_configs : "resource.name.startsWith('projects/_/secrets/${git_repo_auth_config.credentials_secret_name}')"
			]
		))
