...
public class AccountRolePersistenceImpl
	...
	private FinderPath _finderPathWithPaginationFindByAccountEntryIds;
	private FinderPath _finderPathWithoutPaginationFindByAccountEntryIds;
	private FinderPath _finderPathCountByAccountEntryIds;
	private FinderPath _finderPathWithPaginationCountByAccountEntryIds;
	 ...
	 */
	@Override
	public List<AccountRole> findByAccountEntryIds(long accountEntryId) {
		return findByAccountEntryIds(
	 ...
	 */
	@Override
	public List<AccountRole> findByAccountEntryIds(
		long accountEntryId, int start, int end) {

		return findByAccountEntryIds(accountEntryId, start, end, null);
	}
	...
	public List<AccountRole> findByAccountEntryIds(
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				finderPath = _finderPathWithoutPaginationFindByAccountEntryIds;
				finderArgs = new Object[] {accountEntryId};
			}
		}
		else if (useFinderCache) {
			finderPath = _finderPathWithPaginationFindByAccountEntryIds;
			finderArgs = new Object[] {
		...
		if (list == null) {
			...
			query.append(_FINDER_COLUMN_ACCOUNTENTRYIDS_ACCOUNTENTRYID_2);
	 ...
	 */
	@Override
	public AccountRole findByAccountEntryIds_First(
			long accountEntryId,
			OrderByComparator<AccountRole> orderByComparator)
		throws NoSuchRoleException {

		AccountRole accountRole = fetchByAccountEntryIds_First(
	 ...
	 */
	@Override
	public AccountRole fetchByAccountEntryIds_First(
		long accountEntryId, OrderByComparator<AccountRole> orderByComparator) {

		List<AccountRole> list = findByAccountEntryIds(
	 ...
	 */
	@Override
	public AccountRole findByAccountEntryIds_Last(
			long accountEntryId,
			OrderByComparator<AccountRole> orderByComparator)
		throws NoSuchRoleException {

		AccountRole accountRole = fetchByAccountEntryIds_Last(
			accountEntryId, orderByComparator);
	 ...
	 */
	@Override
	public AccountRole fetchByAccountEntryIds_Last(
		long accountEntryId, OrderByComparator<AccountRole> orderByComparator) {

		int count = countByAccountEntryIds(accountEntryId);
	 ...
	 */
	@Override
	public AccountRole[] findByAccountEntryIds_PrevAndNext(
		...
		try {
			...
			array[0] = getByAccountEntryIds_PrevAndNext(
				session, accountRole, accountEntryId, orderByComparator, true);

			array[1] = accountRole;

			array[2] = getByAccountEntryIds_PrevAndNext(
	...
	protected AccountRole getByAccountEntryIds_PrevAndNext(
	 ...
	 */
	@Override
	public List<AccountRole> findByAccountEntryIds(long[] accountEntryIds) {
		return findByAccountEntryIds(
	...
	public List<AccountRole> findByAccountEntryIds(
		...
		return findByAccountEntryIds(accountEntryIds, start, end, null);
	}
	...
	public List<AccountRole> findByAccountEntryIds(
		...
		if (useFinderCache) {
			list = (List<AccountRole>)finderCache.getResult(
				_finderPathWithPaginationFindByAccountEntryIds, finderArgs,
				this);
		...
		if (list == null) {
			...
			if (accountEntryIds.length > 0) {
				...
				query.append(_FINDER_COLUMN_ACCOUNTENTRYIDS_ACCOUNTENTRYID_7);
			...
			try {
				...
				if (useFinderCache) {
					finderCache.putResult(
						_finderPathWithPaginationFindByAccountEntryIds,
						finderArgs, list);
				}
			}
			catch (Exception e) {
	 ...
	 */
	@Override
	public void removeByAccountEntryIds(long accountEntryId) {
		for (AccountRole accountRole :
				findByAccountEntryIds(
	 ...
	 */
	@Override
	public int countByAccountEntryIds(long accountEntryId) {
		FinderPath finderPath = _finderPathCountByAccountEntryIds;
	 ...
	 */
	@Override
	public int countByAccountEntryIds(long[] accountEntryIds) {
		if (accountEntryIds == null) {
			accountEntryIds = new long[0];
		}
		else if (accountEntryIds.length > 1) {
		...
		Long count = (Long)finderCache.getResult(
			_finderPathWithPaginationCountByAccountEntryIds, finderArgs, this);
		...
		if (count == null) {
			...
			try {
				...
				finderCache.putResult(
					_finderPathWithPaginationCountByAccountEntryIds, finderArgs,
					count);
			}
			catch (Exception e) {
				finderCache.removeResult(
					_finderPathWithPaginationCountByAccountEntryIds,
					finderArgs);
	...
	private static final String
		_FINDER_COLUMN_ACCOUNTENTRYIDS_ACCOUNTENTRYID_2 =
			"accountRole.accountEntryId = ?";

	private static final String
		_FINDER_COLUMN_ACCOUNTENTRYIDS_ACCOUNTENTRYID_7 =
	...
	public AccountRole updateImpl(AccountRole accountRole) {
		...
		if (!_columnBitmaskEnabled) {
			...
			finderCache.removeResult(_finderPathCountByAccountEntryIds, args);
			finderCache.removeResult(
				_finderPathWithoutPaginationFindByAccountEntryIds, args);
			...
			if ((accountRoleModelImpl.getColumnBitmask() &
				 _finderPathWithoutPaginationFindByAccountEntryIds.
					 getColumnBitmask()) != 0) {
				...
				finderCache.removeResult(
					_finderPathCountByAccountEntryIds, args);
				finderCache.removeResult(
					_finderPathWithoutPaginationFindByAccountEntryIds, args);
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByAccountEntryIds = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, AccountRoleImpl.class,
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByAccountEntryIds",
			new String[] {
				Long.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			});

		_finderPathWithoutPaginationFindByAccountEntryIds = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, AccountRoleImpl.class,
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByAccountEntryIds",
			new String[] {Long.class.getName()},
			AccountRoleModelImpl.ACCOUNTENTRYID_COLUMN_BITMASK);

		_finderPathCountByAccountEntryIds = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, Long.class,
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByAccountEntryIds",
			new String[] {Long.class.getName()});

		_finderPathWithPaginationCountByAccountEntryIds = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, Long.class,
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "countByAccountEntryIds",
			new String[] {Long.class.getName()});
