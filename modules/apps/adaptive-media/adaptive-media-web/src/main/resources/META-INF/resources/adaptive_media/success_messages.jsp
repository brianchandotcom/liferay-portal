...
<liferay-ui:success key="configurationEntriesDeleted">
	...
	<c:choose>
		<c:when test="<%= amImageConfigurationEntries.size() == 1 %>">
			<liferay-ui:message
				arguments="<%= HtmlUtil.escape(amImageConfigurationEntries.get(0).getName()) %>"
				key="x-was-deleted-successfully"
				translateArguments="<%= false %>"
			/>
		</c:when>
		<c:otherwise>
			<liferay-ui:message
				arguments='<%= HtmlUtil.escape(ListUtil.toString(amImageConfigurationEntries, "name")) %>'
				key="x-were-deleted-successfully"
				translateArguments="<%= false %>"
			/>
		</c:otherwise>
	</c:choose>
...
<liferay-ui:success key="configurationEntryAddedAndIDRenamed">
	...
	<liferay-ui:message
		arguments="<%= new String[] {HtmlUtil.escape(amImageConfigurationEntry.getName()), amImageConfigurationEntry.getUUID()} %>"
		key="x-was-saved-successfully.-the-id-was-duplicated-and-renamed-to-x"
		translateArguments="<%= false %>"
	/>
</liferay-ui:success>
...
<liferay-ui:success key="configurationEntryEnabled">
	...
	<liferay-ui:message
		arguments="<%= HtmlUtil.escape(amImageConfigurationEntry.getName()) %>"
		key="x-was-enabled-successfully"
		translateArguments="<%= false %>"
	/>
</liferay-ui:success>
...
<liferay-ui:success key="configurationEntryDisabled">
	...
	<liferay-ui:message
		arguments="<%= HtmlUtil.escape(amImageConfigurationEntry.getName()) %>"
		key="x-was-disabled-successfully"
		translateArguments="<%= false %>"
	/>
</liferay-ui:success>
...
<liferay-ui:success key="highResolutionConfigurationEntryAdded">
	...
	<liferay-ui:message
		arguments="<%= new String[] {HtmlUtil.escape(addedConfigurationEntries[0].getName()), HtmlUtil.escape(addedConfigurationEntries[1].getName())} %>"
		key="x-and-x-were-saved-successfully"
		translateArguments="<%= false %>"
	/>
</liferay-ui:success>
