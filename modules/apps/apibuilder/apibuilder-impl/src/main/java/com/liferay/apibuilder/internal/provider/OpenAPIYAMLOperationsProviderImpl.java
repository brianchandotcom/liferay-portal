...
package com.liferay.apibuilder.internal.provider;
...
import com.liferay.apibuilder.provider.OpenAPIYAMLOperationsProvider;
		...
		if (StringUtil.equals(type, "boolean")) {
			...
			if (StringUtil.equals(schema.getFormat(), "int64")) {
				return SetUtil.fromArray(
					...
					PrimaryKeyInfoFieldType.INSTANCE);
			}
	...
	private List<Operation> _getOperations(
		...
		for (Method method : Method.values()) {
			...
			for (Map.Entry
				...
				for (Map.Entry<String, Content> entry2 :
					...
					builder.withResponse(
						new com.liferay.apibuilder.operation.Response(
			...
			for (Parameter operationParameter : operation.getParameters()) {
				InfoField<?> infoField = successfulInfoFields.get(
				...
				builder.withParameterInfoField(
					infoField, operationParameter.getName());
			}
