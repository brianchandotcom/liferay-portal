...
public class AppBuilderAppPersistenceImpl
	...
	public List<AppBuilderApp> findByG_C_D_S(
		long groupId, long companyId, long ddmStructureId, String status) {

		return findByG_C_D_S(
			groupId, companyId, ddmStructureId, status, QueryUtil.ALL_POS,
			QueryUtil.ALL_POS, null);
	}
	...
	public List<AppBuilderApp> findByG_C_D_S(
		...
		return findByG_C_D_S(
			groupId, companyId, ddmStructureId, status, start, end, null);
	}
	...
	public List<AppBuilderApp> findByG_C_D_S(
			...
			if ((list != null) && !list.isEmpty()) {
				for (AppBuilderApp appBuilderApp : list) {
					if ((groupId != appBuilderApp.getGroupId()) ||
						(companyId != appBuilderApp.getCompanyId()) ||
						(ddmStructureId != appBuilderApp.getDdmStructureId()) ||
						!status.equals(appBuilderApp.getStatus())) {
		...
		if (list == null) {
			...
			query.append(_FINDER_COLUMN_G_C_D_S_GROUPID_2);

			query.append(_FINDER_COLUMN_G_C_D_S_COMPANYID_2);

			query.append(_FINDER_COLUMN_G_C_D_S_DDMSTRUCTUREID_2);

			boolean bindStatus = false;

			if (status.isEmpty()) {
				query.append(_FINDER_COLUMN_G_C_D_S_STATUS_3);
			}
			else {
				bindStatus = true;

				query.append(_FINDER_COLUMN_G_C_D_S_STATUS_2);
			}
	...
	public AppBuilderApp findByG_C_D_S_First(
		...
		AppBuilderApp appBuilderApp = fetchByG_C_D_S_First(
			groupId, companyId, ddmStructureId, status, orderByComparator);
	...
	public AppBuilderApp fetchByG_C_D_S_First(
		...
		List<AppBuilderApp> list = findByG_C_D_S(
			groupId, companyId, ddmStructureId, status, 0, 1,
			orderByComparator);
	...
	public AppBuilderApp fetchByG_C_D_S_Last(
		...
		int count = countByG_C_D_S(groupId, companyId, ddmStructureId, status);
		...
		List<AppBuilderApp> list = findByG_C_D_S(
			groupId, companyId, ddmStructureId, status, count - 1, count,
			orderByComparator);
	...
	public AppBuilderApp[] findByG_C_D_S_PrevAndNext(
			long appBuilderAppId, long groupId, long companyId,
			long ddmStructureId, String status,
			OrderByComparator<AppBuilderApp> orderByComparator)
		throws NoSuchAppException {
	...
	protected AppBuilderApp getByG_C_D_S_PrevAndNext(
		Session session, AppBuilderApp appBuilderApp, long groupId,
		long companyId, long ddmStructureId, String status,
		OrderByComparator<AppBuilderApp> orderByComparator, boolean previous) {
	...
	private static final String _FINDER_COLUMN_G_C_D_S_STATUS_2 =
		"appBuilderApp.status = ?";

	private static final String _FINDER_COLUMN_G_C_D_S_STATUS_3 =
		"(appBuilderApp.status IS NULL OR appBuilderApp.status = '')";
	...
	public AppBuilderApp updateImpl(AppBuilderApp appBuilderApp) {
		...
		if (!_columnBitmaskEnabled) {
			...
			finderCache.removeResult(_finderPathCountByG_C_D_S, args);
			finderCache.removeResult(
				_finderPathWithoutPaginationFindByG_C_D_S, args);
			...
			if ((appBuilderAppModelImpl.getColumnBitmask() &
				 _finderPathWithoutPaginationFindByG_C_D_S.
					 getColumnBitmask()) != 0) {
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByG_C_D_S = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, AppBuilderAppImpl.class,
		...
		_finderPathWithoutPaginationFindByG_C_D_S = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, AppBuilderAppImpl.class,
		...
		_finderPathCountByG_C_D_S = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, Long.class,
