...
public class GroupModelListenerTest {
	...
	private long _getGroupId(AssetListEntry assetListEntry1) throws Exception {
		assetListEntry1 = _assetListEntryLocalService.getAssetListEntry(
			assetListEntry1.getAssetListEntryId());

		UnicodeProperties unicodeProperties = UnicodePropertiesBuilder.create(
			true
		).fastLoad(
			assetListEntry1.getTypeSettings(_segmentsEntry.getSegmentsEntryId())
		).build();
