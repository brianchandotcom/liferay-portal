	...
	%>
		...
		<li class="list-group-item list-group-item-flex <%= ((previewClassNameId == assetEntry.getClassNameId()) && (previewClassPK == assetEntry.getClassPK())) ? "active" : StringPool.BLANK %>" <%= AUIUtil.buildData(fragmentsEditorData) %>>
			...
			<clay:content-col
				expand="<%= true %>"
			>
				<p class="h4 list-group-title text-truncate" <%= inViewMode ? assetAnalyticsAttributesProvider.buildAttributes(AssetAnalyticsAttributesProvider.ACTION_IMPRESSION, AssetAnalyticsAttributesProvider.FIELD_TITLE) : StringPool.BLANK %>>
				...
				<p class="list-group-subtitle text-truncate" <%= inViewMode ? assetAnalyticsAttributesProvider.buildAttributes(AssetAnalyticsAttributesProvider.ACTION_IMPRESSION, "subtitle") : StringPool.BLANK %>>
					<%= sb.toString() %>
				</p>
