...
public class BatchPlannerMappingPersistenceImpl
	...
	private FinderPath _finderPathFetchByBPPI_EN;
	private FinderPath _finderPathCountByBPPI_EN;

	/**
	 * Returns the batch planner mapping where batchPlannerPlanId = &#63; and externalName = &#63; or throws a <code>NoSuchMappingException</code> if it could not be found.
	 *
	 * @param batchPlannerPlanId the batch planner plan ID
	 ...
	 */
	@Override
	public BatchPlannerMapping findByBPPI_EN(
			long batchPlannerPlanId, String externalName)
		throws NoSuchMappingException {

		BatchPlannerMapping batchPlannerMapping = fetchByBPPI_EN(
			batchPlannerPlanId, externalName);
	...
	/**
	 * Returns the batch planner mapping where batchPlannerPlanId = &#63; and externalName = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param batchPlannerPlanId the batch planner plan ID
	 * @param externalName the external name
	 ...
	 */
	@Override
	public BatchPlannerMapping fetchByBPPI_EN(
		long batchPlannerPlanId, String externalName) {

		return fetchByBPPI_EN(batchPlannerPlanId, externalName, true);
	}

	/**
	 * Returns the batch planner mapping where batchPlannerPlanId = &#63; and externalName = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param batchPlannerPlanId the batch planner plan ID
	 * @param externalName the external name
	 * @param useFinderCache whether to use the finder cache
	...
	@Override
	public BatchPlannerMapping fetchByBPPI_EN(
		long batchPlannerPlanId, String externalName, boolean useFinderCache) {

		Object[] finderArgs = null;

		if (useFinderCache) {
			finderArgs = new Object[] {batchPlannerPlanId, externalName};
		}
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByBPPI_EN, finderArgs);
		}
		...
		if (result == null) {
			...
			sb.append(_FINDER_COLUMN_BPPI_EN_BATCHPLANNERPLANID_2);

			boolean bindExternalName = false;

			if (externalName == null) {
				sb.append(_FINDER_COLUMN_BPPI_EN_EXTERNALNAME_1);
			}
			else if (externalName.isEmpty()) {
				sb.append(_FINDER_COLUMN_BPPI_EN_EXTERNALNAME_3);
			}
			else {
				bindExternalName = true;

				sb.append(_FINDER_COLUMN_BPPI_EN_EXTERNALNAME_2);
			}
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByBPPI_EN, finderArgs, list);
					}
				}
				else {
	...
	/**
	 * Removes the batch planner mapping where batchPlannerPlanId = &#63; and externalName = &#63; from the database.
	 *
	 * @param batchPlannerPlanId the batch planner plan ID
	 * @param externalName the external name
	 ...
	 */
	@Override
	public BatchPlannerMapping removeByBPPI_EN(
			long batchPlannerPlanId, String externalName)
		throws NoSuchMappingException {

		BatchPlannerMapping batchPlannerMapping = findByBPPI_EN(
			batchPlannerPlanId, externalName);
	...
	/**
	 * Returns the number of batch planner mappings where batchPlannerPlanId = &#63; and externalName = &#63;.
	 *
	 * @param batchPlannerPlanId the batch planner plan ID
	 * @param externalName the external name
	 * @return the number of matching batch planner mappings
	 */
	@Override
	public int countByBPPI_EN(long batchPlannerPlanId, String externalName) {
		FinderPath finderPath = _finderPathCountByBPPI_EN;

		Object[] finderArgs = new Object[] {batchPlannerPlanId, externalName};
	...
	private static final String _FINDER_COLUMN_BPPI_EN_BATCHPLANNERPLANID_2 =
		"batchPlannerMapping.batchPlannerPlanId = ? AND ";

	private static final String _FINDER_COLUMN_BPPI_EN_EXTERNALNAME_1 =
		"batchPlannerMapping.externalName IS NULL";

	private static final String _FINDER_COLUMN_BPPI_EN_EXTERNALNAME_2 =
		"batchPlannerMapping.externalName = ?";

	private static final String _FINDER_COLUMN_BPPI_EN_EXTERNALNAME_3 =
		"(batchPlannerMapping.externalName IS NULL OR batchPlannerMapping.externalName = '')";
	...
	public void cacheResult(BatchPlannerMapping batchPlannerMapping) {
		...
		finderCache.putResult(
			_finderPathFetchByBPPI_EN,
			new Object[] {
				batchPlannerMapping.getBatchPlannerPlanId(),
				batchPlannerMapping.getExternalName()
			},
			batchPlannerMapping);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		Object[] args = new Object[] {
			...
			batchPlannerMappingModelImpl.getExternalName()
		};

		finderCache.putResult(_finderPathCountByBPPI_EN, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByBPPI_EN, args, batchPlannerMappingModelImpl);
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByBPPI_EN = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByBPPI_EN",
			new String[] {Long.class.getName(), String.class.getName()},
			new String[] {"batchPlannerPlanId", "externalName"}, true);

		_finderPathCountByBPPI_EN = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByBPPI_EN",
			new String[] {Long.class.getName(), String.class.getName()},
			new String[] {"batchPlannerPlanId", "externalName"}, false);
	}
