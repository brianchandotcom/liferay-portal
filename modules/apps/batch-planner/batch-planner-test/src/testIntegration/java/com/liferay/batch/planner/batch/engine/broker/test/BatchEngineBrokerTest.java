...
public class BatchEngineBrokerTest {
	...
	private void _assertEqualsExportCSV(
		...
		int externalReferenceCodeIndex = expectedCSVRecordsList.get(
			0
		).indexOf(
			"externalReferenceCode"
		);

		Map<String, List<String>> csvRecordsMap = _toCSVRecordsMap(
			csvRecords.subList(1, csvRecords.size()),
			externalReferenceCodeIndex);

		List<String> actualCSVRecordStrings = csvRecordsMap.get(
			externalReferenceCode);
	...
	private Map<String, List<String>> _toCSVRecordsMap(
		...
		Map<String, List<String>> csvRecordsMap = new HashMap<>();
		...
		for (CSVRecord csvRecord : csvRecords) {
			...
			csvRecordsMap.put(key, csvRecordStrings);
		}
