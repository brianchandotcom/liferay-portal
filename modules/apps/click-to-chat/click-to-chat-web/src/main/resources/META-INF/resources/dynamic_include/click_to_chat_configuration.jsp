...
<div class="row">
	<div class="col-md-12">
		...
		<aui:input checked="<%= clickToChatEnabled %>" inlineLabel="right" label='<%= LanguageUtil.get(resourceBundle, "enable-click-to-chat") %>' labelCssClass="simple-toggle-switch" name="TypeSettingsProperties--enabled--" type="toggle-switch" value="<%= clickToChatEnabled %>" />
	</div>
</div>

<div class="form-group row">
	<div class="col-md-12">
		<aui:select label="site-settings-strategy" name="TypeSettingsProperties--siteSettingsStrategy--" onchange='<%= liferayPortletResponse.getNamespace() + "onChangeClickToChatSiteSettingsStrategy(event);" %>' value="<%= clickToChatConfiguration.siteSettingsStrategy() %>">
...
<div class="row" id="<%= liferayPortletResponse.getNamespace() + "divProviderFields" %>">
	<div class="col-md-6">
		...
		<aui:select label="chat-provider" name="TypeSettingsProperties--chatProviderId--" onchange='<%= liferayPortletResponse.getNamespace() + "onChangeClickToChatChatProviderId(event);" %>' value="<%= clickToChatChatProviderId %>">
			<aui:option label="" value="" />
		...
		<aui:input checked="<%= clickToChatGuestUsersAllowed %>" inlineLabel="right" label='<%= LanguageUtil.get(resourceBundle, "guest-users-allowed") %>' labelCssClass="simple-toggle-switch" name="TypeSettingsProperties--guestUsersAllowed--" type="toggle-switch" value="<%= clickToChatGuestUsersAllowed %>" />
	</div>

	<div class="col-md-6">
		<aui:input label="chat-provider-account-id" name="TypeSettingsProperties--chatProviderAccountId--" type="text" value="<%= clickToChatConfiguration.chatProviderAccountId() %>" />
