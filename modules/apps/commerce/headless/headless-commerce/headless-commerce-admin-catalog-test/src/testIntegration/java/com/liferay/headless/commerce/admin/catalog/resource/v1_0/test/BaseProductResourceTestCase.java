...
public abstract class BaseProductResourceTestCase {
	...
	@Test
	public void testPostProductsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			productResource.postProductsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Product[] products = getProducts(exportTask);

		long totalCount = products.length;

		Product product1 = testGetProductsPage_addProduct(randomProduct());

		Product product2 = testGetProductsPage_addProduct(randomProduct());

		httpResponse = productResource.postProductsPageExportBatchHttpResponse(
			null, null, null, null, null, null);

		exportTask = ExportTask.toDTO(httpResponse.getContent());

		products = getProducts(exportTask);

		Assert.assertEquals(totalCount + 2, products.length);

		assertContains(product1, Arrays.asList(products));
		assertContains(product2, Arrays.asList(products));
	}
	...
	protected Product[] getProducts(ExportTask exportTask) throws Exception {
