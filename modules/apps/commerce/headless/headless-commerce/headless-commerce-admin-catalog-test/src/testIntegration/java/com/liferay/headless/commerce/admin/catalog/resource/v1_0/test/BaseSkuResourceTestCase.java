...
public abstract class BaseSkuResourceTestCase {
	...
	@Test
	public void testPostSkusPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			skuResource.postSkusPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Sku[] skus = getSkus(exportTask);

		long totalCount = skus.length;

		Sku sku1 = testGetSkusPage_addSku(randomSku());

		Sku sku2 = testGetSkusPage_addSku(randomSku());

		httpResponse = skuResource.postSkusPageExportBatchHttpResponse(
			null, null, null, null, null, null);

		exportTask = ExportTask.toDTO(httpResponse.getContent());

		skus = getSkus(exportTask);

		Assert.assertEquals(totalCount + 2, skus.length);

		assertContains(sku1, Arrays.asList(skus));
		assertContains(sku2, Arrays.asList(skus));
	}
	...
	protected Sku[] getSkus(ExportTask exportTask) throws Exception {
		...
		try {
			return SkuSerDes.toDTOs(zipReader.getEntryAsString("export.json"));
		}
		finally {
			zipReader.close();
		}
