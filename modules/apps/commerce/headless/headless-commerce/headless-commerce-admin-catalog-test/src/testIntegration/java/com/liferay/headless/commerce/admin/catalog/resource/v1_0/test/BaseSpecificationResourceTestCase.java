...
public abstract class BaseSpecificationResourceTestCase {
	...
	@Test
	public void testPostSpecificationsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			specificationResource.postSpecificationsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Specification[] specifications = getSpecifications(exportTask);

		long totalCount = specifications.length;
		...
		specifications = getSpecifications(exportTask);

		Assert.assertEquals(totalCount + 2, specifications.length);

		assertContains(specification1, Arrays.asList(specifications));
		assertContains(specification2, Arrays.asList(specifications));
	}
	...
	protected Specification[] getSpecifications(ExportTask exportTask)
		throws Exception {
		...
		try {
			return SpecificationSerDes.toDTOs(
