...
public abstract class BaseReplenishmentItemResourceTestCase {
	...
	@Test
	public void testPostReplenishmentItemsPageExportBatch() throws Exception {
		String sku = testGetReplenishmentItemsPage_getSku();
		String irrelevantSku = testGetReplenishmentItemsPage_getIrrelevantSku();
		...
		ReplenishmentItem[] replenishmentItems = getReplenishmentItems(
			exportTask);

		long totalCount = replenishmentItems.length;
		...
		if (irrelevantSku != null) {
			...
			httpResponse =
				replenishmentItemResource.
					postReplenishmentItemsPageExportBatchHttpResponse(
						irrelevantSku, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			replenishmentItems = getReplenishmentItems(exportTask);

			Assert.assertEquals(1, replenishmentItems.length);

			assertEquals(irrelevantReplenishmentItem, replenishmentItems[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, replenishmentItems.length);

		assertContains(replenishmentItem1, Arrays.asList(replenishmentItems));
		assertContains(replenishmentItem2, Arrays.asList(replenishmentItems));
	}
	...
	protected ReplenishmentItem[] getReplenishmentItems(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ReplenishmentItemSerDes.toDTOs(
