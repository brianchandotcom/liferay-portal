...
public abstract class BaseOrderItemResourceTestCase {
	...
	@Test
	public void testPostOrderItemsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			orderItemResource.postOrderItemsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		OrderItem[] orderItems = getOrderItems(exportTask);

		long totalCount = orderItems.length;
		...
		orderItems = getOrderItems(exportTask);

		Assert.assertEquals(totalCount + 2, orderItems.length);

		assertContains(orderItem1, Arrays.asList(orderItems));
		assertContains(orderItem2, Arrays.asList(orderItems));
	}
	...
	protected OrderItem[] getOrderItems(ExportTask exportTask)
		throws Exception {
