...
public abstract class BaseOrderResourceTestCase {
	...
	@Test
	public void testPostOrdersPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			orderResource.postOrdersPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Order[] orders = getOrders(exportTask);

		long totalCount = orders.length;

		Order order1 = testGetOrdersPage_addOrder(randomOrder());

		Order order2 = testGetOrdersPage_addOrder(randomOrder());

		httpResponse = orderResource.postOrdersPageExportBatchHttpResponse(
			null, null, null, null, null, null);

		exportTask = ExportTask.toDTO(httpResponse.getContent());

		orders = getOrders(exportTask);

		Assert.assertEquals(totalCount + 2, orders.length);

		assertContains(order1, Arrays.asList(orders));
		assertContains(order2, Arrays.asList(orders));
	}
	...
	protected Order[] getOrders(ExportTask exportTask) throws Exception {
