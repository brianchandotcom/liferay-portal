...
public abstract class BaseOrderRuleResourceTestCase {
	...
	@Test
	public void testPostOrderRulesPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			orderRuleResource.postOrderRulesPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		OrderRule[] orderRules = getOrderRules(exportTask);

		long totalCount = orderRules.length;
		...
		orderRules = getOrderRules(exportTask);

		Assert.assertEquals(totalCount + 2, orderRules.length);

		assertContains(orderRule1, Arrays.asList(orderRules));
		assertContains(orderRule2, Arrays.asList(orderRules));
	}
	...
	protected OrderRule[] getOrderRules(ExportTask exportTask)
		throws Exception {
