...
public abstract class BasePriceListResourceTestCase {
	...
	@Test
	public void testPostPriceListsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			priceListResource.postPriceListsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		PriceList[] priceLists = getPriceLists(exportTask);

		long totalCount = priceLists.length;
		...
		priceLists = getPriceLists(exportTask);

		Assert.assertEquals(totalCount + 2, priceLists.length);

		assertContains(priceList1, Arrays.asList(priceLists));
		assertContains(priceList2, Arrays.asList(priceLists));
	}
	...
	protected PriceList[] getPriceLists(ExportTask exportTask)
		throws Exception {
