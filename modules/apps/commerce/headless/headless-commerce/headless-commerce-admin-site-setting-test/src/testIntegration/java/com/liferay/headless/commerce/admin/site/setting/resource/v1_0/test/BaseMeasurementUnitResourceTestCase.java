...
public abstract class BaseMeasurementUnitResourceTestCase {
	...
	@Test
	public void testPostMeasurementUnitsPageExportBatch() throws Exception {
		...
		MeasurementUnit[] measurementUnits = getMeasurementUnits(exportTask);

		long totalCount = measurementUnits.length;
		...
		measurementUnits = getMeasurementUnits(exportTask);

		Assert.assertEquals(totalCount + 2, measurementUnits.length);

		assertContains(measurementUnit1, Arrays.asList(measurementUnits));
		assertContains(measurementUnit2, Arrays.asList(measurementUnits));
	}
	...
	protected MeasurementUnit[] getMeasurementUnits(ExportTask exportTask)
		throws Exception {
		...
		try {
			return MeasurementUnitSerDes.toDTOs(
