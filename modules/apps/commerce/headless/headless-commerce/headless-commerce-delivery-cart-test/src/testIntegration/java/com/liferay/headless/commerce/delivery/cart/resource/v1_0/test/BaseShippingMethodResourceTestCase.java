...
public abstract class BaseShippingMethodResourceTestCase {
	...
	@Test
	public void testPostCartShippingMethodsPageExportBatch() throws Exception {
		Long cartId = testGetCartShippingMethodsPage_getCartId();
		Long irrelevantCartId =
			testGetCartShippingMethodsPage_getIrrelevantCartId();
		...
		ShippingMethod[] shippingMethods = getShippingMethods(exportTask);

		long totalCount = shippingMethods.length;
		...
		if (irrelevantCartId != null) {
			...
			httpResponse =
				shippingMethodResource.
					postCartShippingMethodsPageExportBatchHttpResponse(
						irrelevantCartId, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			shippingMethods = getShippingMethods(exportTask);

			Assert.assertEquals(1, shippingMethods.length);

			assertEquals(irrelevantShippingMethod, shippingMethods[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, shippingMethods.length);

		assertContains(shippingMethod1, Arrays.asList(shippingMethods));
		assertContains(shippingMethod2, Arrays.asList(shippingMethods));
	}
	...
	protected ShippingMethod[] getShippingMethods(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ShippingMethodSerDes.toDTOs(
