...
public abstract class BaseAccountResourceTestCase {
	...
	public void testGetChannelAccountsPageWithPagination() throws Exception {
		...
		Page<Account> page1 = accountResource.getChannelAccountsPage(
			channelId, null, null, Pagination.of(1, itemLimit), null);

		List<Account> accounts1 = (List<Account>)page1.getItems();

		if (accounts1.size() < itemLimit) {
			itemLimit = accounts1.size();
		}

		int pages = (int)Math.ceil(accountPage.getTotalCount() / itemLimit);
		List<Account> allItems = new ArrayList<Account>();
		...
		if (pages > 2) {
			for (int pageNum = 2; pageNum < pages; pageNum++) {
				Assert.assertEquals(
					accounts1.toString(), itemLimit, accounts1.size());

				Page<Account> page = accountResource.getChannelAccountsPage(
					channelId, null, null, Pagination.of(pageNum, itemLimit),
					null);
		...
		assertContains(account1, allItems);
		assertContains(account2, allItems);
		assertContains(account3, allItems);
	}
