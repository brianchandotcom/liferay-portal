			...
			<span class="message-info">
				<c:choose>
					<c:when test="<%= layoutTypePortlet.isCustomizedView() %>">
						...
						<liferay-ui:icon-help message="customizable-user-help" resourceBundle="<%= resourceBundle %>" />
					</c:when>
					<c:otherwise>
						...
						<c:if test="<%= informationMessagesControlMenuEntry.hasUpdateLayoutPermission(themeDisplay) %>">
							<liferay-ui:icon-help message="customizable-admin-help" resourceBundle="<%= resourceBundle %>" />
						</c:if>
					</c:otherwise>
				</c:choose>
			</span>
