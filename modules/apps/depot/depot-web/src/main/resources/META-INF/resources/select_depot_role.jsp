...
<clay:management-toolbar displayContext="<%= (DepotAdminSelectRoleManagementToolbarDisplayContext)request.getAttribute(DepotAdminWebKeys.DEPOT_ADMIN_SELECT_ROLE_MANAGEMENT_TOOLBAL_DISPLAY_CONTEXT) %>" />
...
>
	<c:choose>
		<c:when test="<%= depotAdminSelectRoleDisplayContext.isStep1() %>">
			...
			<liferay-ui:search-container searchContainer="<%= step1.getSearchContainer() %>">
				...
				>
					<liferay-ui:search-container-column-text name="name" value="<%= HtmlUtil.escape(group.getDescriptiveName(locale)) %>" />

					<liferay-ui:search-container-column-text name="roleType" value="<%= LanguageUtil.get(request, group.getTypeLabel()) %>" />
		...
		<c:when test="<%= depotAdminSelectRoleDisplayContext.isStep2() %>">
			...
			<liferay-ui:search-container headerNames="name" searchContainer="<%= step2.getSearchContainer() %>">
				<liferay-ui:search-container-row
