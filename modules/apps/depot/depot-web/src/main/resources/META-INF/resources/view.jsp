				...
				>
					...
					<c:choose>
						<c:when test="<%= depotAdminDisplayContext.isDisplayStyleDescriptive() %>">
							...
							>
								<h5>
									<aui:a cssClass="selector-button" href="<%= depotAdminDisplayContext.getEditDepotURL(curGroup) %>">
						...
						<c:otherwise>
							<liferay-ui:search-container-column-text
								...
								orderable="<%= true %>"
							>
								<aui:a href="<%= depotAdminDisplayContext.getEditDepotURL(curGroup) %>" label="<%= HtmlUtil.escape(curGroup.getDescriptiveName(locale)) %>" localizeLabel="<%= false %>" />
							</liferay-ui:search-container-column-text>
