		...
		<c:choose>
			<c:when test="<%= showThumbnail && ImageProcessorUtil.hasImages(fileVersion) %>">
				<div class="asset-thumbnail">
					<img alt="<liferay-ui:message escapeAttribute="<%= true %>" key="image" />" class="img-thumbnail" src="<%= dlurlHelper.getPreviewURL(fileEntry, fileVersion, themeDisplay, "&imageThumbnail=1") %>" style="<%= DLUtil.getThumbnailStyle(true, 0, 128, 128) %>" />
				</div>
			...
			<c:when test="<%= showThumbnail && PDFProcessorUtil.hasImages(fileVersion) %>">
				<div class="asset-thumbnail">
					<img alt="<liferay-ui:message escapeAttribute="<%= true %>" key="document" />" class="img-thumbnail" src="<%= dlurlHelper.getPreviewURL(fileEntry, fileVersion, themeDisplay, "&documentThumbnail=1") %>" style="<%= DLUtil.getThumbnailStyle(true, 0, 128, 128) %>" />
				</div>
			...
			<c:when test="<%= showThumbnail && VideoProcessorUtil.hasVideo(fileVersion) %>">
				<div class="asset-thumbnail">
					<img alt="<liferay-ui:message escapeAttribute="<%= true %>" key="video" />" class="img-thumbnail" src="<%= dlurlHelper.getPreviewURL(fileEntry, fileVersion, themeDisplay, "&videoThumbnail=1") %>" style="<%= DLUtil.getThumbnailStyle(true, 0, 128, 128) %>" />
				</div>
