...
create unique index IX_E7F5E74F on DDLRecord (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);

create index IX_6705D180 on DDLRecordSet (DDMStructureId);
create unique index IX_6B9D0467 on DDLRecordSet (groupId, recordSetKey[$COLUMN_LENGTH:75$], companyId);
create index IX_5938C39F on DDLRecordSet (uuid_[$COLUMN_LENGTH:75$], companyId);
create unique index IX_B44797A7 on DDLRecordSet (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);

create index IX_1C4E1CC9 on DDLRecordSetVersion (recordSetId, status);
create unique index IX_DC8C1A83 on DDLRecordSetVersion (recordSetId, version[$COLUMN_LENGTH:75$], companyId);

create index IX_762ADC7 on DDLRecordVersion (recordId, status);
create unique index IX_23491E81 on DDLRecordVersion (recordId, version[$COLUMN_LENGTH:75$], companyId);
