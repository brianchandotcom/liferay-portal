...
public class ImportPreviewResourceImplTest {
	...
	public void setUp() {
		...
		_exportedGroupProvider = Mockito.mock(ExportedGroupProvider.class);

		ReflectionTestUtil.setFieldValue(
			_importPreviewResourceImpl, "_exportedGroupProvider",
			_exportedGroupProvider);
	...
	public void testGetPreviewSitesWhenSiteExportImportIsDisabled()
		...
		Mockito.when(
			_exportedGroupProvider.getExportedGroups(_fileEntry)
		).thenReturn(
			ListUtil.fromArray(Mockito.mock(ExportedGroup.class))
		);

		Assert.assertEquals(0, _getPreviewSites(false).length);

		Mockito.verify(
			_exportedGroupProvider, Mockito.never()
		).getExportedGroups(
			_fileEntry
		);
	}
