...
public class GroupImporter {
	...
	public GroupImporter(
		...
		ExportedGroupProvider exportedGroupProvider,
		ExportImportGroupHelper exportImportGroupHelper,
		...
		_exportedGroupProvider = exportedGroupProvider;
		_exportImportGroupHelper = exportImportGroupHelper;
	...
	public void importGroups(
		...
		List<ExportedGroup> exportedGroups =
			_exportedGroupProvider.getExportedGroups(portletDataContext);
		...
		for (ExportedGroup exportedGroup : exportedGroups) {
			...
			Group group = _fetchGroup(portletDataContext, exportedGroup);

			if (group == null) {
				continue;
			}

			_updateParentGroup(portletDataContext, group, exportedGroup);

			unsafeBiConsumer.accept(
				_createPortletDataContext(
					portletDataContext, exportedGroup, group),
				userId);
		}
	...
	private PortletDataContext _createPortletDataContext(
			PortletDataContext portletDataContext, ExportedGroup exportedGroup,
			Group group)
		throws Exception {
		...
		Element rootElement = _getRootElement(
			groupPortletDataContext, exportedGroup.getGroupId());
		...
		groupPortletDataContext.setSourceGroupId(exportedGroup.getGroupId());
	...
	private Group _fetchGroup(
		...
		try {
			...
			if (group == null) {
				...
				_addReportEntry(
					...
					StringBundler.concat(
						...
						" does not exist in the target system. Create the ",
			...
			if (!_exportImportGroupHelper.isGroupSupported(group)) {
	...
	private void _updateParentGroup(
		...
		String parentGroupExternalReferenceCode =
			exportedGroup.getParentGroupExternalReferenceCode();
		...
		if (parentGroup == null) {
			_addReportEntry(
				portletDataContext, exportedGroup.getExternalReferenceCode(),
				StringBundler.concat(
					"The parent group ",
					exportedGroup.getParentGroupExternalReferenceCode(),
					" for ", exportedGroup.getDescriptiveName(),
		...
		try {
			...
			_addReportEntry(
				...
				StringBundler.concat(
					...
					exportedGroup.getParentGroupExternalReferenceCode(),
					" for ", exportedGroup.getDescriptiveName(),
					" is below it in the target system. Leaving the group ",
					"where it is"),
				ExportImportReportEntryConstants.TYPE_WARNING);
		}
		catch (PortalException portalException) {
			...
			_addReportEntry(
				...
				StringBundler.concat(
					...
					exportedGroup.getDescriptiveName(),
	...
	private final ExportedGroupProvider _exportedGroupProvider;
	private final ExportImportGroupHelper _exportImportGroupHelper;
