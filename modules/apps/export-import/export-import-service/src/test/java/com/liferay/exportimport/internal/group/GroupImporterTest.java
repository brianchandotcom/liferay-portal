...
public class GroupImporterTest {
	...
	public void setUp() throws Exception {
		...
		_exportImportGroupHelper = Mockito.mock(ExportImportGroupHelper.class);
		...
		_groupImporter = new GroupImporter(
			Mockito.mock(ClassNameLocalService.class), _exportedGroupProvider,
		...
		Mockito.when(
			_exportImportGroupHelper.isGroupSupported(Mockito.any())
		).thenReturn(
			true
		);
	...
	public void testImportGroupsWhenGroupExportImportIsDisabled()
		...
		_setUpExportedGroups(_createExportedGroup("group", null));
		...
		Mockito.verifyNoInteractions(_exportedGroupProvider);
	}
	...
	@Test
	public void testImportGroupsWhenGroupIsNotMovable() throws Exception {
		_setUpExportedGroups(
			_createExportedGroup("child", "parent"),
			_createExportedGroup("other", null));
	...
	public void testImportGroupsWhenGroupIsNotSelected() throws Exception {
		_setUpExportedGroups(
			_createExportedGroup("selected", null),
			_createExportedGroup("unselected", null));
	...
	public void testImportGroupsWhenGroupIsNotSupported() throws Exception {
		...
		Group group = _setUpGroups("group");

		Mockito.when(
			_exportImportGroupHelper.isGroupSupported(group)
		).thenReturn(
			false
		);
	...
	@Test
	public void testImportGroupsWhenManifestHasHeader() throws Exception {
		_setUpExportedGroups(_createExportedGroup("group", null));
		_setUpGroups("group");
	...
	@Test
	public void testImportGroupsWhenParentGroupIsBelowTheGroup()
		throws Exception {

		_setUpExportedGroups(_createExportedGroup("child", "parent"));
		...
		Assert.assertEquals(Arrays.asList("child"), _importGroups("child"));
	...
	@Test
	public void testImportGroupsWhenPassIsGroupScoped() throws Exception {
		Mockito.when(
			_portletDataContext.getParameterMap()
		).thenReturn(
	...
	public void testImportGroupsWhenSelectedGroupIsMissingFromTheLARFile()
		...
		_setUpExportedGroups(_createExportedGroup("carried", null));
	...
	public void testUpdateParentGroup() throws Exception {
		...
		_updateParentGroup(_createExportedGroup("child", "parent"), group);
	...
	public void testUpdateParentGroupWhenGroupKeyIsDuplicated()
		...
		_runWithExpectedWarn(
			...
			() -> _updateParentGroup(
				_createExportedGroup("child", "parent"), _mockGroup(0)));
	...
	public void testUpdateParentGroupWhenParentGroupExternalReferenceCodeIsNull()
		...
		_updateParentGroup(_createExportedGroup("child", null), _mockGroup(0));
	...
	public void testUpdateParentGroupWhenParentGroupIsMissing()
		...
		ExportedGroup exportedGroup = _createExportedGroup("child", "parent");

		_updateParentGroup(exportedGroup, _mockGroup(0));
	...
	private ExportedGroup _createExportedGroup(
		String externalReferenceCode, String parentExternalReferenceCode) {

		return new ExportedGroup(
			0, "Group", externalReferenceCode, _SOURCE_GROUP_ID,
	...
	private void _setUpExportedGroups(ExportedGroup... exportedGroups)
		throws Exception {

		Mockito.when(
			_exportedGroupProvider.getExportedGroups(_portletDataContext)
		).thenReturn(
			ListUtil.fromArray(exportedGroups)
		);
	}
	...
	private void _updateParentGroup(ExportedGroup exportedGroup, Group group) {
		ReflectionTestUtil.invoke(
			_groupImporter, "_updateParentGroup",
			new Class<?>[] {
				PortletDataContext.class, Group.class, ExportedGroup.class
			},
			_portletDataContext, group, exportedGroup);
	}
