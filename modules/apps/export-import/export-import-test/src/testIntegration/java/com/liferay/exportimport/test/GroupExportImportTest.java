...
public class GroupExportImportTest {
	...
	public void testExportGroups() throws Exception {
		...
		List<ExportedGroup> exportedGroups = _getExportedGroups(file);
		...
		Assert.assertEquals(_group.getGroupId(), exportedGroup.getGroupId());
		Assert.assertEquals(
			_group.getDescriptiveName(
				LocaleUtil.fromLanguageId(_group.getDefaultLanguageId())),
			exportedGroup.getDescriptiveName());

		Assert.assertNull(exportedGroup.getParentGroupExternalReferenceCode());

		String path = exportedGroup.getPath();
	...
	public void testExportGroupsWithChildGroup() throws Exception {
		...
		ExportedGroup childExportedGroup = _getExportedGroup(
			exportedGroups, childGroup.getExternalReferenceCode());

		Assert.assertEquals(
			_group.getExternalReferenceCode(),
			childExportedGroup.getParentGroupExternalReferenceCode());

		ExportedGroup exportedGroup = _getExportedGroup(
			exportedGroups, _group.getExternalReferenceCode());

		Assert.assertEquals(1, exportedGroup.getChildGroupCount());
	}
	...
	private ExportedGroup _getExportedGroup(
		List<ExportedGroup> exportedGroups, String externalReferenceCode) {
	...
	private List<ExportedGroup> _getExportedGroups(File file) throws Exception {
		...
		try {
			return _exportedGroupProvider.getExportedGroups(fileEntry);
		}
		finally {
