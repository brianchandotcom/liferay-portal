...
function getMonthDayListText(monthDays: number[], locale: string): string {
	const monthDayRanges = toMonthDayRanges(monthDays);
	...
	return sub(
		...
		getListText(monthDayRanges, locale)
	);
}
...
function getMonthlyRepeatText(
	...
	if (scheduleValues.repeatType === RepeatType.DayOfWeek) {
		...
		return sub(
			Liferay.Language.get('repeats-in-x-on-the-x'),
			monthListText,
			weekdayOrdinalText
		);
	}
	...
	if (
		...
		if (everyMonth) {
			return sub(Liferay.Language.get('repeats-every-x'), unitText);
		}

		return sub(Liferay.Language.get('repeats-in-x'), monthListText);
	}
	...
	return sub(
		Liferay.Language.get('repeats-in-x-on-x'),
		monthListText,
		monthDayListText
	);
}
...
function getYearlyRepeatText(
	...
	if (scheduleValues.repeatType === RepeatType.DayOfWeek) {
		return sub(
			Liferay.Language.get('repeats-every-x-in-x-on-the-x'),
			unitText,
			monthListText,
	...
	return sub(
		Liferay.Language.get('repeats-every-x-in-x-on-x'),
		unitText,
		monthListText,
		getMonthDayListText(
...
function getRepeatText(scheduleValues: ScheduleValues, locale: string): string {
	if (scheduleValues.unit === IntervalUnit.Day) {
		return sub(
			Liferay.Language.get('repeats-every-x'),
