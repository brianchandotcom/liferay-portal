...
<c:choose>
	<c:when test="<%= categoryKeys.length > 1 %>">
		...
		>
			...
			for (String categoryKey : categoryKeys) {
				request.setAttribute(FormNavigatorKeys.CURRENT_TAB, categoryKey);
				request.setAttribute(FormNavigatorKeys.FORM_NAVIGATOR_ENTRIES, FormNavigatorEntryUtil.getFormNavigatorEntries(id, categoryKey, user, formModelBean));
			%>
			...
			String errorTab = (String)request.getAttribute(FormNavigatorKeys.ERROR_TAB);
	...
	<c:otherwise>
		...
		<%
		request.setAttribute(FormNavigatorKeys.FORM_NAVIGATOR_ENTRIES, FormNavigatorEntryUtil.getFormNavigatorEntries(id, user, formModelBean));
		%>
