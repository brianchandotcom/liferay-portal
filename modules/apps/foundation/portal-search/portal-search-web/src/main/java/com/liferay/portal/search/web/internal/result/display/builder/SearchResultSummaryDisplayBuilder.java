...
public class SearchResultSummaryDisplayBuilder {
	...
	protected SearchResultSummaryDisplayContext build(
		...
		if (assetEntry != null) {
			searchResultSummaryDisplayContext.assetEntryUserId =
				getAssetEntryUserId(assetEntry);
			searchResultSummaryDisplayContext.hasUserPortrait = true;
		}

		searchResultSummaryDisplayContext.viewURL = checkViewURL(viewURL);

		searchResultSummaryDisplayContext.highlightedTitle =
			summary.getHighlightedTitle();

		if (hasAssetRendererURLDownload(assetRenderer)) {
			searchResultSummaryDisplayContext.assetRendererURLDownload =
				assetRenderer.getURLDownload(_themeDisplay);
			searchResultSummaryDisplayContext.hasAssetRendererURLDownload =
				true;
			searchResultSummaryDisplayContext.title = summary.getTitle();
		}

		searchResultSummaryDisplayContext.modelResource =
		...
		if (_locale != summary.getLocale()) {
			...
			searchResultSummaryDisplayContext.hasLocaleReminder = true;
			searchResultSummaryDisplayContext.localeLanguageId =
				LocaleUtil.toLanguageId(summaryLocale);
			searchResultSummaryDisplayContext.localeReminder = _language.format(
				_request,
		...
		if (Validator.isNotNull(summary.getContent())) {
			searchResultSummaryDisplayContext.content =
				summary.getHighlightedContent();
			searchResultSummaryDisplayContext.hasContent = true;
		}

		if (hasAssetCategoriesOrTags(assetEntry)) {
			searchResultSummaryDisplayContext.className = className;
			searchResultSummaryDisplayContext.classPK = classPK;
			searchResultSummaryDisplayContext.hasAssetCategoriesOrTags = true;
			searchResultSummaryDisplayContext.fieldAssetCategoryIds =
				Field.ASSET_CATEGORY_IDS;
			searchResultSummaryDisplayContext.fieldAssetTagNames =
				Field.ASSET_TAG_NAMES;
			searchResultSummaryDisplayContext.portletURL =
				_portletURLFactory.getPortletURL();
		}

		if (_searchResultPreferences.isDisplayResultsInDocumentForm()) {
			searchResultSummaryDisplayContext.documentFormFieldDisplayContexts =
				buildFields();
			searchResultSummaryDisplayContext.hasDocumentForm = true;
		}
	...
	protected SearchResultFieldDisplayContext buildField(Field field) {
		...
		searchResultFieldDisplayContext.boost = field.getBoost();
		searchResultFieldDisplayContext.isArray = isArray(field);
		searchResultFieldDisplayContext.isNumeric = field.isNumeric();
		searchResultFieldDisplayContext.isTokenized = field.isTokenized();
		searchResultFieldDisplayContext.name = field.getName();
		searchResultFieldDisplayContext.valuesToString = getValuesToString(
			field);
