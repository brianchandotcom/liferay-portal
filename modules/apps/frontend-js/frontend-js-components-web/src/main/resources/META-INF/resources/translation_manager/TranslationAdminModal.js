...
const TranslationAdminModal = ({
	activeLocaleIds: initialActiveLocaleIds = emptyArray,
	ariaLabels = {
	...
	const [activeLocaleIds, setActiveLocaleIds] = useState(
		initialActiveLocaleIds
	);
	...
	const {observer} = useModal({
		onClose: () => {
			setVisible(false);
			onClose([...activeLocaleIds]);
		},
	});

	const handleAddLocale = (localeId) => {
		setActiveLocaleIds([...activeLocaleIds, localeId]);
	...
	const handleRemoveLocale = (localeId) => {
		const newActiveLocaleIds = [...activeLocaleIds];
		newActiveLocaleIds.splice(activeLocaleIds.indexOf(localeId), 1);
		setActiveLocaleIds(newActiveLocaleIds);
	...
	useEffect(() => {
		setActiveLocaleIds(initialActiveLocaleIds);
	}, [initialActiveLocaleIds]);
...
TranslationAdminModal.propTypes = {
	activeLocaleIds: PropTypes.arrayOf(PropTypes.string),
	...
	onActiveLocaleIdsChange: PropTypes.func,
