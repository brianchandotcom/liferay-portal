...
public class RegionResourceTest extends BaseRegionResourceTestCase {
	...
	private void _testGetRegionsPageWithFilter() throws Exception {
		...
		// Sleep for 1 second to ensure that region1 and region2 are created
		// 1 second apart

		Thread.sleep(1000);

		Region region1 = _addRegion(keyword);

		Thread.sleep(1000);

		Region region2 = _addRegion(keyword);
		...
		page = regionResource.getRegionsPage(
			null, keyword,
			"dateCreated ge " + dateFormat.format(region1.getDateModified()),
			Pagination.of(1, 2), null);

		Assert.assertEquals(2, page.getTotalCount());

		// Sleep for 1 second to ensure that region1 and region2 are modified
		// 1 second apart

		Thread.sleep(1000);

		region1.setName(
			keyword + StringUtil.toLowerCase(RandomTestUtil.randomString()));
