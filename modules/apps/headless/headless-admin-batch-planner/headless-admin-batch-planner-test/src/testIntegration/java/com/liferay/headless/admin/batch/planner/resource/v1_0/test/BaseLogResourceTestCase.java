...
public abstract class BaseLogResourceTestCase {
	...
	@Test
	public void testGetLogsPage() throws Exception {
		Page<Log> page = logResource.getLogsPage(
			testGetLogsPage_getId(), Pagination.of(1, 2));

		Assert.assertEquals(0, page.getTotalCount());

		Long id = testGetLogsPage_getId();
		Long irrelevantId = testGetLogsPage_getIrrelevantId();

		if (irrelevantId != null) {
			Log irrelevantLog = testGetLogsPage_addLog(
				irrelevantId, randomIrrelevantLog());

			page = logResource.getLogsPage(irrelevantId, Pagination.of(1, 2));
		...
		Log log1 = testGetLogsPage_addLog(id, randomLog());

		Log log2 = testGetLogsPage_addLog(id, randomLog());

		page = logResource.getLogsPage(id, Pagination.of(1, 2));
	...
	@Test
	public void testGetLogsPageWithPagination() throws Exception {
		Long id = testGetLogsPage_getId();
		...
		Log log3 = testGetLogsPage_addLog(id, randomLog());

		Page<Log> page1 = logResource.getLogsPage(id, Pagination.of(1, 2));

		List<Log> logs1 = (List<Log>)page1.getItems();

		Assert.assertEquals(logs1.toString(), 2, logs1.size());

		Page<Log> page2 = logResource.getLogsPage(id, Pagination.of(2, 2));
		...
		Page<Log> page3 = logResource.getLogsPage(id, Pagination.of(1, 3));

		assertEqualsIgnoringOrder(
			Arrays.asList(log1, log2, log3), (List<Log>)page3.getItems());
	}

	protected Log testGetLogsPage_addLog(Long id, Log log) throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}

	protected Long testGetLogsPage_getId() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}

	protected Long testGetLogsPage_getIrrelevantId() throws Exception {
		return null;
	}

	@Test
	public void testGraphQLGetLogsPage() throws Exception {
		Long id = testGetLogsPage_getId();
		...
		JSONObject logsJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(graphQLField), "JSONObject/data",
			"JSONObject/logs");

		Assert.assertEquals(0, logsJSONObject.get("totalCount"));

		Log log1 = testGraphQLLog_addLog();
		Log log2 = testGraphQLLog_addLog();

		logsJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(graphQLField), "JSONObject/data",
			"JSONObject/logs");

		Assert.assertEquals(2, logsJSONObject.get("totalCount"));

		assertEqualsIgnoringOrder(
			Arrays.asList(log1, log2),
			Arrays.asList(LogSerDes.toDTOs(logsJSONObject.getString("items"))));
	}
