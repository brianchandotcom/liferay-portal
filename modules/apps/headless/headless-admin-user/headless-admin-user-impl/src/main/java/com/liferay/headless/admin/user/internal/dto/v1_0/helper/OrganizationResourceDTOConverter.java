...
public class OrganizationResourceDTOConverter
	...
	@Override
	public Organization toDTO(DTOConverterContext dtoConverterContext)
		throws Exception {

		com.liferay.portal.kernel.model.Organization organization = getObject(
			(String)dtoConverterContext.getId());

		return _toOrganization(dtoConverterContext, organization);
	}

	private static HoursAvailable _createHoursAvailable(
	...
	private static String _formatHour(int hour) {
		if (hour == -1) {
			return null;
		}
	...
	private static Service _toService(OrgLabor orgLabor) throws Exception {
		...
		return new Organization() {
			{
				...
				services = transformToArray(
					_orgLaborService.getOrgLabors(
						organization.getOrganizationId()),
					OrganizationResourceDTOConverter::_toService,
					Service.class);
