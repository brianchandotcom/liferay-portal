...
public abstract class BaseWorkflowInstanceResourceTestCase {
	...
	public void testPostWorkflowInstancesPageExportBatch() throws Exception {
		...
		WorkflowInstance[] workflowInstances = getWorkflowInstances(exportTask);

		long totalCount = workflowInstances.length;
		...
		workflowInstances = getWorkflowInstances(exportTask);

		Assert.assertEquals(totalCount + 2, workflowInstances.length);

		assertContains(workflowInstance1, Arrays.asList(workflowInstances));
		assertContains(workflowInstance2, Arrays.asList(workflowInstances));
	}
	...
	protected WorkflowInstance[] getWorkflowInstances(ExportTask exportTask)
		throws Exception {
		...
		try {
			return WorkflowInstanceSerDes.toDTOs(
