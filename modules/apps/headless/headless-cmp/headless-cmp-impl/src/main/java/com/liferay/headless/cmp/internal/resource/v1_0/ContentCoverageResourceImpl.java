...
public class ContentCoverageResourceImpl
	...
	public ContentCoverage getProjectContentCoverage(Long projectId)
		...
		booleanQuery.addFilterQueryClauses(
			_createLinkedObjectEntriesQuery(objectEntry),
	...
	private BooleanQuery _createLinkedObjectEntriesQuery(
		...
		if (relatedCMPTaskObjectEntryIds.length > 0) {
			TermsQuery termsQuery = QueriesUtil.terms("cmpTaskObjectEntryIds");

			for (long relatedCMPTaskObjectEntryId :
					relatedCMPTaskObjectEntryIds) {

				termsQuery.addValue(
					String.valueOf(relatedCMPTaskObjectEntryId));
			}
