...
public class ContentCoverageResourceImpl
	...
	public ContentCoverage getProjectContentCoverage(Long projectId)
		...
		List<AssetCategory> funnelStageAssetCategories = _getAssetCategories(
			assetCategories, "L_CMP_FUNNEL_STAGE");

		List<AssetCategory> personaAssetCategories = _getAssetCategories(
			assetCategories, "L_CMP_PERSONAS");

		SearchResponse searchResponse = _search(
			assetTagNames, funnelStageAssetCategories, personaAssetCategories);
		...
		return new ContentCoverage() {
			{
				...
				setPersonas(
					() -> transformToArray(
						...
						Persona.class));
				setTotalAssetCount(searchResponse::getCount);
			}
		};
	}
	...
	private SearchResponse _search(
		...
		FiltersAggregation filtersAggregation = _createFiltersAggregation(
			personaAssetCategories, _PERSONAS_AGGREGATION_NAME);

		filtersAggregation.addChildAggregation(
			_createFiltersAggregation(
				funnelStageAssetCategories, _FUNNEL_STAGES_AGGREGATION_NAME));
			...
			for (Bucket funnelStageBucket :
				...
				contentCoverageEntries.add(
					new ContentCoverageEntry() {
						{
							setFunnelStageId(funnelStageBucket::getKey);
							setPersonaId(personaBucket::getKey);
							setTotalCount(funnelStageBucket::getDocCount);
						}
					});
			}
		}
