...
public class DataMaskTest {
	...
	public void testApply() {
		...
		Assert.assertEquals(
			...
			_apply(
				new DataMask(
					_secretDetectionPattern, "Secrets", null, "[$1\\X]"),
				"value: secret"));
		Assert.assertEquals(
	...
	private DataMask _emailMask() {
		return new DataMask(
			_emailDetectionPattern, "Email addresses", null, "[EMAIL_ADDRESS]");
	}

	private DataMask _ipv6Mask() {
		return new DataMask(
			_ipv6DetectionPattern, "IPv6 → /48", _ipv6ReplacementPattern,
			"::/48");
	}
