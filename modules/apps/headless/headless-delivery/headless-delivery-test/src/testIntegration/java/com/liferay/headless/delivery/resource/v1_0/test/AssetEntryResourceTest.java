...
public class AssetEntryResourceTest extends BaseAssetEntryResourceTestCase {
	...
	private void _testGetAssetEntriesPagePermissions() throws Exception {
		...
		BlogsEntry viewableBlogsEntry = _addBlogsEntry(groupId);
		BlogsEntry notViewableBlogsEntry = _addBlogsEntry(groupId);
		...
		_resourcePermissionLocalService.removeResourcePermission(
			...
			String.valueOf(notViewableBlogsEntry.getEntryId()),
		...
		AssetEntry viewableAssetEntry = _getAssetEntry(
			nestedFieldsAssetEntries, viewableBlogsEntry.getEntryId());

		Assert.assertTrue(
			ArrayUtil.exists(
				viewableAssetEntry.getPermissions(),
				permission ->
					Objects.equals(
		...
		AssetEntry notViewableAssetEntry = _getAssetEntry(
			nestedFieldsAssetEntries, notViewableBlogsEntry.getEntryId());

		Assert.assertFalse(
			ArrayUtil.exists(
				notViewableAssetEntry.getPermissions(),
				permission ->
					Objects.equals(
						permission.getRoleName(), RoleConstants.GUEST) &&
					ArrayUtil.contains(
		...
		Assert.assertNull(
			_getAssetEntry(
				assetEntries, viewableBlogsEntry.getEntryId()
			).getPermissions());
	}
