...
public abstract class BaseContentStructureResourceTestCase {
	...
	public void testPostAssetLibraryContentStructuresPageExportBatch()
		...
		ContentStructure[] contentStructures = getContentStructures(exportTask);

		long totalCount = contentStructures.length;
		...
		if (irrelevantAssetLibraryId != null) {
			...
			contentStructures = getContentStructures(exportTask);

			Assert.assertEquals(1, contentStructures.length);

			assertEquals(irrelevantContentStructure, contentStructures[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, contentStructures.length);

		assertContains(contentStructure1, Arrays.asList(contentStructures));
		assertContains(contentStructure2, Arrays.asList(contentStructures));
	}
	...
	protected ContentStructure[] getContentStructures(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ContentStructureSerDes.toDTOs(
