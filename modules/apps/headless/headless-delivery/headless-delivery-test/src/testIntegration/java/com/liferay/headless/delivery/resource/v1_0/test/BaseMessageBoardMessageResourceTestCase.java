...
public abstract class BaseMessageBoardMessageResourceTestCase {
	...
	@Test
	public void testPostMessageBoardThreadMessageBoardMessagesPageExportBatch()
		throws Exception {
		...
		HttpInvoker.HttpResponse httpResponse =
			messageBoardMessageResource.
				postMessageBoardThreadMessageBoardMessagesPageExportBatchHttpResponse(
					messageBoardThreadId, null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		MessageBoardMessage[] messageBoardMessages = getMessageBoardMessages(
			exportTask);

		long totalCount = messageBoardMessages.length;
		...
		if (irrelevantMessageBoardThreadId != null) {
			...
			httpResponse =
				messageBoardMessageResource.
					postMessageBoardThreadMessageBoardMessagesPageExportBatchHttpResponse(
						irrelevantMessageBoardThreadId, null, null, null, null,
						null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			messageBoardMessages = getMessageBoardMessages(exportTask);

			Assert.assertEquals(1, messageBoardMessages.length);

			assertEquals(
				irrelevantMessageBoardMessage, messageBoardMessages[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, messageBoardMessages.length);

		assertContains(
			messageBoardMessage1, Arrays.asList(messageBoardMessages));
		assertContains(
			messageBoardMessage2, Arrays.asList(messageBoardMessages));
	}
	...
	@Test
	public void testPostSiteMessageBoardMessagesPageExportBatch()
		throws Exception {
	...
	protected MessageBoardMessage[] getMessageBoardMessages(
		...
		try {
			return MessageBoardMessageSerDes.toDTOs(
