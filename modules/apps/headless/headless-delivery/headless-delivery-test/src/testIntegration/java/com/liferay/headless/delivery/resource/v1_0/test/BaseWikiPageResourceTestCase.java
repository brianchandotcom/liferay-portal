...
public abstract class BaseWikiPageResourceTestCase {
	...
	@Test
	public void testPostWikiNodeWikiPagesPageExportBatch() throws Exception {
		Long wikiNodeId = testGetWikiNodeWikiPagesPage_getWikiNodeId();
		Long irrelevantWikiNodeId =
			testGetWikiNodeWikiPagesPage_getIrrelevantWikiNodeId();

		HttpInvoker.HttpResponse httpResponse =
			wikiPageResource.postWikiNodeWikiPagesPageExportBatchHttpResponse(
				wikiNodeId, null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		WikiPage[] wikiPages = getWikiPages(exportTask);

		long totalCount = wikiPages.length;
			...
			httpResponse =
				wikiPageResource.
					postWikiNodeWikiPagesPageExportBatchHttpResponse(
						irrelevantWikiNodeId, null, null, null, null, null,
						null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			wikiPages = getWikiPages(exportTask);

			Assert.assertEquals(1, wikiPages.length);

			assertEquals(irrelevantWikiPage, wikiPages[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, wikiPages.length);

		assertContains(wikiPage1, Arrays.asList(wikiPages));
		assertContains(wikiPage2, Arrays.asList(wikiPages));
	}
	...
	protected WikiPage[] getWikiPages(ExportTask exportTask) throws Exception {
