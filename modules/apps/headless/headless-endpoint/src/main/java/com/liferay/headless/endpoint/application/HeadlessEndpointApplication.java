...
package com.liferay.headless.endpoint.application;

import com.liferay.headless.endpoint.dto.Hint;
import com.liferay.headless.endpoint.dto.Resource;
import com.liferay.headless.endpoint.dto.Resources;
...
@Component(
	property = {
		JaxrsWhiteboardConstants.JAX_RS_APPLICATION_BASE + "=/endpoint",
		JaxrsWhiteboardConstants.JAX_RS_EXTENSION_SELECT + "=(osgi.jaxrs.name=Liferay.Vulcan)",
		JaxrsWhiteboardConstants.JAX_RS_NAME + "=Liferay.Headless.Endpoint"
	},
	service = Application.class
)
public class HeadlessEndpointApplication extends Application {
	...
	private Resource _getResource(
		...
		String[] verbs = stream.map(
			resourceMethodInfoDTO1 -> resourceMethodInfoDTO1.method
		).toArray(
			String[]::new
		);
	...
	private Map<String, List<ResourceMethodInfoDTO>>
		...
		String serverURL = StringUtil.replace(absolutePath, "/endpoint/", "");
