			...
			<c:when test="<%= Validator.isNotNull(journalContentDisplayContext.getArticleId()) %>">
				<c:choose>
					...
					<c:when test="<%= article.isScheduled() %>">
						<div class="alert alert-warning">
							<liferay-ui:message arguments='<%= new Object[] {HtmlUtil.escape(article.getTitle(locale)), DateUtil.getDate(article.getDisplayDate(), "dd MMM yyyy - HH:mm:ss", locale)} %>' key="x-is-scheduled-and-will-be-displayed-on-x" />
						</div>
					</c:when>
