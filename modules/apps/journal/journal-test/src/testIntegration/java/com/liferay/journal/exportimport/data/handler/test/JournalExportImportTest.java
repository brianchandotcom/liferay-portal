...
public class JournalExportImportTest extends BasePortletExportImportTestCase {
	...
	public void testExportImportJournalFolderWithWorkflowRestriction()
		...
		folder = JournalFolderLocalServiceUtil.updateFolder(
			TestPropsValues.getUserId(), group.getGroupId(),
			folder.getFolderId(), folder.getParentFolderId(), folder.getName(),
			folder.getDescription(), new long[0],
		...
		JournalFolder importedFolder =
			JournalFolderLocalServiceUtil.fetchJournalFolderByUuidAndGroupId(
				folderUuid, importedGroup.getGroupId());

		Assert.assertEquals(
			JournalFolderConstants.RESTRICTION_TYPE_WORKFLOW,
			importedFolder.getRestrictionType());
	...
	private String _buildImageDynamicElementXML(FileEntry fileEntry) {
		return StringBundler.concat(
			...
			"<![CDATA[{\"alt\": \"alt text\",\"groupId\": \"",
			fileEntry.getGroupId(), "\",\"uuid\": \"", fileEntry.getUuid(),
	...
	private JournalArticle _exportPortletWithMissingArticleReference()
		...
		try {
			JournalArticle referencedArticle =
				JournalTestUtil.addArticleWithXMLContent(
					randomGroup.getGroupId(),
					JournalFolderConstants.DEFAULT_PARENT_FOLDER_ID,
					JournalArticleConstants.CLASS_NAME_ID_DEFAULT,
					_buildXMLContent("{}"),
					_addDataDefinition(randomGroup.getGroupId()), null,
					LocaleUtil.US);
