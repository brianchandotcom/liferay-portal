...
public class KBArticleImporterUtil {
	...
	public static FileEntry addImageFileEntry(
		...
		return addImageFileEntry(
			userId, kbArticle, imageFileName, inputStream, new HashMap<>());
	}
	...
	protected static FileEntry addImageFileEntry(
		...
		try {
			...
			int newImageSize = getStreamSize(inputStream);
			long existingImageSize = fileEntry.getSize();

			if (newImageSize != existingImageSize) {
	...
	protected static int getStreamSize(InputStream is) throws IOException {
		int chunk = 0;
		int size = 0; //is.mark(0);
