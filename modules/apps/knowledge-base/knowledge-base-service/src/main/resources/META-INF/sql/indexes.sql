...
create unique index IX_7C6A6D34 on KBArticle (resourcePrimKey, groupId, version, companyId);
...
create unique index IX_A7430176 on KBArticle (resourcePrimKey, version, companyId);
create index IX_4E87D659 on KBArticle (uuid_[$COLUMN_LENGTH:75$], companyId);
create unique index IX_EF28F62D on KBArticle (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);
...
create unique index IX_32EDD964 on KBComment (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);
...
create unique index IX_2A59A2E7 on KBFolder (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);

create index IX_83D9CC13 on KBTemplate (groupId);
create index IX_853770AB on KBTemplate (uuid_[$COLUMN_LENGTH:75$], companyId);
create unique index IX_3AECD81B on KBTemplate (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);