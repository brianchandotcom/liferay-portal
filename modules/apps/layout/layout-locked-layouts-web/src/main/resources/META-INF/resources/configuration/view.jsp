...
<clay:sheet-section role="group" aria-labelledby='<%= liferayPortletResponse.getNamespace() + "automaticUnlockingTitle" %>'>
	...
	>
		<clay:content-col
			expand="<%= true %>"
		>
			<aui:input aria-describedby='<%= liferayPortletResponse.getNamespace() + "autosaveTimeHiddenDescription" %>' label="autosave-time" name="autosaveTime" required="<%= true %>" type="number" value="<%= lockedLayoutsConfigurationDisplayContext.getAutosaveTime() %>" wrapperCssClass="c-mb-1">
				<aui:validator name="number" />
				<aui:validator name="min">1</aui:validator>
				<aui:validator name="max">99999</aui:validator>
			</aui:input>

			<p class="text-3 text-secondary" id="<portlet:namespace />autosaveTimeHiddenDescription"><liferay-ui:message key="set-a-value-in-minutes-between-1-and-99.999" /></p>
		</clay:content-col>
	</clay:content-row>
</clay:sheet-section>