...
public class ListTypeEntryPersistenceImpl
	...
	private FinderPath _finderPathFetchByLTDI_N;
	private FinderPath _finderPathCountByLTDI_N;
	 ...
	 */
	@Override
	public ListTypeEntry findByLTDI_N(long listTypeDefinitionId, String name)
		throws NoSuchListTypeEntryException {

		ListTypeEntry listTypeEntry = fetchByLTDI_N(listTypeDefinitionId, name);
	 ...
	 */
	@Override
	public ListTypeEntry fetchByLTDI_N(long listTypeDefinitionId, String name) {
		return fetchByLTDI_N(listTypeDefinitionId, name, true);
	}
	 ...
	 */
	@Override
	public ListTypeEntry fetchByLTDI_N(
		...
		if (useFinderCache) {
			finderArgs = new Object[] {listTypeDefinitionId, name};
		}
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByLTDI_N, finderArgs);
		}
		...
		if (result instanceof ListTypeEntry) {
			...
			if ((listTypeDefinitionId !=
					listTypeEntry.getListTypeDefinitionId()) ||
				!Objects.equals(name, listTypeEntry.getName())) {
		...
		if (result == null) {
			...
			sb.append(_FINDER_COLUMN_LTDI_N_LISTTYPEDEFINITIONID_2);

			boolean bindName = false;

			if (name.isEmpty()) {
				sb.append(_FINDER_COLUMN_LTDI_N_NAME_3);
			}
			else {
				bindName = true;

				sb.append(_FINDER_COLUMN_LTDI_N_NAME_2);
			}
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByLTDI_N, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"ListTypeEntryPersistenceImpl.fetchByLTDI_N(long, String, boolean) with parameters (" +
	 ...
	 */
	@Override
	public ListTypeEntry removeByLTDI_N(long listTypeDefinitionId, String name)
		throws NoSuchListTypeEntryException {

		ListTypeEntry listTypeEntry = findByLTDI_N(listTypeDefinitionId, name);
	 ...
	 */
	@Override
	public int countByLTDI_N(long listTypeDefinitionId, String name) {
		name = Objects.toString(name, "");

		FinderPath finderPath = _finderPathCountByLTDI_N;

		Object[] finderArgs = new Object[] {listTypeDefinitionId, name};
	...
	private static final String _FINDER_COLUMN_LTDI_N_LISTTYPEDEFINITIONID_2 =
		"listTypeEntry.listTypeDefinitionId = ? AND ";

	private static final String _FINDER_COLUMN_LTDI_N_NAME_2 =
		"listTypeEntry.name = ?";

	private static final String _FINDER_COLUMN_LTDI_N_NAME_3 =
		"(listTypeEntry.name IS NULL OR listTypeEntry.name = '')";
	...
	public void cacheResult(ListTypeEntry listTypeEntry) {
		...
		finderCache.putResult(
			_finderPathFetchByLTDI_N,
			new Object[] {
				listTypeEntry.getListTypeDefinitionId(), listTypeEntry.getName()
			},
			listTypeEntry);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		Object[] args = new Object[] {
			...
			listTypeEntryModelImpl.getName()
		};

		finderCache.putResult(_finderPathCountByLTDI_N, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByLTDI_N, args, listTypeEntryModelImpl);
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByLTDI_N = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByLTDI_N",
			new String[] {Long.class.getName(), String.class.getName()},
			new String[] {"listTypeDefinitionId", "name"}, true);

		_finderPathCountByLTDI_N = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByLTDI_N",
			new String[] {Long.class.getName(), String.class.getName()},
			new String[] {"listTypeDefinitionId", "name"}, false);
	}
