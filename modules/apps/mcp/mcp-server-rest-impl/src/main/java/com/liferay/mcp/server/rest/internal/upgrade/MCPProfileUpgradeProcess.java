...
public class MCPProfileUpgradeProcess extends UpgradeProcess {
	...
	private void _upgradeCompany(long companyId) throws Exception {
		...
		try (SafeCloseable safeCloseable =
			...
			// The backfill must run after the tools object field deletion. A
			// failed batch engine definition update leaves the profileStatus
			// object field committed (the column DDL implicitly commits it)
			// while the object field finder caches keep the previous field
			// list, and an update against that stale list silently drops the
			// profileStatus value. The deletion evicts those caches.
