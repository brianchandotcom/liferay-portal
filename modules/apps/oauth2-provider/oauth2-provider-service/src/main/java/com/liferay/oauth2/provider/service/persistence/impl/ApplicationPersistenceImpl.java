...
import com.liferay.oauth2.provider.exception.NoSuchApplicationException;
import com.liferay.oauth2.provider.model.Application;
import com.liferay.oauth2.provider.model.impl.ApplicationImpl;
import com.liferay.oauth2.provider.model.impl.ApplicationModelImpl;
import com.liferay.oauth2.provider.service.persistence.ApplicationPersistence;
 ...
 * @see com.liferay.oauth2.provider.service.persistence.ApplicationUtil
 * @generated
 */
@ProviderType
public class ApplicationPersistenceImpl extends BasePersistenceImpl<Application>
	...
	public void cacheResult(Application application) {
		entityCache.putResult(ApplicationModelImpl.ENTITY_CACHE_ENABLED,
	...
	public void clearCache(Application application) {
		entityCache.removeResult(ApplicationModelImpl.ENTITY_CACHE_ENABLED,
	...
	public Application create(long id) {
		...
		application.setCompanyId(companyProvider.getCompanyId());
	...
	/**
	 * Returns the application with the primary key or throws a {@link com.liferay.portal.kernel.exception.NoSuchModelException} if it could not be found.
	 *
	 * @param primaryKey the primary key of the application
	...
	public Application fetchByPrimaryKey(Serializable primaryKey) {
		Serializable serializable = entityCache.getResult(ApplicationModelImpl.ENTITY_CACHE_ENABLED,
		...
		Application application = (Application)serializable;
		...
		if (application == null) {
			...
			try {
				...
				if (application != null) {
					...
					entityCache.putResult(ApplicationModelImpl.ENTITY_CACHE_ENABLED,
						ApplicationImpl.class, primaryKey, nullModel);
				}
			}
			catch (Exception e) {
				entityCache.removeResult(ApplicationModelImpl.ENTITY_CACHE_ENABLED,
	...
	@Override
	public Map<Serializable, Application> fetchByPrimaryKeys(
		Set<Serializable> primaryKeys) {
		if (primaryKeys.isEmpty()) {
			return Collections.emptyMap();
		}

		Map<Serializable, Application> map = new HashMap<Serializable, Application>();
		...
		if (primaryKeys.size() == 1) {
			...
			if (application != null) {
				map.put(primaryKey, application);
			}
		...
		for (Serializable primaryKey : primaryKeys) {
			...
			if (serializable != nullModel) {
				if (serializable == null) {
					...
					map.put(primaryKey, (Application)serializable);
				}
			}
		}
		...
		query.append(_SQL_SELECT_APPLICATION_WHERE_PKS_IN);
		...
		try {
			...
			for (Application application : (List<Application>)q.list()) {
				map.put(application.getPrimaryKeyObj(), application);

				cacheResult(application);

				uncachedPrimaryKeys.remove(application.getPrimaryKeyObj());
			}
	...
	public List<Application> findAll(int start, int end,
		...
		if (retrieveFromCache) {
			list = (List<Application>)finderCache.getResult(finderPath,
					finderArgs, this);
		}
	...
	private static final String _SQL_SELECT_APPLICATION_WHERE_PKS_IN = "SELECT application FROM Application application WHERE id_ IN (";
	...
	private static final Log _log = LogFactoryUtil.getLog(ApplicationPersistenceImpl.class);
