...
public class DynamicClientRegistrationResponse {
	...
	public static DynamicClientRegistrationResponse fromOAuth2Application(
		...
		for (ClientProfile clientProfile : ClientProfile.values()) {
			if (clientProfile.id() == oAuth2Application.getClientProfile()) {
				dynamicClientRegistrationResponse.setApplicationType(
					StringUtil.toLowerCase(clientProfile.name()));
			}
		}

		dynamicClientRegistrationResponse.setClientName(
			oAuth2Application.getName());
		dynamicClientRegistrationResponse.setClientId(
			oAuth2Application.getClientId());

		Date createDate = oAuth2Application.getCreateDate();

		dynamicClientRegistrationResponse.setClientIdIssuedAt(
			createDate.getTime() / 1000);

		dynamicClientRegistrationResponse.setClientSecret(
			oAuth2Application.getClientSecret());
		dynamicClientRegistrationResponse.setClientSecretExpiresAt(0);
		dynamicClientRegistrationResponse.setClientUri(
			oAuth2Application.getHomePageURL());
		dynamicClientRegistrationResponse.setGrantTypes(
			Arrays.asList(
				StringUtil.split(
					StringUtil.toLowerCase(
						oAuth2Application.getAllowedGrantTypes()))));
		dynamicClientRegistrationResponse.setPolicyUri(
			oAuth2Application.getPrivacyPolicyURL());
		dynamicClientRegistrationResponse.setRedirectURIs(
		...
		if (allowedGrantTypesList.contains(GrantType.AUTHORIZATION_CODE) ||
			...
			dynamicClientRegistrationResponse.setResponseTypes(
				Collections.singletonList("code"));
		}

		if (oAuth2Application.getClientSecret() != null) {
			dynamicClientRegistrationResponse.setTokenEndpointAuthMethod(
				"client_secret_post");
		}
		else {
			dynamicClientRegistrationResponse.setTokenEndpointAuthMethod(
	...
	public void setClientSecretExpiresAt(long clientSecretExpiresAt) {
		_clientSecretExpiresAt = clientSecretExpiresAt;
	}
