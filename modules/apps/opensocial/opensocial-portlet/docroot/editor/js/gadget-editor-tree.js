AUI.add(
	...
	function(A) {
		...
		var CSS_CONTEXT_MENU =
			'gadget-editor-tree-node-contextmenuicon icon-sort-down icon-large';

		var CSS_CONTEXT_MENU_OPEN =
			'gadget-editor-tree-node-contextmenuicon-open icon-sort-down icon-large';
		...
		var TreeViewEditor = A.Component.create({
			...
			prototype: {
				...
				appendChild: function(node) {
					...
					TreeViewEditor.superclass.appendChild.apply(
						instance,
						arguments
					);
				...
				sortChildren: function() {
					...
					folderChildren.forEach(function(item, index, collection) {
						if (index != 0) {
							instance.insertAfter(item, collection[index - 1]);
						}
					});

					fileEntryChildren.forEach(function(
						item,
						index,
						collection
					) {
		...
		var TreeNodeEditor = A.Component.create({
			...
			prototype: {
				...
				bindUI: function() {
					...
					instance.after(
						'fileEntryLoadedChange',
						instance._afterFileEntryLoadedChange
					);
					instance.after(
						'gadgetIdChange',
						instance._afterGadgetIdChange
					);
					instance.after('labelChange', instance._afterLabelChange);
					instance.after(
						'permissionsChange',
						instance._afterPermissionsChange
					);
				...
				appendChild: function(node) {
					...
					TreeNodeEditor.superclass.appendChild.apply(
						instance,
						arguments
					);
				...
				_renderContextMenu: function() {
					...
					contextMenu
						.get(CONTENT_BOX)
						.addClass('gadget-editor-contextmenu-toolbar');
					...
					var treeActionOverlayManager = ownerTree.get(
						'treeActionOverlayManager'
					);
					...
					var contextMenuOverlay = new A.OverlayContext({
						after: {
							visibleChange: function(event) {
								contextMenuIcon.toggleClass(
									CSS_CONTEXT_MENU_OPEN,
									event.target.get(VISIBLE)
								);
							}
						},
						...
						on: {
							show: function(event) {
								...
								var activeEditable = ownerTree.get(
									ACTIVE_EDITABLE
								);
						...
						on: {
							startEditing: function(event) {
								...
								if (editableY >= region.bottom) {
									ownerTreeBoundingBox.set(
										'scrollTop',
										editableY
									);
								}

								ownerTreeBoundingBox.setStyle(
									OVERFLOW,
									'hidden'
								);
							},
				...
				_renderFileEntryLoaded: function() {
					...
					if (instance.isLeaf()) {
						...
						instance._closeContextMenuButton.set(
							DISABLED,
							!fileEntryLoaded
						);
						...
						instance
							.get(CONTENT_BOX)
							.toggleClass('tree-node-loaded', fileEntryLoaded);
					}
				},
				...
				_updatePublishButtons: function() {
					...
					if (extension === 'xml') {
						if (instance.get(GADGET_ID) > 0) {
							var unpublishPermission = instance.get(PERMISSIONS)
								.unpublishPermission;

							unpublishMenuButton.set(
								DISABLED,
								!unpublishPermission
							);
		...
		var EditableEditor = A.Component.create({
			...
			prototype: {
				...
				_defStartEditingFn: function(event) {
					...
					EditableEditor.superclass._defStartEditingFn.apply(
						this,
						arguments
					);
				}
