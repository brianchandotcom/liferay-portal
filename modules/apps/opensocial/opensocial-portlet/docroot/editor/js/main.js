		...
		var Editor = A.Component.create({
			...
			prototype: {
				...
				renderUI: function() {
					...
					instance._gadgetEditorHeight = container
						.one('#gadgetEditorContent')
						.height();
				...
				bindUI: function() {
					...
					instance.after(
						'tree-view-editor:append',
						instance._afterTreeViewAppend
					);

					instance.after(
						'tree-node-editor:addFolderNode',
						instance._afterTreeNodeAddNewFolder
					);
					instance.after(
						'tree-node-editor:append',
						instance._afterTreeNodeAppend
					);
					instance.after(
						'tree-node-editor:closeFileEntry',
						instance._afterTreeNodeCloseFileEntry
					);
					instance.after(
						'tree-node-editor:deleteEntry',
						instance._afterTreeNodeDeleteEntry
					);
					instance.after(
						'tree-node-editor:entryIdChange',
						instance._afterTreeNodeEntryIdChange
					);
					instance.after(
						'tree-node-editor:publish',
						instance._afterTreeNodePublish
					);
					instance.after(
						'tree-node-editor:renameEntry',
						instance._afterTreeNodeRenameEntry
					);
					instance.after(
						'tree-node-editor:select',
						instance._afterTreeNodeSelect
					);
					instance.after(
						'tree-node-editor:showURL',
						instance._afterTreeNodeShowURL
					);
					instance.after(
						'tree-node-editor:unpublish',
						instance._afterTreeNodeUnpublish
					);

					instance.after(
						'editable-editor:cancel',
						instance._afterEditableCancel
					);

					instance.on(
						'editable-editor:save',
						instance._onEditableSave
					);

					instance.after(
						'tab-view-editor:selectionChange',
						instance._afterSelectionChange
					);

					instance.after('tab-editor:add', instance._afterTabAdd);
					instance.after('tab-editor:close', instance._afterTabClose);
					instance.after(
						'tab-editor:entryIdChange',
						instance._afterTabEntryIdChange
					);
					instance.after(
						'tab-editor:isDirtyChange',
						instance._afterTabIsDirtyChange
					);
					instance.after(
						'tab-editor:isNewChange',
						instance._afterTabIsNewChange
					);

					instance.on(
						'tab-editor:onEditorChange',
						instance._onTabEditorChange
					);
					instance.on(
						'tab-editor:onEditorFocus',
						instance._onTabEditorFocus
					);

					Liferay.after(
						'publishGadgetSuccess',
						instance._afterPublishGadgetSuccess,
						instance
					);
				},
				...
				_addNewEntryToTreeNode: function(isLeaf, parentFolderId) {
					...
					var parentFolderNode = instance._treeViewEditor.getNodeById(
						parentFolderId
					);
					...
					if (rootFolder || parentFolderNode.get('loaded')) {
						...
						A.setTimeout(function() {
							var entryLabel = instance._getNextNewEntryName(
								isLeaf,
								parentFolderNode
							);

							instance._treeViewEditor.addNewNodeToFolder(
								entryLabel,
								isLeaf,
								parentFolderId
							);
						...
						parentFolderNode.after(LOADED_CHANGE, function() {
							instance._addNewEntryToTreeNode(
								isLeaf,
								parentFolderId
							);
						});
				...
				_afterEditableCancel: function(event) {
					...
					instance.fire(EVENT_CANCEL_RENAME_ENTRY, {
				...
				_afterSelectionChange: function(event) {
					...
					if (activeTab.get(IS_NEW)) {
						lastSelected = instance._treeViewEditor.get(
							LAST_SELECTED
						);
						...
						while (
							parentNode &&
							A.instanceOf(parentNode, A.TreeNode)
						) {
							parentNode.expand();
					...
					instance._saveButton.set(
						DISABLED,
						!activeTab.get(IS_DIRTY)
					);
				...
				_afterTabClose: function(event) {
					...
					instance.fire(EVENT_CLOSE_FILE_ENTRY, {
				...
				_afterTreeNodeAddNewFolder: function(event) {
					...
					instance.fire(EVENT_ADD_FOLDER_NODE, {
				...
				_afterTreeNodeDeleteEntry: function(event) {
					...
					instance.fire(EVENT_DELETE_ENTRY, {
						entryId: event.entryId
					});
				},
				...
				_afterTreeNodePublish: function(event) {
					...
					if (tab && tab.get(IS_DIRTY)) {
						instance._showErrorDialog(
							Liferay.Language.get(
								'save-the-gadget-before-publishing'
							)
						);
					} else {
						var node = instance._getNodeFromDataSet(entryId);
						...
						var uri = instance
							.get('editorGadgetURL')
							.replace(
								'editorGadgetURLPlaceholder',
								node.get(FILE_ENTRY_URL)
							);
				...
				_afterTreeNodeRenameEntry: function(event) {
					...
					instance.fire(EVENT_START_RENAME_ENTRY, {
						entryId: event.entryId
					});
				},
				...
				_afterTreeNodeSelect: function(event) {
					...
					if (target.isLeaf()) {
						instance.fire(EVENT_LOAD_CONTENT, {
				...
				_afterTreeNodeShowURL: function(event) {
					...
					var bodyContent = Lang.sub(TPL_URL_DISPLAY, [
						node.get(LABEL),
						node.get(FILE_ENTRY_URL)
					]);
				...
				_afterTreeNodeUnpublish: function(event) {
					...
					var message = Lang.sub(
						Liferay.Language.get(
							'are-you-sure-you-want-to-unpublish-the-gadget'
						),
						[node.get(LABEL)]
					);

					instance._showConfirmationDialog(
						message,
						instance._unpublishGadget,
						node,
						gadgetId
					);
				},
				...
				_changeEditorFontSize: function(action) {
					...
					var aceEditor = instance._tabViewEditor
						.getSelectedTab()
						.get(ACE_EDITOR)
						.getEditor();
				...
				_createEvents: function() {
					...
					instance.publish(EVENT_ADD_FILE_ENTRY_NODE, {
						defaultFn: instance._defAddFileEntryNodeFn
					});

					instance.publish(EVENT_ADD_FOLDER_NODE, {
						defaultFn: instance._defAddFolderNodeFn
					});

					instance.publish(EVENT_CANCEL_RENAME_ENTRY, {
					...
					instance.publish(EVENT_DELETE_ENTRY, {
						defaultFn: instance._defDeleteEntryFn
					});

					instance.publish(EVENT_LOAD_CONTENT, {
						defaultFn: instance._defLoadContentFn
					});

					instance.publish(EVENT_RENDER_GADGET, {
						defaultFn: instance._defRenderGadgetFn
					});

					instance.publish(EVENT_SAVE_CONTENT, {
						defaultFn: instance._defSaveContentFn
					});

					instance.publish(EVENT_SAVE_RENAME_ENTRY, {
				...
				_defCloseFileEntryFn: function(event) {
					...
					if (tab && tab.get(IS_DIRTY) && !event.noConfirm) {
						...
						var message = Lang.sub(
							Liferay.Language.get(
								'has-not-been-saved-are-you-sure-you-want-to-close-the-tab'
							),
							[tabFileName]
						);
				...
				_defDeleteEntryFn: function(event) {
					...
					if (node.get(GADGET_ID) > 0) {
						message = Lang.sub(
							Liferay.Language.get(
								'are-you-sure-you-want-to-unpublish-and-delete'
							),
							[node.get(LABEL)]
						);
					} else {
				...
				_defLoadContentFn: function(event) {
					...
					if (node.get(FILE_ENTRY_LOADED)) {
						if (
							event.entryId !=
							instance._tabViewEditor
								.getSelectedTab()
								.get(ENTRY_ID)
						) {
							var tabId = instance
								._getTabFromDataSet(event.entryId)
								.get(ID);
						...
						var callback = function(data) {
							...
							instance._tabViewEditor.addExistingDocument(
								event.entryId,
								node.get(LABEL),
								data.content
							);
						...
						instance._requestGetFileEntryContent(
							node.get(ENTRY_ID),
							callback
						);
					}
				...
				_defRenderGadgetFn: function(event) {
					...
					if (tab.get(IS_DIRTY) || tab.get(IS_NEW)) {
						var message = Liferay.Language.get(
							'save-the-gadget-before-previewing'
						);
					...
					instance._requestGetRenderParameters(
						fileEntryURL,
						callback
					);
				},
				...
				_defSaveContentFn: function(event) {
					...
					if (tab.get(IS_NEW) && tab.get(IS_DIRTY)) {
						...
						instance._requestUpdateFileEntryContent(
							event.entryId,
							tab
								.get(ACE_EDITOR)
								.getEditor()
				...
				_defStartRenameEntryFn: function(event) {
					...
					var editable = instance
						._getNodeFromDataSet(event.entryId)
						.get(EDITABLE);
				...
				_deleteEntry: function(node, entryId) {
					...
					if (node.isLeaf()) {
						...
						instance._requestDeleteFileEntry(
							entryId,
							deleteEntryCallback
						);
					} else {
						instance._requestDeleteFolder(
							entryId,
							deleteEntryCallback
						);
					}
				...
				_getIORequest: function(resourceId, callback) {
							...
							instance._getResourceURL(resourceId),
							{
								...
								on: {
									failure: function(event) {
										...
										var message = Lang.sub(
											Liferay.Language.get(
												'request-for-resource-id-failed'
											),
											[resourceId]
										);
					...
					io.once('success', function(event) {
						instance._loadingMask.hide();
				...
				_getNextNewEntryName: function(leafNode, parentNode) {
					...
					do {
						duplicateLabel = children.some(function(item, index) {
							if (
								item.isLeaf() == leafNode &&
								item.get(LABEL).toLowerCase() ==
									label.toLowerCase()
							) {
								i++;
				...
				_onEditableSave: function(event) {
					...
					instance.fire(EVENT_SAVE_RENAME_ENTRY, {
						entryId: event.target.get(ENTRY_ID)
					});
				},
				...
				_onTabEditorFocus: function(event) {
					...
					instance._treeViewEditor
						.get('treeActionOverlayManager')
						.hideAll();

					var activeEditable = instance._treeViewEditor.get(
						'activeEditable'
					);
				...
				_renderToolbar: function() {
					...
					var newFolderButton = new A.Button({
						...
						on: {
							click: function(event) {
								...
								if (parentFolderId == 0) {
									parentFolderId = instance._treeViewEditor
										.getChildren(false)[0]
										.get(ID);
								}
					...
					var saveButton = new A.Button({
						...
						on: {
							click: function(event) {
								var entryId = instance._tabViewEditor
									.getSelectedTab()
									.get(ENTRY_ID);

								instance.fire(EVENT_SAVE_CONTENT, {
									entryId: entryId
								});
							}
						}
					...
					var previewButton = new A.Button({
						...
						on: {
							click: function(event) {
								...
								instance.fire(EVENT_RENDER_GADGET, {
									entryId: entryId
								});
							}
						}
				...
				_renderTreeViewEditor: function() {
					...
					var treeViewEditor = new A.TreeViewEditor({
						...
						publishGadgetPermission: instance.get(
							'publishGadgetPermission'
						)
					}).render();

					var node = treeViewEditor.addRootNode(
						'OpenSocial Gadgets',
						instance.get(ROOT_FOLDER_ID)
					);
				...
				_requestUpdateFileEntryContent: function(
					fileEntryId,
					content,
					callback
				) {
				...
				_requestUpdateFileEntryTitle: function(
					fileEntryId,
					title,
					callback
				) {
				...
				_saveRenameExistingEntry: function(node) {
					...
					if (node.isLeaf()) {
						instance._requestUpdateFileEntryTitle(
							entryId,
							label,
							callback
						);
					} else {
						instance._requestUpdateFolderName(
							entryId,
							label,
							callback
						);
				...
				_saveRenameNewEntry: function(node) {
					...
					if (
						parentFolderNode.get(ID) ==
						instance._treeViewEditor.get(ID)
					) {
					...
					var callback = function(data) {
						...
						if (dataError) {
							...
							var lastSelected = instance._treeViewEditor.get(
								LAST_SELECTED
							);
				...
				_showConfirmationDialog: function(message, callback) {
					...
					var confirmationDialog = instance._createDialog(
				...
				_showSearchDialog: function() {
					...
					instance._searchDialog = instance._createDialog(
						'Search',
						form.get(BOUNDING_BOX),
						options
					);
				},
