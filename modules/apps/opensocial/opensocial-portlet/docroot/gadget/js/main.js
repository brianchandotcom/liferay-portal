...
AUI.add(
	...
	function(A) {
		...
		var TPL_IFRAME =
			'<iframe class="' +
			CSS_CLASS_GADGET +
			'" frameborder="no" {height} id="{iframeId}" name="{iframeId}" scrolling="{scrolling}" src="{src}" {width}></iframe>';
			...
			prototype: {
				...
				renderUI: function() {
					...
					if (instance.get('requiresPubsub')) {
						...
						var container = gadgets.pubsub2router.hub.getContainer(
							iframeId
						);

						if (container) {
							delete gadgets.pubsub2router.hub._containers[
								iframeId
							];
						}
						...
						container = new OpenAjax.hub.IframeContainer(
							...
							iframeId,
							{
								...
								IframeContainer: {
									...
									timeout: instance.get(
										'pubsubURILoadTimeout'
									),
									tunnelURI: shindig
										.uri(
											instance.get('serverBase') +
												instance.get('rpcRelay')
										)
										.resolve(
											shindig.uri(window.location.href)
										),
						...
						var iframe = Lang.sub(TPL_IFRAME, {
							height: height
								? 'height="' + height + '"'
								: STR_EMPTY,
							iframeId: iframeId,
				...
				bindUI: function() {
					...
					instance.after({
						additionalParamsChange:
							instance._afterAdditionalParamsChange,
				...
				_afterAdditionalParamsChange: function(event) {
					...
					for (i in newAdditionalParams) {
						src = instance._setSrcParameter(
							encodeURIComponent(i),
							encodeURIComponent(newAdditionalParams[i]),
							src
						);
					}
				...
				_afterViewParamsChange: function(event) {
					...
					if (parentUrl) {
						instance._refreshSrcParameter(
							'view-params',
							encodeURIComponent(JSON.stringify(viewParams))
						);
					}
				},
				...
				_getIframeUrl: function(value) {
					...
					url =
						instance.get('serverBase') +
						'ifr?' +
						A.QueryString.stringify(urlData);
					...
					for (var i in userPrefs) {
						buffer.push(
							'&up_' +
								encodeURIComponent(i) +
								'=' +
								encodeURIComponent(userPrefs[i])
						);
					}
		...
		var CookieStore = A.Component.create({
			...
			prototype: {
				savePrefs: function(gadget) {
					...
					return A.Cookie.setSubs(
						instance.get('userPrefsKey'),
						gadget.get('userPrefs')
					);
				}
			}
		});
			...
			if (isArray(topic)) {
				...
				for (var i = 0; i < topic.length; i++) {
					...
					if (containsString(eventName, PREFIX)) {
						...
						subscriptionId.push(
							subscribeGadgetEvent(eventName, fn)
						);
					}
				}
		...
		var subscribeGadgetEvent = function(topic, fn) {
			...
			var subscriptionId = gadgets.pubsub2router.hub.subscribe(
				eventType,
				fn
			);
		...
		gadgets.rpc.register('requestNavigateTo', function(view, viewParams) {
			var gadget = Gadget.get(this.f);

			var portletURL = new Liferay.PortletURL.createURL(
				gadget.get('baseRenderURL')
			);
