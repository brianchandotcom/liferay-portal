...
public class PortalCORSServletFilter extends BaseFilter {
	...
	private CORSSupport _getCORSSupport(String urlPath, long companyId) {
		Map<String, CORSSupport> exactPatternMatcher =
			_portalCORSConfigurationManager.getExactPatternMatcher(companyId);

		if (exactPatternMatcher == null) {
			exactPatternMatcher =
				_portalCORSConfigurationManager.getExactPatternMatcher(
					CompanyConstants.SYSTEM);
		}

		CORSSupport corsSupport = exactPatternMatcher.get(urlPath);
		...
		Map<String, CORSSupport> wildcardPatternMatcher =
			_portalCORSConfigurationManager.getWildcardPatternMatcher(
				companyId);
		...
		for (int i = urlPath.length(); i > 0; --i) {
			corsSupport = wildcardPatternMatcher.get(
			...
			if ((lastDot > 0) && (urlPath.charAt(i - 1) == '.')) {
				lastDot = i - 1;
			}
		}

		Map<String, CORSSupport> extensionPatternMatcher =
			_portalCORSConfigurationManager.getExtensionPatternMatcher(
				companyId);

		if (extensionPatternMatcher == null) {
			extensionPatternMatcher =
				_portalCORSConfigurationManager.getExtensionPatternMatcher(
					CompanyConstants.SYSTEM);
		}

		return extensionPatternMatcher.get("*" + urlPath.substring(lastDot));
	}
