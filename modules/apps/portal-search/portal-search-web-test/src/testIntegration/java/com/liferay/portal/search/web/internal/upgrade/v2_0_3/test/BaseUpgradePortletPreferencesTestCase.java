...
public abstract class BaseUpgradePortletPreferencesTestCase {
	...
	public void setUp() throws Exception {
		...
		String[] portletKeys = getPortletIds();

		portletIds = new String[portletKeys.length];

		for (int i = 0; i < portletKeys.length; i++) {
			portletIds[i] = LayoutTestUtil.addPortletToLayout(
				layout, portletKeys[i]);
		}
	}
	...
	protected void assertPortletPreferences(
		Map<String, String> expectedMap, long ownerId, int ownerType, long plid,
		String portletId) {
	...
	protected void testUpgrade(
		...
		for (String portletId : portletIds) {
			updateLayoutPortletPreference(portletId, map);
		}

		runUpgrade();

		for (String portletId : portletIds) {
			assertPortletPreferences(expectedMap, portletId);
		}
	}
	...
	protected void testUpgrade(
		...
		for (String portletId : portletIds) {
			...
			assertPortletPreferences(
				map, group.getGroupId(), ownerType, plid, portletId);
		}
		...
		for (String portletId : portletIds) {
			assertPortletPreferences(
				expectedMap, group.getGroupId(), ownerType, plid, portletId);
		}
	}
	...
	protected void updateLayoutPortletPreference(
		...
		assertPortletPreferences(portletPreferencesMap, portletId);
	}
