...
public class CategoryFacetConfigurationDisplayContext {
	...
	public JSONArray getGroupsJSONArray() {
		try {
			...
			Map<Long, List<AssetVocabulary>> vocabulariesMap =
				_getVocabulariesMap(groups);
	...
	private Map<Long, List<AssetVocabulary>> _getVocabulariesMap(
		List<Group> groups) {

		Map<Long, List<AssetVocabulary>> vocabulariesMap = new HashMap<>();
		...
		for (AssetVocabulary vocabulary : vocabularies) {
			vocabulariesMap.computeIfAbsent(
				vocabulary.getGroupId(), key -> new ArrayList<>()
			).add(
				vocabulary
			);
		}
	...
	private JSONObject _toJSONObject(
			Group group, List<AssetVocabulary> vocabularies)
		throws PortalException {
		...
		for (AssetVocabulary vocabulary : vocabularies) {
			childrenJSONArray.put(
				_jsonFactory.createJSONObject(
					...
					"name", vocabulary.getTitle(_locale)
				));
		}
