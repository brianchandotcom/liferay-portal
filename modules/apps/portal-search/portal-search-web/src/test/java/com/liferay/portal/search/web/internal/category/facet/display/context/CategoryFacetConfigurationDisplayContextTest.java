...
public class CategoryFacetConfigurationDisplayContextTest {
	...
	public void testGetGroupsJSONArrayNestsVocabulariesUnderTheirOwningGroup()
		...
		AssetVocabulary depotVocabulary = _mockVocabulary(depotGroup);
		AssetVocabulary siteVocabulary = _mockVocabulary(siteGroup);

		Mockito.when(
			_assetVocabularyService.getGroupVocabularies(
				new long[] {siteGroup.getGroupId(), depotGroup.getGroupId()})
		).thenReturn(
			Arrays.asList(siteVocabulary, depotVocabulary)
		);
		...
		_assertVocabularyChild(
			jsonArray.getJSONObject(0), siteGroup, siteVocabulary);
		_assertVocabularyChild(
			jsonArray.getJSONObject(1), depotGroup, depotVocabulary);
	}
	...
	private void _assertVocabularyChild(
			JSONObject groupJSONObject, Group group, AssetVocabulary vocabulary)
		throws Exception {
		...
		JSONObject vocabularyJSONObject = childrenJSONArray.getJSONObject(0);

		Assert.assertEquals(
			group.getExternalReferenceCode() + "&&" +
				vocabulary.getExternalReferenceCode(),
			vocabularyJSONObject.getString("externalReferenceCode"));
		Assert.assertEquals(
			vocabulary.getVocabularyId(), vocabularyJSONObject.getLong("id"));
		Assert.assertEquals(
			vocabulary.getTitle(LocaleUtil.US),
			vocabularyJSONObject.getString("name"));
	}
	...
	private AssetVocabulary _mockVocabulary(Group group) {
