...
public class ContentSecurityPolicyFilter extends BasePortalFilter {
	...
	public boolean isFilterEnabled(
		...
		if (!contentSecurityPolicyConfiguration.enabled() ||
			...
			_isPageEditorRequest(httpServletRequest)) {
	...
	private boolean _isPageEditorRequest(
		HttpServletRequest httpServletRequest) {

		// The page editor renders under the layout friendly URL with
		// "p_l_mode=edit", which is not an excluded path, so CSP would be
		// enforced on this admin surface. It embeds CKEditor and AUI
		// ParseContent, which use eval and inline styles. Require an
		// authenticated user so an anonymous request cannot disable the
		// policy on a public page. Layout permission cannot be checked
		// here because this filter runs before the layout is resolved.
