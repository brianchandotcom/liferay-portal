...
<div class="portlet-configuration-edit-templates">
	...
	<aui:form action="<%= deleteArchivedSetupsURL %>" name="fm">
		<div class="portlet-configuration-body-content">
			<clay:management-toolbar displayContext="<%= portletConfigurationTemplatesManagementToolbarDisplayContext %>" />
			...
			<div class="container-fluid-1280">
				...
				<liferay-ui:search-container id="archivedSettings" searchContainer="<%= portletConfigurationTemplatesDisplayContext.getArchivedSettingsSearchContainer() %>">
					...
					>
						<c:choose>
							<c:when test='<%= Objects.equals(portletConfigurationTemplatesDisplayContext.getDisplayStyle(), "descriptive") %>'>
								<liferay-ui:search-container-column-icon icon="archive" />

								<liferay-ui:search-container-column-text colspan="<%= 2 %>">
									<h6 class="text-default">
										<liferay-ui:message
											arguments="<%= new String[] {LanguageUtil.getTimeDescription(locale, System.currentTimeMillis() - archivedSettings.getModifiedDate().getTime(), true), HtmlUtil.escape(archivedSettings.getUserName())} %>"
											key="x-ago-by-x"
								...
								<liferay-ui:search-container-column-text>
									<clay:dropdown-actions defaultEventHandler="<%= PortletConfigurationWebKeys.ARCHIVED_SETUPS_DROPDOWN_DEFAULT_EVENT_HANDLER %>" dropdownItems="<%= portletConfigurationTemplatesDisplayContext.getActionDropdownItems(archivedSettings) %>" />
								</liferay-ui:search-container-column-text>
							</c:when>
							<c:when test='<%= Objects.equals(portletConfigurationTemplatesDisplayContext.getDisplayStyle(), "icon") %>'>
								...
								<liferay-ui:search-container-column-text>
									<clay:vertical-card verticalCard="<%= new ArchivedSettingsVerticalCard(archivedSettings, renderRequest, renderResponse) %>" />
								</liferay-ui:search-container-column-text>
							</c:when>
							<c:when test='<%= Objects.equals(portletConfigurationTemplatesDisplayContext.getDisplayStyle(), "list") %>'>
								...
								<liferay-ui:search-container-column-text name="user-name" truncate="<%= true %>">
					...
					<liferay-ui:search-iterator displayStyle="<%= portletConfigurationTemplatesDisplayContext.getDisplayStyle() %>" markupView="lexicon" />
