(function() {
			...
			var MeetingScheduler = A.Component.create({
				...
				prototype: {
					initializer: function(config) {
						...
						instance._containerNode = instance.byId(
							config.containerId
						);
						...
						instance._endDatePicker = instance._getComponent(
							config.endDatePickerName + 'DatePicker'
						);
						instance._endTimePicker = instance._getComponent(
							config.endTimePickerName + 'TimePicker'
						);
						instance._startDatePicker = instance._getComponent(
							config.startDatePickerName + 'DatePicker'
						);
						instance._startTimePicker = instance._getComponent(
							config.startTimePickerName + 'TimePicker'
						);
					...
					_initPicker: function(picker) {
						...
						var inputNode = A.one(
							attrs.container._node.children[0]
						);
					...
					_setEndDatePickerDate: function() {
						...
						instance._endDatePicker.selectDates([
							instance._endDate
						]);
					},
					...
					_setEndTimePickerTime: function() {
						...
						instance._endTimePicker.selectDates([
							instance._endDate
						]);
					},
					...
					_setStartDatePickerDate: function() {
						...
						instance._startDatePicker.selectDates([
							instance._startDate
						]);
					},
					...
					_setStartTimePickerTime: function() {
						...
						instance._startTimePicker.selectDates([
							instance._startDate
						]);
					},
					...
					_validate: function() {
						...
						if (meetingEventDate) {
							...
							var helpInline = meetingEventDate.one(
								'.help-inline'
							);
	...
	AUI.add(
		...
		function(A) {
			...
			var TPL_PARTICIPANT_LIST_ITEM =
				'<div class="participant-invited" id="{namespace}powwowParticipantInvited{participantNumber}">' +
			...
			var TPL_ROW_FIELDS_INPUTS =
				'<input id="{namespace}powwowParticipantName{participantNumber}" name="{namespace}powwowParticipantNames" type="hidden" value="{name}" />' +
			...
			var MeetingUtil = A.Component.create({
				...
				prototype: {
					initializer: function(config) {
						...
						instance._creatorEmailAddress =
							config.creatorEmailAddress;
						instance._creatorUserId = config.creatorUserId;
						instance._formName = formName;
						instance._meetingId = config.meetingId;
						instance._participantDataField = instance.byId(
							config.participantDataField
						);
						instance._participantInvitedSelector =
							config.participantInvitedSelector;
						instance._participantKeywords = participantKeywords;
						instance._participantListContainer = instance.byId(
							config.participantList
						);
						instance._participantNameInput = instance.byId(
							config.participantName
						);
						instance._portletKey = config.portletKey;
						instance._prefixParticipantEmailAddress =
							config.prefixParticipantEmailAddress;
						instance._prefixParticipantInvited =
							config.prefixParticipantInvited;
						instance._prefixParticipantName =
							config.prefixParticipantName;
						instance._prefixParticipantParticipantUserId =
							config.prefixParticipantParticipantUserId;
						instance._prefixParticipantType =
							config.prefixParticipantType;
						...
						instance._participantKeywordsInput = instance.byId(
							participantKeywords
						);
						...
						if (instance._meetingId <= 0) {
							instance.addParticipantToList(
								instance._creatorName,
								instance._creatorUserId,
								instance._creatorEmailAddress,
								'user',
								false,
							...
							instance._currentHostEmailAddress =
								instance._creatorEmailAddress;
							instance._currentHostParticipantUserId =
								instance._creatorUserId;
						}
					},

					addParticipantToList: function(
						name,
						participantUserId,
						emailAddress,
						cssClass,
						...
						var participantListContainer =
							instance._participantListContainer;

						var participants = instance.get(STR_PARTICIPANTS);

						var participantIndex = instance._findParticipant(
							participantUserId,
							emailAddress
						);
						...
						var displayName = instance._getParticipantDisplayName(
							participant.name,
						...
						if (participantListContainer) {
							participantListContainer.append(
								Lang.sub(TPL_PARTICIPANT_LIST_ITEM, data)
							);

							var makeHostSelector = instance.one(
								STR_HASH +
									namespace +
									instance._prefixParticipantInvited +
									participant.participantNumber +
									' .make-host-participant'
							);

							if (
								cssClass == STR_GUEST ||
								instance._meetingId > 0
							) {
								makeHostSelector.hide();
							}

							var participantMenu = instance.byId(
								STR_PARTICIPANT_MENU +
									participant.participantNumber
							);
						...
						var rowFields = instance.one(
							instance._rowFieldsSelector
						);

						if (rowFields) {
							rowFields.append(
								Lang.sub(TPL_ROW_FIELDS_INPUTS, data)
							);
						}
					},
					...
					editParticipantFromList: function(participantData) {
						...
						if (typeof type !== 'undefined') {
							...
							var powwowParticipantType = instance.byId(
								instance._prefixParticipantType +
									participantNumber
							);
							...
							var powwowParticipantInvitedName = instance.one(
								STR_HASH +
									instance.NS +
									instance._prefixParticipantInvited +
									participantNumber +
							...
							if (powwowParticipantInvitedName) {
								...
								powwowParticipantInvitedName.set(
									'text',
									displayName
								);
							}
					...
					removeParticipantFromList: function(participantNumber) {
						...
						var powwowParticipantInvited = instance.byId(
							instance._prefixParticipantInvited +
								participantNumber
						);
						...
						var powwowParticipantName = instance.byId(
							instance._prefixParticipantName + participantNumber
						);
						...
						var powwowParticipantParticipantUserId = instance.byId(
							instance._prefixParticipantParticipantUserId +
								participantNumber
						);
						...
						var powwowParticipantEmailAddress = instance.byId(
							instance._prefixParticipantEmailAddress +
								participantNumber
						);
						...
						var powwowParticipantType = instance.byId(
							instance._prefixParticipantType + participantNumber
						);
					...
					_addParticipantByEmail: function(event) {
						...
						if (instance._enableEmailInput) {
							...
							instance._validateParticipantEmail.validateField(
								instance.NS + instance._participantKeywords
							);

							var participantKeywordsInput =
								instance._participantKeywordsInput;
							var participantNameInput =
								instance._participantNameInput;

							if (
								participantKeywordsInput &&
								participantNameInput &&
								!instance._validateParticipantEmail.hasErrors()
							) {
								instance.addParticipantToList(
									participantNameInput.val(),
									0,
									participantKeywordsInput.val(),
									STR_GUEST,
									false,
									PARTICIPANT_TYPE_ATTENDEE
								);
					...
					_bindAutoComplete: function() {
						...
						var inputNode =
							STR_HASH +
							instance.NS +
							instance._participantKeywords;
						var resultFilters = [
							STR_PHRASE_MATCH,
							instance._getAutoCompleteParticipantsResultFilter
						];

						var autoCompleteConfiguration = instance._getAutoCompleteConfiguration(
							inputNode,
							resultFilters,
							instance._getAutoCompleteParticipantsResultHandler
						);

						var participantsAutoCompleteList = new A.AutoCompleteList(
							autoCompleteConfiguration
						);

						participantsAutoCompleteList.after(STR_SELECT, function(
							itemNode,
							result
						) {
							if (participantKeywordsInput) {
								...
								var validateParticipantEmail =
									instance._validateParticipantEmail;

								if (validateParticipantEmail.hasErrors()) {
									validateParticipantEmail.resetField(
										participantKeywordsInput
									);
								}
							}
						...
						participantsAutoCompleteList.on(STR_SELECT, function(
							itemNode,
							result
						) {
							instance.addParticipantToList(
								itemNode.result.raw.fullName,
								itemNode.result.raw.userId,
								itemNode.result.raw.emailAddress,
								'user',
								false,
								PARTICIPANT_TYPE_ATTENDEE
							);
					...
					_bindUI: function() {
						...
						instance._validateParticipantEmail = new A.FormValidator(
							{
								boundingBox:
									STR_HASH + namespace + instance._formName,
								rules: rules
							}
						);
						...
						if (participantKeywordsInput) {
							...
							participantKeywordsInput.on('keyup', function(
								event
							) {
								if (
									participantKeywordsInput.val() ==
									...
									instance._validateParticipantEmail.resetField(
										participantKeywordsInput
									);
								}
							});
						...
						Liferay.provide(
							...
							function(participantNumber) {
								instance.removeParticipantFromList(
									participantNumber
								);
							},
							['aui-base']
						...
						Liferay.provide(
							...
							function(participantNumber) {
								...
								participantData.participantNumber = instance._findParticipant(
									instance._currentHostParticipantUserId,
									instance._currentHostEmailAddress
								);

								instance.editParticipantFromList(
									participantData
								);
					...
					_getAutoCompleteConfiguration: function(
						inputNode,
						resultFilters,
						resultHandler
					) {
						...
						configuration.resultFormatter =
							instance._getAutoCompleteResultFormatter;
						configuration.resultHighlighter = STR_PHRASE_MATCH;
						configuration.resultTextLocator =
							instance._getAutoCompleteResultTextLocator;

						configuration.source = function(query, callback) {
							instance._getAutoCompleteSource(
								query,
								callback,
								instance,
								resultHandler
					...
					_getAutoCompleteParticipantsResultFilter: function(
						query,
						results
					) {
						var instance = this;
						...
						for (var i = 0; i < results.length; i++) {
							if (
								!A.one(
									'.participant-email-address[value="' +
										results[i].raw.emailAddress +
										'"]'
								)
							) {
					...
					_getAutoCompleteParticipantsResultHandler: function(
						responseData,
						instance
					) {
						var participantNameInput =
						...
						if (participantNameInput) {
							participantNameInput.toggle(
								instance._participantKeywordsInput.val() !=
									STR_BLANK && noResults
							);
						}
					},
					...
					_getAutoCompleteSource: function(
						query,
						callback,
						instance,
						resultHandler
					...
					_getParticipantDisplayName: function(
						name,
						participantUserId,
						emailAddress,
						type
						...
						if (type == PARTICIPANT_TYPE_HOST) {
							displayName = Lang.sub(
								Liferay.Language.get('x-host'),
								[displayName]
							);
					...
					_sendMeetingForm: function() {
						...
						var participantDataField =
							instance._participantDataField;

						participantDataField.val(
							JSON.stringify(instance.get(STR_PARTICIPANTS))
						);
