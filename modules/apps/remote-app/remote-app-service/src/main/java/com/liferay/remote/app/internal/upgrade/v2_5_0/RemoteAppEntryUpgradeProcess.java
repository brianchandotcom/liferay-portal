...
public class RemoteAppEntryUpgradeProcess extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		_addBundledAppTypeColumn();

		_addBundledAppURLColumn();

		_addCustomElementUseESMColumn();
	}

	private void _addBundledAppTypeColumn() throws Exception {
		if (!hasColumn(RemoteAppEntryTable.TABLE_NAME, "bundledAppType")) {
			alter(
				RemoteAppEntryTable.class,
				new AlterTableAddColumn("bundledAppType", "VARCHAR(75)"));
		}
	}

	private void _addBundledAppURLColumn() throws Exception {
		if (!hasColumn(RemoteAppEntryTable.TABLE_NAME, "bundledAppURL")) {
			alter(
				RemoteAppEntryTable.class,
				new AlterTableAddColumn("bundledAppURL", "STRING"));
		}
	}
