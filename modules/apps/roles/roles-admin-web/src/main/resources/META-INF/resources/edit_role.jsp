...
<c:if test="<%= role != null %>">
	<c:choose>
		<c:when test="<%= currentRoleType.getType() == RoleConstants.TYPE_REGULAR %>">
...
<portlet:renderURL var="editRoleRenderURL">
	...
	<portlet:param name="type" value="<%= String.valueOf(currentRoleType.getType()) %>" />
</portlet:renderURL>
	...
	<aui:fieldset-group markupView="lexicon">
		<aui:fieldset>
			<c:choose>
				<c:when test="<%= role == null %>">
					<aui:select name="type">
						...
						<%
						List<RoleType> roleTypes = RoleTypeRetrieverUtil.getRoleTypes(request);

						for (RoleType roleType : roleTypes) {
						%>

							<aui:option label="<%= roleType.getName() %>" value="<%= roleType.getType() %>" />
				...
				<c:otherwise>
					<aui:input label="type" name="typeLabel" type="resource" value="<%= LanguageUtil.get(request, currentRoleType.getName()) %>" />

					<c:if test="<%= role == null %>">
						<aui:input name="type" type="hidden" value="<%= String.valueOf(currentRoleType.getType()) %>" />
					</c:if>
				</c:otherwise>
			</c:choose>
			...
			<c:if test="<%= role != null %>">
				...
				<%
				String[] subtypes = currentRoleType.getSubtypes();
				%>
