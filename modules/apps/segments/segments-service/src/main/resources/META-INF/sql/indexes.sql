...
create unique index IX_262BE0F5 on SegmentsEntry (groupId, segmentsEntryKey[$COLUMN_LENGTH:75$], companyId);
...
create unique index IX_17D4961A on SegmentsEntry (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);
...
create unique index IX_52EB6263 on SegmentsEntryRel (segmentsEntryId, classNameId, classPK, companyId);

create index IX_65648B53 on SegmentsEntryRole (roleId);
create unique index IX_C1703C14 on SegmentsEntryRole (segmentsEntryId, roleId, companyId);

create index IX_50DD7CFA on SegmentsExperience (groupId, classNameId, classPK, active_);
create unique index IX_7B264601 on SegmentsExperience (groupId, classNameId, classPK, priority, companyId);
create index IX_5B21AD71 on SegmentsExperience (groupId, segmentsEntryId, classNameId, classPK, active_);
create unique index IX_3F927A5D on SegmentsExperience (groupId, segmentsExperienceKey[$COLUMN_LENGTH:75$], companyId);
...
create unique index IX_92950462 on SegmentsExperience (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);
...
create unique index IX_4C4A873D on SegmentsExperiment (groupId, segmentsExperimentKey[$COLUMN_LENGTH:75$], companyId);
...
create unique index IX_A7BF4EAF on SegmentsExperiment (uuid_[$COLUMN_LENGTH:75$], groupId, companyId);

create unique index IX_7A46E741 on SegmentsExperimentRel (segmentsExperimentId, segmentsExperienceId, companyId);