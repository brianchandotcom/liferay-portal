...
public class SiteAdminDisplayContext {
	...
	public String getOpenGraphImageURL() {
		LayoutSEOSiteEntry layoutSEOSiteEntry = _getLayoutSEOSiteEntry();

		if ((layoutSEOSiteEntry == null) ||
			(layoutSEOSiteEntry.getOpenGraphImageFileEntryId() == 0)) {
	...
	public boolean isOpenGraphEnabled() {
		...
		if (layoutSEOSiteEntry != null) {
			return layoutSEOSiteEntry.isOpenGraphEnabled();
		}
	...
	private LayoutSEOSiteEntry _getLayoutSEOSiteEntry() {
		return LayoutSEOSiteEntryLocalServiceUtil.
			fetchLayoutSEOSiteEntryByGroupId(_getGroupId());
	}
