								...
								<c:if test="<%= group != null %>">
									<c:choose>
										<c:when test="<%= group.getPublicLayoutsPageCount() > 0 %>">
											<aui:a label="open-public-pages" href="<%= group.getDisplayURL(themeDisplay, false) %>" target="_blank" />
										</c:when>
										<c:otherwise>
								...
								<c:if test="<%= group != null %>">
									<c:choose>
										<c:when test="<%= group.getPrivateLayoutsPageCount() > 0 %>">
											<aui:a label="open-private-pages" href="<%= group.getDisplayURL(themeDisplay, true) %>" target="_blank" />
										</c:when>
										<c:otherwise>
