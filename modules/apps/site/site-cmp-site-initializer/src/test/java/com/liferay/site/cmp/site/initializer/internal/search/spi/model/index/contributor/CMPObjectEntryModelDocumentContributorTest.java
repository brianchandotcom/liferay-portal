...
public class CMPObjectEntryModelDocumentContributorTest {
	...
	public void testContributeWithObjectEntryFolder() {
		ObjectEntryFolder objectEntryFolderA = Mockito.mock(
			ObjectEntryFolder.class);

		long objectEntryFolderAId = RandomTestUtil.randomLong();

		Mockito.when(
			objectEntryFolderA.getTreePath()
		).thenReturn(
			StringBundler.concat(
				StringPool.SLASH, objectEntryFolderAId, StringPool.SLASH)
		);

		Mockito.when(
			_objectEntryFolderLocalService.fetchObjectEntryFolder(
				objectEntryFolderAId)
		).thenReturn(
			objectEntryFolderA
		);
		...
		ObjectEntryFolder objectEntryFolderBBB = Mockito.mock(
			ObjectEntryFolder.class);

		long objectEntryFolderBId = RandomTestUtil.randomLong();
		long objectEntryFolderBBId = RandomTestUtil.randomLong();
		long objectEntryFolderBBBId = RandomTestUtil.randomLong();

		Mockito.when(
			objectEntryFolderBBB.getTreePath()
		).thenReturn(
			StringBundler.concat(
				StringPool.SLASH, objectEntryFolderBId, StringPool.SLASH,
				objectEntryFolderBBId, StringPool.SLASH, objectEntryFolderBBBId,
				StringPool.SLASH)
		);

		Mockito.when(
			_objectEntryFolderLocalService.fetchObjectEntryFolder(
				objectEntryFolderBBBId)
		).thenReturn(
			objectEntryFolderBBB
		);
