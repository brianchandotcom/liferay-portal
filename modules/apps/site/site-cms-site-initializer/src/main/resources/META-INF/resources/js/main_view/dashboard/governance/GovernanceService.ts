...
async function getAssetStatistics(
	...
	const [statistics, similarAssetGroups] = await Promise.all([
		ApiHelper.get<AssetStatistics>(
			`/o/headless-cms/v1.0/asset-statistics?${scope}`,
			signal
		),
		ApiHelper.get<{totalCount: number}>(
			`/o/headless-cms/v1.0/similar-asset-groups?${scope}pageSize=1`,
			signal
		),
	]);
	...
	return {
		...
		data: {
			...statistics.data,
			duplicatedCount: similarAssetGroups.data?.totalCount,
		},
	};
}
