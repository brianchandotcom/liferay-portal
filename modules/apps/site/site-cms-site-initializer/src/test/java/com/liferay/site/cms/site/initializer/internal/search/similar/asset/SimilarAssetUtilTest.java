...
public class SimilarAssetUtilTest {
	...
	@Test
	public void testBlankTextYieldsNoSimilarAssetBuckets() {
		Assert.assertEquals(
			0, SimilarAssetUtil.getSimilarAssetBuckets(null).length);
		Assert.assertEquals(
			0, SimilarAssetUtil.getSimilarAssetBuckets("").length);
		Assert.assertEquals(
			0, SimilarAssetUtil.getSimilarAssetBuckets("   ").length);
	}
	...
	public void testDeterministic() {
		Assert.assertArrayEquals(
			SimilarAssetUtil.getSimilarAssetBuckets(_A),
			SimilarAssetUtil.getSimilarAssetBuckets(_A));
	}
	...
	public void testDeterministicAcrossDefaultLocales() {
		...
		try {
			...
			String[] similarAssetBuckets =
				SimilarAssetUtil.getSimilarAssetBuckets(_TURKISH);
	...
	@Test
	public void testDistinctTextSharesNoSimilarAssetBuckets() {
		Assert.assertEquals(
			0, _getSharedSimilarAssetBucketCount(_A, _DISTINCT));
	}

	@Test
	public void testNearDuplicateSharesMoreThanDistinct() {
		Assert.assertTrue(
			_getSharedSimilarAssetBucketCount(_A, _A + " 2") >
				_getSharedSimilarAssetBucketCount(_A, _DISTINCT));
	}

	@Test
	public void testNearDuplicateSharesMostSimilarAssetBuckets() {
		Assert.assertTrue(
			_getSharedSimilarAssetBucketCount(_A, _A + " 2") >= 20);
	}

	private int _getSharedSimilarAssetBucketCount(String text1, String text2) {
		Set<String> similarAssetBuckets = new HashSet<>();

		for (String similarAssetBucket :
				SimilarAssetUtil.getSimilarAssetBuckets(text1)) {

			similarAssetBuckets.add(similarAssetBucket);
		}

		int count = 0;

		for (String similarAssetBucket :
				SimilarAssetUtil.getSimilarAssetBuckets(text2)) {

			if (similarAssetBuckets.contains(similarAssetBucket)) {
				count++;
			}
		}
