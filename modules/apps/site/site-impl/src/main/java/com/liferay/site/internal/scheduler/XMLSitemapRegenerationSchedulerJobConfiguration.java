...
public class XMLSitemapRegenerationSchedulerJobConfiguration
	...
	private void _regenerateSitemaps(long companyId) {
		...
		if (xmlSitemapRegenerationEntries.isEmpty()) {
			return;
		}

		Set<String> regeneratedKeys = new HashSet<>();

		for (XMLSitemapRegenerationEntry xmlSitemapRegenerationEntry :
				xmlSitemapRegenerationEntries) {
			...
			try {
				String regeneratedKey = StringBundler.concat(
					...
					StringPool.POUND, xmlSitemapRegenerationEntry.getGroupId());

				if (regeneratedKeys.add(regeneratedKey)) {
					_sitemapManager.regenerateSitemap(
						xmlSitemapRegenerationEntry.getAssetTypeKey(),
						companyId, xmlSitemapRegenerationEntry.getGroupId());
				}
			}
			catch (Exception exception) {
				_log.error(
					"Unable to regenerate the sitemap for XML sitemap " +
						"regeneration entry " + xmlSitemapRegenerationEntry,
					exception);
			}
		}
	}
