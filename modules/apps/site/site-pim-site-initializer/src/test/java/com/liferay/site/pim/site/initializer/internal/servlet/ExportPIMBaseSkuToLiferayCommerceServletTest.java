...
public class ExportPIMBaseSkuToLiferayCommerceServletTest {
	...
	private ObjectEntry _setUpObjectEntry(String code, long groupId)
		throws Exception {
		...
		Mockito.when(
			_objectEntryLocalService.getObjectEntries(
				groupId, _OBJECT_DEFINITION_ID, QueryUtil.ALL_POS,
				QueryUtil.ALL_POS)
		).thenReturn(
	...
	private DepotEntry _setUpSpaceDepotEntries(long... groupIds) {
	...
	private void _testDoGetWithMultipleDepotEntries() throws Exception {
		...
		_setUpObjectEntry("SKU-1", _GROUP_ID);
		_setUpObjectEntry("SKU-2", _OTHER_GROUP_ID);
	...
	private void _testDoGetWithPortalException() throws Exception {
		...
		ObjectEntry objectEntry = _setUpObjectEntry("SKU-1", _GROUP_ID);
