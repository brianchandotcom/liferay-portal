...
<c:if test="<%= sitemapCompanyConfigurationDisplayContext.cachedGenerationEnabled() %>">
	<p class="c-mb-0 small text-secondary"><liferay-ui:message key="last-updated" />: <%= sitemapCompanyConfigurationDisplayContext.getLastRegenerateSitemapDate() %></p>

	<p class="c-mb-3 small text-secondary"><liferay-ui:message key="next-scheduled" />: <%= sitemapCompanyConfigurationDisplayContext.isRegenerateSitemapInProgress() ? LanguageUtil.get(request, "generating-now") : sitemapCompanyConfigurationDisplayContext.getNextRegenerateSitemapDate() %></p>
</c:if>
...
<c:if test="<%= sitemapCompanyConfigurationDisplayContext.isGenerationModeSectionVisible() %>">
	...
	>
		...
		<clay:content-row
			cssClass='<%= cachedGenerationEnabled ? "c-pl-4" : "hide" %>'
		...
		>
			...
			<clay:content-col
				cssClass='<%= sitemapCompanyConfigurationDisplayContext.isRegenerateSitemapWeekly() ? "c-pr-2" : "hide" %>'
				id='<%= liferayPortletResponse.getNamespace() + "xmlSitemapRegenerationDayField" %>'
			>
				<clay:select
					id='<%= liferayPortletResponse.getNamespace() + "xmlSitemapRegenerationDayOfWeek" %>'
					label='<%= LanguageUtil.get(request, "day-of-week") %>'
					name="xmlSitemapRegenerationDayOfWeek"
					options="<%= sitemapCompanyConfigurationDisplayContext.getXMLSitemapRegenerationDayOfWeekSelectOptions() %>"
				/>
			</clay:content-col>

			<clay:content-col
				cssClass='<%= sitemapCompanyConfigurationDisplayContext.isRegenerateSitemapHourly() ? "hide" : "c-pr-2" %>'
				id='<%= liferayPortletResponse.getNamespace() + "xmlSitemapRegenerationTimeField" %>'
			>
				<div class="form-group">
					<label class="control-label"><liferay-ui:message key="time" /></label>

					<liferay-ui:input-time
						amPmParam="xmlSitemapRegenerationTimeAmPm"
						amPmValue="<%= sitemapCompanyConfigurationDisplayContext.getXMLSitemapRegenerationTimeAmPm() %>"
						cssClass="form-group form-group-inline range-options"
						hourParam="xmlSitemapRegenerationTimeHour"
						hourValue="<%= sitemapCompanyConfigurationDisplayContext.getXMLSitemapRegenerationTimeHour() %>"
						minuteParam="xmlSitemapRegenerationTimeMinute"
						minuteValue="<%= sitemapCompanyConfigurationDisplayContext.getXMLSitemapRegenerationTimeMinute() %>"
					/>
				</div>
			</clay:content-col>

			<clay:content-col
				cssClass='<%= sitemapCompanyConfigurationDisplayContext.isRegenerateSitemapHourly() ? "hide" : "" %>'
				expand="<%= true %>"
