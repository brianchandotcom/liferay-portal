...
public class StaticSiteExporterImpl implements StaticSiteExporter {
	...
	public StaticSiteExport export(long groupId, Set<Locale> locales)
		...
		try (AutoCloseable autoCloseable =
			...
			List<StaticSiteExportLayout> staticSiteExportLayouts =
				_exportLayouts(groupId, locales);
	...
	private List<StaticSiteExportLayout> _exportLayouts(
	...
	private List<StaticSiteExportResource> _fetchResources(
		...
		for (String url : urls) {
			...
			try {
				content = staticSiteExportResourceFetcher.fetch(url);
			}
			catch (Exception exception) {
			...
			staticSiteExportResources.add(
				new StaticSiteExportResourceImpl(content, url));
		}
