...
public class K8sAgentImpl implements K8sAgent {
	...
	public K8sAgentImpl(
		...
		errorMessages.put(401, "Unauthorized! ".concat(_errorMessage));
		errorMessages.put(403, "Forbidden!".concat(_errorMessage));
	...
	public void deleteConfigMap(String name) {
		...
		if (configMap != null) {
			...
			if (_log.isDebugEnabled()) {
				_log.debug("Deleted ".concat(name));
			}
		}
	}
	...
	private static final String _errorMessage =
		"Configured service account doesn't have access. Service account may " +
			"have been revoked.";
