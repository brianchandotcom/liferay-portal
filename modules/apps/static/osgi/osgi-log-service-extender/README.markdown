...
### Gogo Shell Logger Commands

In addition, Gogo shell commands are provided to interact with log levels in real time. The available commands are in the `logger` command scope (following the syntax `<scope>:<command>`).
...
* `logger:levels <context>` - List the levels associated with a given logger context.
...
* `logger:level <context> <name> <level>` - Set the level of a named logger in a given logger context.
