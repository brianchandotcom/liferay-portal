...
public class StyleBookEntryZipProcessorImpl
		...
		try {
			if (styleBookEntry == null) {
				styleBookEntry = _styleBookEntryEntryService.addStyleBookEntry(
					null, groupId, frontendTokenDefinition,
	...
	private void _importStyleBookEntries(
		...
		if (Validator.isNotNull(styleBookEntryContent)) {
			...
			String frontendTokenDefinitionPath =
				styleBookEntryJSONObject.getString(
					"frontendTokenDefinitionPath");

			if (Validator.isNotNull(frontendTokenDefinitionPath)) {
				frontendTokenDefinition = _getStyleBookEntryContent(
					zipFile, fileName, frontendTokenDefinitionPath);
			}
