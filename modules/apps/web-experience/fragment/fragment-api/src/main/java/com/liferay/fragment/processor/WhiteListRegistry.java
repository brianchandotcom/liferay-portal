 ...
 */
@Component(immediate = true, service = WhiteListRegistry.class)
public class WhiteListRegistry {
	...
	protected void setWhiteListEntry(WhiteListEntry whiteListEntry) {
		_whiteListEntries.put(
			whiteListEntry.getTagName(), whiteListEntry.getAttributes());
	}

	protected void unsetWhiteListEntry(WhiteListEntry whiteListEntry) {
		_whiteListEntries.remove(whiteListEntry.getTagName());
	}
