...
public class WebHookEntryClayDataSetActionProvider
	...
	public List<DropdownItem> getDropdownItems(
		...
		WebHookClayDataSetEntry webHookClayDataSetEntry =
			(WebHookClayDataSetEntry)model;

		return DropdownItemListBuilder.add(
			dropdownItem -> _buildEditWebHookEntryAction(
				dropdownItem, httpServletRequest, webHookClayDataSetEntry)
		).add(
			dropdownItem -> _buildDeleteWebHookEntryAction(
				dropdownItem, httpServletRequest, webHookClayDataSetEntry)
		).build();
	}

	private void _buildDeleteWebHookEntryAction(
		...
		dropdownItem.setHref(
			PortletURLBuilder.create(
				...
				"/web_hook_admin/delete_web_hook_entry"
			).setParameter(
				"webHookEntryId", webHookClayDataSetEntry.getWebHookEntryId()
			).buildString());
	...
	private void _buildEditWebHookEntryAction(
		DropdownItem dropdownItem, HttpServletRequest httpServletRequest,
		WebHookClayDataSetEntry webHookClayDataSetEntry) {

		PortletURL editWebHookEntryURL = PortletURLBuilder.create(
		...
		editWebHookEntryURL.setParameter("redirect", currentURL);

		dropdownItem.setHref(editWebHookEntryURL);
