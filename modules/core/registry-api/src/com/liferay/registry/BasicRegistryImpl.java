...
public class BasicRegistryImpl implements Registry {
	...
	public <T> T getService(ServiceReference<T> serviceReference) {
		...
		for (Entry<ServiceReference<?>, Object> entry : getServices().entrySet()) {
	...
	public <T> Collection<ServiceReference<T>> getServiceReferences(
		Class<T> clazz, String filterString) throws Exception {
	...
	public <T> ServiceReference<T>[] getServiceReferences(
		String className, String filterString) throws Exception {
	...
	public class BasicServiceRegistration<S>
	...
	public class BasicServiceReference<T> implements ServiceReference<T> {
		...
		public int compareTo(Object serviceReference) {
			BasicServiceReference<?> other =
				(BasicServiceReference<?>)serviceReference;

			final int thisRanking = (Integer)_properties.get("service.ranking");
			final int otherRanking = (Integer)other._properties.get(
				"service.ranking");
			...
			final long thisId = (Long)_properties.get("service.id");
			final long otherId = (Long)other._properties.get("service.id");
		...
		public String[] getPropertyKeys() {
			return _properties.navigableKeySet().toArray(new String[0]);
		}
