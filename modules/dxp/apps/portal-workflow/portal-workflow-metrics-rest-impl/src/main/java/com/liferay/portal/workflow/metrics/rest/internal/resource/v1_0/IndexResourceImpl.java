...
public class IndexResourceImpl extends BaseIndexResourceImpl {
	...
	private static final Map<String, String> _indexNameSuffixMap =
		HashMapBuilder.put(
			"instance", WorkflowMetricsIndexEntityNameConstant.POSTFIX_INSTANCE
		).put(
			"node", WorkflowMetricsIndexEntityNameConstant.POSTFIX_NODE
		).put(
			"process", WorkflowMetricsIndexEntityNameConstant.POSTFIX_PROCESS
		).put(
			"sla-instance-result",
			WorkflowMetricsIndexEntityNameConstant.POSTFIX_SLA_INSTANCE_RESULT
		).put(
			"sla-task-result",
			WorkflowMetricsIndexEntityNameConstant.POSTFIX_SLA_TASK_RESULT
		).put(
			"task", WorkflowMetricsIndexEntityNameConstant.POSTFIX_TASK
		).put(
			"transition",
			WorkflowMetricsIndexEntityNameConstant.POSTFIX_TRANSITION
		).build();
