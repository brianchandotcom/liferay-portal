...
public abstract class BaseCalendarResourceTestCase {
	...
	public void testPostCalendarsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			calendarResource.postCalendarsPageExportBatchHttpResponse(
				null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Calendar[] calendars = getCalendars(exportTask);

		long totalCount = calendars.length;
		...
		calendars = getCalendars(exportTask);

		Assert.assertEquals(totalCount + 2, calendars.length);

		assertContains(calendar1, Arrays.asList(calendars));
		assertContains(calendar2, Arrays.asList(calendars));
	}
	...
	protected Calendar[] getCalendars(ExportTask exportTask) throws Exception {
