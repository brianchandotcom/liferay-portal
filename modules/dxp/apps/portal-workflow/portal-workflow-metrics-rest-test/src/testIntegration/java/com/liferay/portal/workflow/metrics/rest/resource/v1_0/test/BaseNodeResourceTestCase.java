...
public abstract class BaseNodeResourceTestCase {
	...
	@Test
	public void testPostProcessNodesPageExportBatch() throws Exception {
		Long processId = testGetProcessNodesPage_getProcessId();
		Long irrelevantProcessId =
			testGetProcessNodesPage_getIrrelevantProcessId();

		HttpInvoker.HttpResponse httpResponse =
			nodeResource.postProcessNodesPageExportBatchHttpResponse(
				processId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Node[] nodes = getNodes(exportTask);

		long totalCount = nodes.length;
		...
		if (irrelevantProcessId != null) {
			...
			httpResponse =
				nodeResource.postProcessNodesPageExportBatchHttpResponse(
					irrelevantProcessId, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			nodes = getNodes(exportTask);

			Assert.assertEquals(1, nodes.length);

			assertEquals(irrelevantNode, nodes[0]);
		}
		...
		httpResponse = nodeResource.postProcessNodesPageExportBatchHttpResponse(
			processId, null, null, null);
		...
		Assert.assertEquals(totalCount + 2, nodes.length);

		assertContains(node1, Arrays.asList(nodes));
		assertContains(node2, Arrays.asList(nodes));
	}
	...
	protected Node[] getNodes(ExportTask exportTask) throws Exception {
		...
		try {
			return NodeSerDes.toDTOs(zipReader.getEntryAsString("export.json"));
		}
		finally {
			zipReader.close();
		}
