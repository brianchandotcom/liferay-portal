...
public abstract class BaseTaskResourceTestCase {
	...
	@Test
	public void testPostProcessTasksPageExportBatch() throws Exception {
		Long processId = testGetProcessTasksPage_getProcessId();
		Long irrelevantProcessId =
			testGetProcessTasksPage_getIrrelevantProcessId();

		HttpInvoker.HttpResponse httpResponse =
			taskResource.postProcessTasksPageExportBatchHttpResponse(
				processId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Task[] tasks = getTasks(exportTask);

		long totalCount = tasks.length;
		...
		if (irrelevantProcessId != null) {
			...
			tasks = getTasks(exportTask);

			Assert.assertEquals(1, tasks.length);

			assertEquals(irrelevantTask, tasks[0]);
		}
		...
		httpResponse = taskResource.postProcessTasksPageExportBatchHttpResponse(
			processId, null, null, null);
		...
		Assert.assertEquals(totalCount + 2, tasks.length);

		assertContains(task1, Arrays.asList(tasks));
		assertContains(task2, Arrays.asList(tasks));
	}
	...
	protected Task[] getTasks(ExportTask exportTask) throws Exception {
		...
		try {
			return TaskSerDes.toDTOs(zipReader.getEntryAsString("export.json"));
		}
		finally {
			zipReader.close();
		}
