...
public class ElementInstanceUtilTest {
	...
	public void testUnpackCustomSXPElementRecomputesConfigurationEntry()
		...
		ElementInstance elementInstance = ElementInstanceUtil.unpack(
			ElementInstance.unsafeToDTO(
				JSONUtil.put(
					"configurationEntry",
					_createConfigurationJSONObject(_WRONG_VALUE)
				).put(
					"sxpElement", _createSXPElementJSONObject(_WRONG_VALUE)
				).put(
		...
		Assert.assertFalse(
			configurationEntry, configurationEntry.contains(_WRONG_VALUE));
	}
	...
	private static final String _CORRECT_VALUE = "my-test-site&&TESTCATEGORY";

	private static final String _WRONG_VALUE = "WRONG__MATCHES_NOTHING";
