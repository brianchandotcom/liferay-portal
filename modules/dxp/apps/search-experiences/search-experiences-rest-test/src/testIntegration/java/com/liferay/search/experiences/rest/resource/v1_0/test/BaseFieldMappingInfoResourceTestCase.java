...
public abstract class BaseFieldMappingInfoResourceTestCase {
	...
	public void testPostFieldMappingInfosPageExportBatch() throws Exception {
		...
		FieldMappingInfo[] fieldMappingInfos = getFieldMappingInfos(exportTask);

		long totalCount = fieldMappingInfos.length;
		...
		fieldMappingInfos = getFieldMappingInfos(exportTask);

		Assert.assertEquals(totalCount + 2, fieldMappingInfos.length);

		assertContains(fieldMappingInfo1, Arrays.asList(fieldMappingInfos));
		assertContains(fieldMappingInfo2, Arrays.asList(fieldMappingInfos));
	}
	...
	protected FieldMappingInfo[] getFieldMappingInfos(ExportTask exportTask)
		throws Exception {
		...
		try {
			return FieldMappingInfoSerDes.toDTOs(
