...
public abstract class BaseQueryPrefilterContributorResourceTestCase {
	...
	@Test
	public void testPostQueryPrefilterContributorsPageExportBatch()
		throws Exception {
		...
		QueryPrefilterContributor[] queryPrefilterContributors =
			getQueryPrefilterContributors(exportTask);

		long totalCount = queryPrefilterContributors.length;
		...
		queryPrefilterContributors = getQueryPrefilterContributors(exportTask);

		Assert.assertEquals(totalCount + 2, queryPrefilterContributors.length);

		assertContains(
			queryPrefilterContributor1,
			Arrays.asList(queryPrefilterContributors));
		assertContains(
			queryPrefilterContributor2,
			Arrays.asList(queryPrefilterContributors));
	}
	...
	protected QueryPrefilterContributor[] getQueryPrefilterContributors(
		...
		try {
			return QueryPrefilterContributorSerDes.toDTOs(
