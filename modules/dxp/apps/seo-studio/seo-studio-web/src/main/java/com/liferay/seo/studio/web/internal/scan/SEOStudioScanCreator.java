...
public class SEOStudioScanCreator {
	...
	public void createScans(
		...
		if (scheduledScanKey != null) {
			long seoStudioScanRunId = _fetchSEOStudioScanRunId(
				companyId, scheduledScanKey, seoStudioScanRunObjectDefinition);
				...
				() -> {
					...
					ObjectEntry seoStudioScanRunObjectEntry =
						_addSEOStudioScanRunObjectEntry(
							accountEntryId, scheduledScanKey,
							seoStudioDomainObjectEntry,
							seoStudioScanRunObjectDefinition, serviceContext,
							triggeredBy, userId);
					...
					for (String engineKey : enabledEngineKeys) {
						_addSEOStudioScanObjectEntry(
							...
							engineKey, seoStudioScanObjectDefinition,
							seoStudioScanRunId, serviceContext, userId);
					}
	...
	private ObjectEntry _addSEOStudioScanObjectEntry(
			...
			ObjectDefinition seoStudioScanObjectDefinition,
			long seoStudioScanRunId, ServiceContext serviceContext, long userId)
		throws Exception {

		return _objectEntryLocalService.addObjectEntry(
			0, userId, seoStudioScanObjectDefinition.getObjectDefinitionId(),
	...
	private ObjectEntry _addSEOStudioScanRunObjectEntry(
			long accountEntryId, String scheduledScanKey,
			ObjectEntry seoStudioDomainObjectEntry,
			ObjectDefinition seoStudioScanRunObjectDefinition,
	...
	private long _fetchSEOStudioScanRunId(
		...
		if (ListUtil.isEmpty(seoStudioScanRunIds)) {
			return 0;
		}

		return seoStudioScanRunIds.get(0);
	}
