...
describe('generateCandidates', () => {
	...
	it('throws with the raw response text when the response is not JSON', async () => {
		...
		await expect(
			generateCandidates(TITLE_AUTO_FIX_DEFINITION, 'content')
	...
	it('invokes the title generator agent with the page content', async () => {
		...
		await generateCandidates(TITLE_AUTO_FIX_DEFINITION, 'my page content');
...
describe('applyFix', () => {
	...
	it('posts the insight type, value, and page URL to the SEO Studio Auto Fix backend', async () => {
		...
		expect(url).toBe('/o/seo-studio-auto-fix/auto-fix');
		expect(options.method).toBe('POST');
		expect(options.body.get('insightType')).toBe(
			TITLE_AUTO_FIX_DEFINITION.insightTypeName
		);
		expect(options.body.get('pageURL')).toBe(
			'http://example.com/web/customer/products'
		);
		expect(options.body.get('value')).toBe('New Title');
	});
