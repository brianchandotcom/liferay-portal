...
public class LDAPUserImporterImpl implements LDAPUserImporter, UserImporter {
	...
	public void importUsers(long companyId) throws Exception {
		...
		try {
			...
			LockLocalServiceUtil.lock(
				...
				_ldapImportConfiguration.ldapImportLockExpirationTime());
	...
	public void importUsers(long ldapServerId, long companyId)
		...
		try {
			...
			String importMethod = PrefsPropsUtil.getString(
				...
				_ldapImportConfiguration.ldapImportMethod());
	...
	protected void activate(Map<String, Object> properties) {
		_ldapExportConfiguration = Configurable.createConfigurable(
			LDAPExportConfiguration.class, properties);

		_ldapImportConfiguration = Configurable.createConfigurable(
			LDAPImportConfiguration.class, properties);

		_ldapUserIgnoreAttributes = SetUtil.fromArray(
			_ldapImportConfiguration.ldapUserIgnoreAttributes());
	}
	...
	protected void addRole(
		...
		if (!_ldapImportConfiguration.ldapImportCreateRolePerGroup()) {
			return;
		}
	...
	protected User addUser(long companyId, LDAPUser ldapUser, String password)
		...
		if (!_ldapImportConfiguration.ldapImportUserPasswordEnabled()) {
			autoPassword =
				_ldapImportConfiguration.
					ldapImportUserPasswordAutogenerated() &&
					 !PropsValues.AUTH_PIPELINE_ENABLE_LIFERAY_CHECK;

			if (!autoPassword) {
				String defaultPassword =
					_ldapImportConfiguration.ldapImportUserPasswordDefault();
	...
	protected Set<Long> importGroup(
		...
		if (_ldapImportConfiguration.ldapImportGroupCacheEnabled()) {
	...
	protected void importGroups(
		...
		if (Validator.isNotNull(groupMappingsUser) &&
			PrefsPropsUtil.getBoolean(
				...
				_ldapImportConfiguration.
					ldapImportGroupSearchFilterEnabled())) {
	...
	protected User updateUser(
		...
		if (PrefsPropsUtil.getBoolean(
				...
				_ldapExportConfiguration.ldapExportEnabled())) {
		...
		if (!_ldapImportConfiguration.ldapImportUserPasswordEnabled()) {
			password = _ldapImportConfiguration.ldapImportUserPasswordDefault();
	...
	private volatile LDAPExportConfiguration _ldapExportConfiguration;
