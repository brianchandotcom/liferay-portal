 ...
 */
public class CustomElementConfigurer extends BaseClientExtensionConfigurer {

	public static final String BUILD_CUSTOM_ELEMENT_TASK_NAME = "buildCustomElement";

	@Override
	public void apply(Project project, ClientExtension clientExtension) {
		super.apply(project, clientExtension);

		Copy customElementTask = _addTaskBuildCustomElement(project, clientExtension);
		...
		jarTaskProvider.configure(
			new Action<Jar>() {
				...
				public void execute(Jar jar) {
					jar.dependsOn(customElementTask);
				}
		...
		project.afterEvaluate(
			new Action<Project>() {
				...
				public void execute(Project project) {
					...
					try {
						_customElementBundleInstructions(
	...
	private Copy _addTaskBuildCustomElement(
		Project project, ClientExtension clientExtension) {

		Copy copy = GradleUtil.addTask(
			project, BUILD_CUSTOM_ELEMENT_TASK_NAME + clientExtension.id.toUpperCase(),
			Copy.class);

		copy.setDescription("Assembles custom element.");
	...
	private void _customElementBundleInstructions(
		...
		bundleExtension.instruction(
			"-includeresource.css." + clientExtension.id,
			"META-INF/resources/=build/;filter:=*.css;recursive:=false");
		bundleExtension.instruction(
			"-includeresource.js." + clientExtension.id,
			"META-INF/resources/=build/;filter:=*.js;recursive:=false");
