...
public class GitHubJobMessageUtil {
	...
	public static void getGitHubJobMessage(Project project) throws Exception {
		...
		if (jobResults.size() > 0) {
			...
			if (firstJobResult._result.equals("ABORTED")) {
				sb.append(firstJobResult._getMessage());
			}
			else {
				sb.append(
					generateHeaderText(
				...
				for (JobResult jobResult : jobResults) {
					...
					sb.append(jobResult._getMessage());
				}
		...
		if (sb.length() == 0) {
			...
			if (javacOutputFile.exists()) {
				...
				sb.append(
					JenkinsResultsParserUtil.fixJSON(javacOutputFileContent));
				sb.append("</pre>");
			}
		}
	...
	private static class JobResult {
		...
		private JobResult(JSONObject jsonObject, Project project, String url) {
		...
		private JobResult(Project project, String url) throws Exception {
			this(
