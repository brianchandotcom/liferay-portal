...
test(
	...
	async ({page, segmentsPage}) => {
		...
		await test.step('When a segment designer adds a segment', async () => {
			...
			await segmentsPage.addSegmentField('Browser', 'Session', segmentName);
...
test(
	...
	async ({page, segmentsPage}) => {
		...
		await test.step('When a segment designer adds a segment', async () => {
			...
			await segmentsPage.addSegmentField('Language', 'Session', segmentName);
...
test(
	...
	async ({page, pageEditorPage, segmentsPage}) => {
		...
		await test.step('When edits the segment with Region criterion', async () => {
			...
			await segmentsPage.addSegmentField('Region', 'Organization', segmentName2);
