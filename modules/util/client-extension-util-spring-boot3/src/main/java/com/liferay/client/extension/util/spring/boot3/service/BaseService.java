...
public abstract class BaseService {
	...
	protected Disposable asyncDelete(
		...
		return _getWebClient(
			...
			_getHttpHeaders(httpHeadersMap)
		).bodyValue(
			body
		).exchangeToMono(
	...
	private Consumer<HttpHeaders> _getHttpHeaders(
		Map<String, String> httpHeadersMap) {

		if (httpHeadersMap == null) {
			return httpHeaders -> httpHeaders.setAll(new HashMap<>());
		}

		return httpHeaders -> httpHeaders.setAll(httpHeadersMap);
	}
