...
public class EagerBlobEntityPersistenceTest {
	...
	public void testUpdateExisting() throws Exception {
		...
		newEagerBlobEntity.setCompanyId(RandomTestUtil.nextLong());

		newEagerBlobEntity.setUserId(RandomTestUtil.nextLong());

		newEagerBlobEntity.setUserName(RandomTestUtil.randomString());

		newEagerBlobEntity.setCreateDate(RandomTestUtil.nextDate());

		newEagerBlobEntity.setModifiedDate(RandomTestUtil.nextDate());

		newEagerBlobEntity.setField1(RandomTestUtil.randomString());

		newEagerBlobEntity.setField2(RandomTestUtil.randomBoolean());

		newEagerBlobEntity.setField3(RandomTestUtil.nextInt());

		newEagerBlobEntity.setField4(RandomTestUtil.nextDate());

		newEagerBlobEntity.setField5(RandomTestUtil.randomString());
		String newFieldBlobString = RandomTestUtil.randomString();

		byte[] newFieldBlobBytes = newFieldBlobString.getBytes("UTF-8");

		Blob newFieldBlobBlob = new OutputBlob(
			new ByteArrayInputStream(newFieldBlobBytes),
			newFieldBlobBytes.length);

		newEagerBlobEntity.setFieldBlob(newFieldBlobBlob);
		...
		Assert.assertEquals(
			existingEagerBlobEntity.getCompanyId(),
			newEagerBlobEntity.getCompanyId());
		Assert.assertEquals(
			existingEagerBlobEntity.getUserId(),
			newEagerBlobEntity.getUserId());
		Assert.assertEquals(
			existingEagerBlobEntity.getUserName(),
			newEagerBlobEntity.getUserName());
		Assert.assertEquals(
			Time.getShortTimestamp(existingEagerBlobEntity.getCreateDate()),
			Time.getShortTimestamp(newEagerBlobEntity.getCreateDate()));
		Assert.assertEquals(
			Time.getShortTimestamp(existingEagerBlobEntity.getModifiedDate()),
			Time.getShortTimestamp(newEagerBlobEntity.getModifiedDate()));
		Assert.assertEquals(
			existingEagerBlobEntity.getField1(),
			newEagerBlobEntity.getField1());
		Assert.assertEquals(
			existingEagerBlobEntity.isField2(), newEagerBlobEntity.isField2());
		Assert.assertEquals(
			existingEagerBlobEntity.getField3(),
			newEagerBlobEntity.getField3());
		Assert.assertEquals(
			Time.getShortTimestamp(existingEagerBlobEntity.getField4()),
			Time.getShortTimestamp(newEagerBlobEntity.getField4()));
		Assert.assertEquals(
			existingEagerBlobEntity.getField5(),
			newEagerBlobEntity.getField5());
		Blob existingFieldBlob = existingEagerBlobEntity.getFieldBlob();

		Assert.assertTrue(
			Arrays.equals(
				existingFieldBlob.getBytes(1, (int)existingFieldBlob.length()),
				newFieldBlobBytes));
	}
	...
	protected EagerBlobEntity addEagerBlobEntity() throws Exception {
		...
		eagerBlobEntity.setCompanyId(RandomTestUtil.nextLong());

		eagerBlobEntity.setUserId(RandomTestUtil.nextLong());

		eagerBlobEntity.setUserName(RandomTestUtil.randomString());

		eagerBlobEntity.setCreateDate(RandomTestUtil.nextDate());

		eagerBlobEntity.setModifiedDate(RandomTestUtil.nextDate());

		eagerBlobEntity.setField1(RandomTestUtil.randomString());

		eagerBlobEntity.setField2(RandomTestUtil.randomBoolean());

		eagerBlobEntity.setField3(RandomTestUtil.nextInt());

		eagerBlobEntity.setField4(RandomTestUtil.nextDate());

		eagerBlobEntity.setField5(RandomTestUtil.randomString());
		String fieldBlobString = RandomTestUtil.randomString();

		byte[] fieldBlobBytes = fieldBlobString.getBytes("UTF-8");

		Blob fieldBlobBlob = new OutputBlob(
			new ByteArrayInputStream(fieldBlobBytes), fieldBlobBytes.length);

		eagerBlobEntity.setFieldBlob(fieldBlobBlob);
