...
public class UpgradeTableBuilder {
	...
	private String _getContent(
		...
		lines.add(_getCopyright());
		lines.add("\n");
		lines.add("package " + packagePath + ";");
		lines.add("\n");
		lines.add("import com.liferay.portal.kernel.upgrade.UpgradeProcess;");
		lines.add("\n");
		lines.add("/**");
		lines.add(" * @author " + author);
		lines.add(" * @generated by buildUpgradeTable command");
		lines.add(" */");
		lines.add("public class " + className + " {");
		lines.add("\n");
		lines.add("public static UpgradeProcess create() {");
		lines.add("return new UpgradeProcess() {");
		lines.add("\n");
		lines.add("@Override");
		lines.add("protected void doUpgrade() throws Exception {");
		lines.add("if (!hasTable(TABLE_NAME)) {");
		lines.add("runSQL(TABLE_SQL_CREATE);");
		lines.add("}");
		lines.add("}");
		lines.add("};");
		lines.add("}");
		lines.add("\n");
		lines.add("private static final String TABLE_NAME =" + tableName + ";");
		lines.add("\n");
		lines.add(
		...
		for (String line : lines) {
			...
			for (int i = 0; i < openCurlyBraces; i++) {
				sb.append("\t");
			}
