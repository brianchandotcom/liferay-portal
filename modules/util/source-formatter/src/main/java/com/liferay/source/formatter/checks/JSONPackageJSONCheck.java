...
public class JSONPackageJSONCheck extends BaseFileCheck {
	...
	private synchronized Map<String, String> _getExpectedDependencyVersionsMap(
		...
		List<String> internalLinkedWorkspaceNames = getAttributeValues(
			_INTERNAL_LINKED_WORKSPACE_NAMES_KEY, absolutePath);

		for (String workspaceName : internalLinkedWorkspaceNames) {
			_expectedDependencyVersionsMap.put(workspaceName, "*");
		}

		return _expectedDependencyVersionsMap;
	}

	private static final String _INTERNAL_LINKED_WORKSPACE_NAMES_KEY =
		"internalLinkedWorkspaceNames";
