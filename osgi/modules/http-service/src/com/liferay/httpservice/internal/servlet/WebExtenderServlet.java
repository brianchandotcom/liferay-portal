...
public class WebExtenderServlet extends PortletServlet
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		properties.put(
			SERVICE_PROPERTY_KEY_BEAN_ID, HttpServlet.class.getName());
		...
		properties.put(
			SERVICE_PROPERTY_KEY_BEAN_ID, HttpService.class.getName());
	...
	public void service(
		...
		if (portlet != null) {
			...
			if (requestURI != null) {
				...
				if (requestURITemp.startsWith(Portal.MODULE_MAPPING)) {
					requestURITemp = requestURITemp.substring(
						Portal.MODULE_MAPPING.length());
				}
	...
	private void printDebugHeaderInfo(HttpServletRequest request) {
	...
	protected void service(
		...
		try {
			...
			if (requestURI.endsWith(INVOKER_PATH) &&
			...
			PortalUtil.sendError(
				HttpServletResponse.SC_NOT_FOUND,
				new IllegalArgumentException(
					"No servlet or resource mapped to this path: " + requestURI),
					request, response);
		}
		finally {
