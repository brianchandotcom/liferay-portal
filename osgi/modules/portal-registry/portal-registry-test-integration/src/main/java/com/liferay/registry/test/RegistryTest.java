...
public class RegistryTest {
	...
	public void trackServicesByClass() {
		...
		Assert.assertEquals(
			b, serviceTracker.getService(serviceRegistrationB.getReference()));
		Assert.assertEquals(2, serviceTracker.getServiceReferences().length);
		Assert.assertEquals(2, serviceTracker.getServices().length);

		SortedMap<ServiceReference<InterfaceOne>, InterfaceOne> tracked =
			serviceTracker.getTracked();

		Assert.assertNotNull(tracked);
		Assert.assertEquals(2, tracked.size());
		Assert.assertEquals(a, tracked.get(tracked.firstKey()));
		Assert.assertEquals(b, tracked.get(tracked.lastKey()));
		...
		tracked = serviceTracker.getTracked();

		Assert.assertNotNull(tracked);
		Assert.assertEquals(0, tracked.size());
	...
	public void trackServicesByClassAndCustomizer() {
		...
		Assert.assertEquals(
			referenceB.get(),
			serviceTracker.getService(serviceRegistrationB.getReference()));
		Assert.assertEquals(2, serviceTracker.getServiceReferences().length);
		Assert.assertEquals(2, serviceTracker.getServices().length);

		SortedMap<ServiceReference<InterfaceOne>, TrackedOne> tracked =
			serviceTracker.getTracked();

		Assert.assertNotNull(tracked);
		Assert.assertEquals(2, tracked.size());
		Assert.assertEquals(referenceA.get(), tracked.get(tracked.firstKey()));
		Assert.assertEquals(referenceB.get(), tracked.get(tracked.lastKey()));
	...
	public void trackServicesByFilter() throws Exception {
		...
		Assert.assertNotNull(tracked);
		Assert.assertEquals(1, tracked.size());
		Assert.assertEquals(b, tracked.get(tracked.firstKey()));
		Assert.assertEquals(b, tracked.get(tracked.lastKey()));
	...
	public void trackServicesByFilterAndCustomizer() throws Exception {
		...
		Assert.assertNotNull(tracked);
		Assert.assertEquals(1, tracked.size());
		Assert.assertEquals(referenceB.get(), tracked.get(tracked.firstKey()));
		Assert.assertEquals(referenceB.get(), tracked.get(tracked.lastKey()));
