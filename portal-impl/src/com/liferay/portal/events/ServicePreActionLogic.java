 ...
 */
public class ServicePreActionLogic {

	public void servicePre(
			boolean initPermissionChecker, File privateLARFile,
			File publicLARFile, HttpServletRequest httpServletRequest,
		...
		ThemeDisplay themeDisplay = _initThemeDisplay(
			initPermissionChecker, privateLARFile, publicLARFile,
	...
	private void _addDefaultUserPrivateLayouts(File privateLARFile, User user)
		throws PortalException {
	...
	private void _addDefaultUserPublicLayouts(File publicLARFile, User user)
		throws PortalException {
	...
	private ThemeDisplay _initThemeDisplay(
		...
		if (signedIn) {
			_updateUserLayouts(privateLARFile, publicLARFile, user);
		}
		...
		if ((layout != null) &&
			...
			if (layoutGroup.isUser()) {
				...
				_updateUserLayouts(privateLARFile, publicLARFile, layoutUser);
	...
	private void _updateUserLayouts(
			File privateLARFile, File publicLARFile, User user)
		throws Exception {
		...
		if (addDefaultUserPrivateLayouts) {
			...
			if (!hasPrivateLayouts) {
				_addDefaultUserPrivateLayouts(privateLARFile, user);
			}
		}
		...
		if (addDefaultUserPublicLayouts) {
			...
			if (!hasPublicLayouts) {
				_addDefaultUserPublicLayouts(publicLARFile, user);
			}
		}
	...
	private static class LayoutComposite {
		...
		private LayoutComposite(Layout layout, List<Layout> layouts) {
			_layout = layout;
			_layouts = layouts;
		}
