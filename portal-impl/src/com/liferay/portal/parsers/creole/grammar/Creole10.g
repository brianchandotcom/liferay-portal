...
@members{
	...
	protected BaseParentableNode buildAndComposeListNode(BaseParentableNode parent, ItemNode child, boolean ordered) {
		BaseParentableNode listNode = null;
		
		if (ordered) {
			listNode = new OrderedListNode(parent);
		} else {
		 	listNode = new UnorderedListNode(parent);
		}

		child.setParent(listNode);
		listNode.addChildASTNode(child);

		parent.addChildASTNode(listNode);
...
list_elems
	...
	:	om = list_ordelem_markup {++$CountLevel::level;$CountLevel::currentMarkup = $om.text;$CountLevel::groups += $om.text;}  
				elem=list_elem {  
					...
					BaseParentableNode parent = $list::currentParent;

					if (top == null) {
						OrderedListItemNode node = new OrderedListItemNode($CountLevel::level, parent, $elem.items);
						parent.addChildASTNode(node);
						...
						if (parent instanceof UnorderedListItemNode) {
							buildAndComposeListNode(parent, node, true);
						} else if (parent instanceof UnorderedListNode) {
							parent = ((UnorderedListNode)parent).getParent();

							buildAndComposeListNode(parent, node, true);
						} else if (parent instanceof OrderedListNode && top instanceof UnorderedListItemNode) {
							parent = ((OrderedListNode)parent).getParent();
	...
	|	um = list_unordelem_markup {++$CountLevel::level; $CountLevel::currentMarkup = $um.text;$CountLevel::groups += $um.text;}  
				elem=list_elem {
					...
					if (top == null) {
						UnorderedListItemNode node = new UnorderedListItemNode($CountLevel::level, parent, $elem.items);
						parent.addChildASTNode(node);
						...
						if (parent instanceof OrderedListItemNode) {
							buildAndComposeListNode(parent, node, false);
						} else if (parent instanceof OrderedListNode) {
							parent = ((OrderedListNode)parent).getParent();

							buildAndComposeListNode(parent, node, false);
						} else if (parent instanceof UnorderedListNode && top instanceof OrderedListItemNode) {
							parent = ((UnorderedListNode)parent).getParent();
