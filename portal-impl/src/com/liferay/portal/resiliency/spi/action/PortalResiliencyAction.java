...
public class PortalResiliencyAction extends Action {
	...
	public static final String PORTAL_RESILIENCY_ACTION_TAG =
		"PORTAL_RESILIENCY_ACTION_SERVED";
	...
	public ActionForward execute(
		...
		AgentResponse agentResponse = (AgentResponse)request.getAttribute(
			SPIAgent.AGENT_RESPONSE);

		agentRequest.populateSessionAttributes(request);

		PrincipalThreadLocal.setPassword(
			(String)request.getSession().getAttribute(WebKeys.USER_PASSWORD));
		...
		try {
			...
			request.setAttribute(PORTAL_RESILIENCY_ACTION_TAG, Boolean.TRUE);
		}
	...
	private void _doExecute(
		...
		Portlet portlet = (Portlet)request.getAttribute(SPIAgent.PORTLET);
		...
		switch (lifecycle) {
			case Action :
				...
				if (!newTypeSettings.equals(typeSettings)) {
					request.setAttribute(
						SPIAgent.LAYOUT_TYPE_SETTINGS, newTypeSettings);
				}

				request.setAttribute(SPIAgent.ACTION_RESULT, actionResult);
			...
			case Event :
				...
				Layout layout = (Layout)request.getAttribute(SPIAgent.LAYOUT);
				Event event = (Event)request.getAttribute(SPIAgent.EVENT);
				...
				request.setAttribute(SPIAgent.EVENT_RESULT, events);
