...
public class UserNotificationDeliveryLocalServiceImpl
	...
	public UserNotificationDelivery addUserNotificationDelivery(
		...
		UserNotificationDelivery delivery =
			userNotificationDeliveryPersistence.create(deliveryId);

		delivery.setCompanyId(user.getCompanyId());
		delivery.setUserId(user.getUserId());
		delivery.setClassNameId(classNameId);
		delivery.setType(type);
		delivery.setEmail(email);
		delivery.setSms(sms);
		delivery.setWebsite(website);

		return userNotificationDeliveryPersistence.update(delivery);
	}
	...
	public UserNotificationDelivery getUserNotificationDelivery(
		...
		UserNotificationDelivery delivery = getUserNotificationDelivery(
			userId, classNameId, type);
		...
		if (delivery == null) {
			...
			delivery = userNotificationDeliveryPersistence.update(delivery);
		}
