...
public class SampleSQLBuilder {
	...
	protected void compressInsertSQL(
			DB db, File dir, String insertSQL,
		...
		if (sb.index() >= _optimizeBufferSize) {
			...
			writeToInsertSQLFile(dir, insertSQLWriters, tableName, sql);
		}
	}
		...
		while ((s = unsyncBufferedReader.readLine()) != null) {
			...
			if (s.length() > 0) {
				if (s.startsWith("insert into ")) {
					compressInsertSQL(
						db, dir, s.substring(12), insertSQLs, insertSQLWriters);
				}
				else {
					otherSQLs.add(s);
				}
	...
	protected void mergeSQL(File mergedSQLFile, File dir, String endSQLFileName)
		throws IOException {
	...
	protected void writeToInsertSQLFile(
			File dir, Map<String, Writer> writers, String tableName, String sql)
		throws IOException {
