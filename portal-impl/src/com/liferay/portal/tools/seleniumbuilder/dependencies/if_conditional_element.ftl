...
<#elseif ifConditionalElement.getName() == "contains">
	...
	(RuntimeVariables.evaluateVariable("${seleniumBuilderFileUtil.escapeJava(string)}", ${context})).contains(
		RuntimeVariables.evaluateVariable("${seleniumBuilderFileUtil.escapeJava(substring)}", ${context}))
<#elseif ifConditionalElement.getName() == "equals">
	...
	(RuntimeVariables.evaluateVariable("${seleniumBuilderFileUtil.escapeJava(arg1)}", ${context})).equals(
		RuntimeVariables.evaluateVariable("${seleniumBuilderFileUtil.escapeJava(arg2)}", ${context}))
<#elseif ifConditionalElement.getName() == "isset">
	<#assign var = ifConditionalElement.attributeValue("var")>

	RuntimeVariables.variableExists(RuntimeVariables.evaluateVariable("${seleniumBuilderFileUtil.escapeJava(var)}", ${context}), ${context})
