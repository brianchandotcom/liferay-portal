...
public class VerifyUUID extends VerifyProcess {
	...
	private static final Log _log = LogFactoryUtil.getLog(VerifyUUID.class);
	...
	private class VerifyUUIDRunnable extends ThrowableAwareRunnable {
		...
		protected void doRun() throws Exception {
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					"Verifying VerifyUUID." +
						_verifiableUUIDModel.getTableName());
			}
			...
			try {
				...
				if (_log.isInfoEnabled()) {
					_log.info(
						"Completed verifying VerifyUUID." +
							_verifiableUUIDModel.getTableName() +" in " +
