...
public class RoleStagedModelDataHandler
	...
	protected void clearPermissions(
			Role importedRole, long companyGroupId,
		...
		for (ResourcePermission groupResourcePermission :
			...
			if ((groupId == companyGroupId) ||
				(groupId == userPersonalSiteGroupId)) {
	...
	protected void doImportStagedModel(
		...
		Element rolesElement = portletDataContext.getImportDataRootElement();

		long sourceUserPersonalSiteGroupId = Long.parseLong(
			rolesElement.attributeValue("user-personal-site-group-id"));
		...
		if (existingRole == null) {
			...
			clearPermissions(
				importedRole, portletDataContext.getCompanyGroupId(),
				userPersonalSiteGroupId);
		}
		...
		for (Permission permission : permissions) {
			if (ResourceBlockLocalServiceUtil.isSupported(
				...
				importResourceBlock(
					portletDataContext, userPersonalSiteGroupId,
					sourceUserPersonalSiteGroupId, importedRole, permission);
			}
			else {
	...
	protected void importResourceBlock(
			PortletDataContext portletDataContext, long userPersonalSiteGroupId,
			long sourceUserPersonalSiteGroupId, Role importedRole,
		...
		if (scope == ResourceConstants.SCOPE_COMPANY) {
			...
			if (sourceGroupId == sourceUserPersonalSiteGroupId) {
				groupId = userPersonalSiteGroupId;
			}
