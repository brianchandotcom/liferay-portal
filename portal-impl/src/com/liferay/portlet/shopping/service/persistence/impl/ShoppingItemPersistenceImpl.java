...
public class ShoppingItemPersistenceImpl extends BasePersistenceImpl<ShoppingItem>
	...
	public Map<Serializable, ShoppingItem> fetchByPrimaryKeys(
		...
		Map<Serializable, ShoppingItem> results = new HashMap<Serializable, ShoppingItem>();
		...
		if (primaryKeys.size() == 1) {
			...
			if (shoppingItem != null) {
				results.put(primaryKey, shoppingItem);
			}
		...
		try {
			...
			for (ShoppingItem shoppingItem : (List<ShoppingItem>)q.list()) {
				results.put(shoppingItem.getPrimaryKeyObj(), shoppingItem);

				cacheResult(shoppingItem);

				cacheMissPks.remove(shoppingItem.getPrimaryKeyObj());
			}
