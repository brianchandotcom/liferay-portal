...
public class LayoutPrototypeStagedModelDataHandlerTest
	...
	protected StagedModel getStagedModel(String uuid, Group group)
		...
		return LayoutPrototypeLocalServiceUtil.
			fetchLayoutPrototypeByUuidAndCompanyId(uuid, group.getCompanyId());
	}
	...
	protected void validateDeletion(
		...
		Assert.assertNull(Layout.class.getName() + " was not deleted", layout);

		List<StagedModel> layoutFriendlyURLDependentStagedModels =
			dependentStagedModelsMap.get(
				LayoutFriendlyURL.class.getSimpleName());

		Assert.assertEquals(1, layoutFriendlyURLDependentStagedModels.size());
		...
		Assert.assertNull(
			LayoutFriendlyURL.class.getName() + " was not deleted",
			layoutFriendlyURL);
	}
