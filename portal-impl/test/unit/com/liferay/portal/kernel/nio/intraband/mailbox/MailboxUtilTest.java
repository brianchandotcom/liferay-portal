...
public class MailboxUtilTest {
	...
	@Test
	public void testDepositMailWithReaperThreadDisabled() throws Exception {
		...
		for (Thread thread : threads) {
			if ((thread != null) &&
				thread.getName().equals("Overdue Mail Reaper Thread")) {
	...
	public void testReceiveMailWithReaperThreadDisabled() {
		...
		// Do reverse, order matters

		ByteBuffer receivedByteBuffer2 = MailboxUtil.receiveMail(receipt2);
		ByteBuffer receivedByteBuffer1 = MailboxUtil.receiveMail(receipt1);

		Assert.assertSame(byteBuffer1, receivedByteBuffer1);
		Assert.assertSame(byteBuffer2, receivedByteBuffer2);
	}
	...
	public void testSendMailFail() {
		...
		try {
			MailboxUtil.sendMail(
				mockRegistrationReference, ByteBuffer.allocate(0));

			Assert.fail();
		}
		catch (MailboxException me) {
			Assert.assertEquals(
				RuntimeException.class, me.getCause().getClass());
		}
	}
	...
	public void testSendMailSuccess() throws MailboxException {
		...
		MockIntraBand mockIntraBand = new MockIntraBand() {
			...
			protected void doSendDatagram(
				...
				BigEndianCodec.putLong(data, 0, testReceipt);
		...
		long receivedReceipt = MailboxUtil.sendMail(
			mockRegistrationReference, ByteBuffer.allocate(0));

		Assert.assertEquals(testReceipt, receivedReceipt);
	}
	...
	public static class ReceiptStubAdvice {
		...
		@Around(
			"execution(public long " +
				"com.liferay.portal.kernel.nio.intraband.mailbox.MailboxUtil$" +
					"ReceiptStub.getReceipt())")
	...
	private Object createReceiptStub() throws Exception {
		Class<?> clazz = Class.forName(
			MailboxUtil.class.getName().concat("$ReceiptStub"));
	...
	private BlockingQueue<Object> getOverdueMailQueue() throws Exception {
