...
public class UpgradeLogProgressTracker {
	...
	private static String _buildCountSQL(String sql) {
		...
		String strippedSQL = StringUtil.trimTrailing(_stripOrderBy(lowerCaseSQL, sql));

		if (strippedSQL.endsWith(StringPool.SEMICOLON)) {
			strippedSQL = StringUtil.trimTrailing(
				strippedSQL.substring(0, strippedSQL.length() - 1));
		}

		return StringBundler.concat(
			"select count(1) from (", strippedSQL, ") tempCountTable_");
	}

	private static boolean _isSelect(String lowerCaseSQL) {
		String trimmed = lowerCaseSQL.stripLeading();
	...
	private static class StatementInvocationHandler
		...
		private void _cleanUpCountStatement(Throwable throwable) {
			if (_log.isDebugEnabled()) {
				_log.debug(
					"Unable to mirror parameter to count statement", throwable);
			}
