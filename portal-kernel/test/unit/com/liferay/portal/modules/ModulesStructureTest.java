...
public class ModulesStructureTest {
			...
			new SimpleFileVisitor<Path>() {
				...
				public FileVisitResult preVisitDirectory(
					...
					if (Files.exists(dirPath.resolve(".gitrepo"))) {
						_testGitRepoBuildScripts(dirPath);
					}
	...
	private String _getGitRepoBuildGradle(Path dirPath) throws IOException {
		...
		return StringUtil.replace(
			_GIT_REPO_BUILD_GRADLE_TEMPLATE, "[$BUILDSCRIPT_DEPENDENCIES$]",
			sb.toString());
	}

	private void _testGitRepoBuildScripts(Path dirPath) throws IOException {
		Path buildGradlePath = dirPath.resolve("build.gradle");
		...
		String buildGradle = FileUtil.read(buildGradlePath.toFile());

		Assert.assertEquals(
			"Incorrect " + buildGradlePath, _getGitRepoBuildGradle(dirPath),
			buildGradle);
		...
		String settingsGradle = FileUtil.read(settingsGradlePath.toFile());

		Assert.assertEquals(
			"Incorrect " + settingsGradlePath,
			_GIT_REPO_SETTINGS_GRADLE_TEMPLATE, settingsGradle);
	}

	private static final String _GIT_REPO_BUILD_GRADLE_TEMPLATE;

	private static final String _GIT_REPO_SETTINGS_GRADLE_TEMPLATE;

	static {
		try {
			ClassLoader classLoader =
				ModulesStructureTest.class.getClassLoader();

			_GIT_REPO_BUILD_GRADLE_TEMPLATE = StringUtil.read(
				classLoader,
				"com/liferay/portal/modules/dependencies/" +
					"git_repo_build_gradle.tmpl");
			_GIT_REPO_SETTINGS_GRADLE_TEMPLATE = StringUtil.read(
				classLoader,
				"com/liferay/portal/modules/dependencies/" +
