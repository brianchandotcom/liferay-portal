...
public class SelectorIntraBand extends BaseIntraBand {
	...
	public RegistrationReference registerChannel(final Channel duplexChannel)
		throws IOException {

		if (duplexChannel == null) {
			throw new NullPointerException("Duplex SelectableChannel is null");
		}
		...
		SelectableChannel duplexSelectableChannel =
			(SelectableChannel)duplexChannel;

		if ((duplexSelectableChannel.validOps() & SelectionKey.OP_READ) == 0) {
			throw new IllegalArgumentException(
				"Duplex SelectableChannel is not valid for reading");
		}

		if ((duplexSelectableChannel.validOps() & SelectionKey.OP_WRITE) == 0) {
			throw new IllegalArgumentException(
	...
	protected void doSendDatagram(
		...
		// Debug notice, if the polling thread runs cancellation at this point,
		// the following interestOps inspection may trigger a
		// CancelledKeyException which is normal in this context.
	...
	// All fields must be final to ensure safe publication, don't remove
	...
	protected class RegisterCallable implements
		...
		public RegistrationReference call() throws Exception {
			if (_readSelectableChannel == _writeSelectableChannel) {
				...
				// before ChannelContext is ready.
				...
				// before ChannelContexts are ready
				// This is also ensuring ChannelContext._registrationReference
				// thread safe publication
		...
		// Finals are required for safe publication
