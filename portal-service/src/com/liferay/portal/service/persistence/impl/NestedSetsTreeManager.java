...
public abstract class NestedSetsTreeManager<T extends NestedSetsTreeNodeModel> {
	...
	public long countAncestors(T t) throws SystemException {
		return doCountAncestors(
			t.getScopeId(), t.getNestedSetsLeft(), t.getNestedSetsRight());
	}
	...
	public void delete(T t) throws SystemException {
		...
		doUpdate(
			t.getScopeId(), -1, nestedSetsLeft, false, nestedSetsRight, false,
			null);
		doUpdate(t.getScopeId(), true, -2, nestedSetsRight, false);
		doUpdate(t.getScopeId(), false, -2, nestedSetsRight, false);
	}
	...
	public void insert(T t, T parent) throws SystemException {
		if (parent == null) {
			long maxNestedSetsRight = getMaxNestedSetsRight(t.getScopeId());
			...
			doUpdate(t.getScopeId(), true, 2, nestedSetsRight, true);

			doUpdate(t.getScopeId(), false, 2, nestedSetsRight, true);
	...
	public void move(T t, T oldParent, T newParent) throws SystemException {
		...
		List<T> childrenList = doGetDescendants(
			t.getScopeId(), nestedSetsLeft, nestedSetsRight);

		long newParentNestedSetsRight = 0;

		if (newParent == null) {
			newParentNestedSetsRight = getMaxNestedSetsRight(t.getScopeId());
		}
		else {
		...
		if (nestedSetsRight < newParentNestedSetsRight) {
			doUpdate(
				t.getScopeId(), -(nestedSetsRight - nestedSetsLeft + 1),
				nestedSetsRight, false, newParentNestedSetsRight, false, null);

			delta = newParentNestedSetsRight - nestedSetsRight - 1;

			doUpdate(
				t.getScopeId(), delta, nestedSetsLeft, true, nestedSetsRight,
				true, childrenList);
		}
		else {
			doUpdate(
