...
public class FinalizeManagerTest {
	...
	public void testManuelClear() throws Exception {
		...
		Map<Reference<?>, FinalizeAction> referenceActionMap =
			(Map<Reference<?>, FinalizeAction>)ReflectionTestUtil.getFieldValue(
				FinalizeManager.class, "_referenceActionMap");

		Assert.assertEquals(
			markFinalizeAction, referenceActionMap.get(reference));

		reference.clear();

		Assert.assertNull(referenceActionMap.get(reference));
