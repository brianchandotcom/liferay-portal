...
public class FIFOUtilTest {
	...
	@Test
	public void testPhantomDeleteOnPoking() throws InterruptedException {
		if (_shouldTest()) {
	...
	public void testReadOnlyTempFolderWithLog() {
		if (_shouldTest()) {
			...
			Assert.assertEquals(
				"FIFO support poking failed.", logRecord.getMessage());
			...
			Assert.assertTrue(
				throwable.getMessage().startsWith(
					"Failed to create FIFO with command \"mkfifo\", external " +
						"process returned "));
		}
	}
	...
	private static boolean _shouldTest() {
		if (OSDetector.isWindows()) {
			_log.warn("Skip testing, this test only runs on non-windows OS");
