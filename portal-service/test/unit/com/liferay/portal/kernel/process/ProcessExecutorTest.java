...
public class ProcessExecutorTest {
	...
	public void testLargeRuntimeClassPath() throws Exception {
		...
		builder.arguments(_createArguments(_JPDA_OPTIONS1));
		builder.runtimeClassPath(largeFileName);

		Future<String> future = ProcessExecutor.execute(
			builder.build(), echoRuntimeClassPathProcessCallable);
	...
	public void testPropertyPassing() throws Exception {
		...
		Future<String> future = ProcessExecutor.execute(
			builder.build(), readPropertyProcessCallable);
	...
	public void testWrongJavaExecutable() {
		...
		try {
			...
			builder.javaExecutable("javax");

			ProcessExecutor.execute(
				builder.build(), dummyReturnProcessCallable);
	...
	private static ProcessConfig _createJPDAProcessConfig(String jpdaOption) {
		...
		builder.arguments(_createArguments(jpdaOption));
		builder.bootstrapClassPath(System.getProperty("java.class.path"));
