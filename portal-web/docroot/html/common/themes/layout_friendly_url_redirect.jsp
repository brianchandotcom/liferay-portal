...
<c:if test="<%= Validator.isNotNull(alternativeLayoutFriendlyURL) %>">
	...
	<p class="redirected-to-message">
		<liferay-ui:message
			arguments="<%= redirectedLink %>"
			key="you-were-redirected-to-x"
	...
	<p class="original-url">
		<liferay-ui:message
			arguments="<%= originalLink %>"
			key="click-the-following-link-if-you-want-to-dismiss-this-redirect-and-access-the-original-url-x"
			translateArguments="<%= false %>"
		/>
	</p>
</c:if>