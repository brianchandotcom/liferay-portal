...
<%
String jspPath = GetterUtil.getString(session.getAttribute(WebKeys.PORTAL_MESSAGE_JSP_PATH));
String message = GetterUtil.getString(session.getAttribute(WebKeys.PORTAL_MESSAGE_MESSAGE));

if (Validator.isNotNull(jspPath) || Validator.isNotNull(message)) {
	String cssClass = GetterUtil.getString(session.getAttribute(WebKeys.PORTAL_MESSAGE_CSS_CLASS), "alert-info");
	String portletId = GetterUtil.getString(session.getAttribute(WebKeys.PORTAL_MESSAGE_PORTLET_ID));
	int timeout = GetterUtil.getInteger(session.getAttribute(WebKeys.PORTAL_MESSAGE_TIMEOUT), 10000);
	boolean useAnimation = GetterUtil.getBoolean(session.getAttribute(WebKeys.PORTAL_MESSAGE_ANIMATION), true);
%>
...
<%
	session.removeAttribute(WebKeys.PORTAL_MESSAGE_CSS_CLASS);
	session.removeAttribute(WebKeys.PORTAL_MESSAGE_PORTLET_ID);
	session.removeAttribute(WebKeys.PORTAL_MESSAGE_TIMEOUT);
	session.removeAttribute(WebKeys.PORTAL_MESSAGE_ANIMATION);
}

session.removeAttribute(WebKeys.PORTAL_MESSAGE_JSP_PATH);
session.removeAttribute(WebKeys.PORTAL_MESSAGE_MESSAGE);
%>