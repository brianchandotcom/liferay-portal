...
<div class="subscribe-action">
	<c:if test="<%= DLFolderPermission.contains(permissionChecker, scopeGroupId, folderId, ActionKeys.SUBSCRIBE) && ((folder == null) || folder.isSupportsSubscribing()) && DLUtil.getEmailFileEntryAnyEventEnabled(dlSettings) %>">
					...
					<c:choose>
						...
						<c:otherwise>
							<div style="float: left; margin: 100px 10px 0px;">
								<img alt="<%= HtmlUtil.escapeAttribute(LanguageUtil.get(pageContext, "error")) %>" border="no" src="<%= themeDisplay.getPathThemeImages() %>/application/forbidden_action.png" />
							</div>
						</c:otherwise>
					</c:choose>
				</c:when>
