...
for (int displayCounterIndex : displayCounterIndexes) {
	...
	List<SocialActivityStatsEntry> stats = null;

	if (chartType.equals("pie")) {
		if (dataRange.equals("year")) {
			stats = SocialActivityStatsEntryLocalServiceUtil.getStatsDistribution(scopeGroupId, counter, SocialStatsUtil.getFirstAtivityDayOfYear(), SocialStatsUtil.getCurrentPeriodEnd());
		}
		else {
			stats = SocialActivityStatsEntryLocalServiceUtil.getStatsDistribution(scopeGroupId, counter, 11, true);
		}
	}
	else if (chartType.equals("tagCloud")) {
		if (dataRange.equals("year")) {
			stats = SocialActivityStatsEntryLocalServiceUtil.getTagsByCounter(scopeGroupId, counter, SocialStatsUtil.getFirstAtivityDayOfYear(), SocialStatsUtil.getCurrentPeriodEnd());
		}
		else {
			stats = SocialActivityStatsEntryLocalServiceUtil.getTagsByCounter(scopeGroupId, counter, 11, true);
		}
		...
		if (dataRange.equals("year")) {
			stats = SocialActivityStatsEntryLocalServiceUtil.getStatsEntries(scopeGroupId, counter, SocialStatsUtil.getFirstAtivityDayOfYear(), SocialStatsUtil.getCurrentPeriodEnd());
		}
		else {
			stats = SocialActivityStatsEntryLocalServiceUtil.getStatsEntries(scopeGroupId, counter, 11, true);
		}
	}
	...
	request.setAttribute("group-statistics:stats-entries", stats);
