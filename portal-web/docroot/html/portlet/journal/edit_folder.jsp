...
<aui:form action="<%= editFolderURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveFolder();" %>'>
	...
	<aui:fieldset>
		...
		<c:if test="<%= rootFolder || (folder != null) %>">
			...
			<%
			List<Long> ddmStructureIds = JournalFolderLocalServiceUtil.getFolderDDMStructureIds(PortalUtil.getCurrentAndAncestorSiteGroupIds(scopeGroupId), folderId, false);
				...
				<div id="<portlet:namespace />overrideParentSettings">
					<c:if test="<%= !rootFolder %>">
						<liferay-ui:search-container
							...
							total="<%= ddmStructureIds.size() %>"
						>
							<liferay-ui:search-container-results
								results="<%= ddmStructureIds %>"
							/>

							<liferay-ui:search-container-row
								className="java.lang.Long"
								escapedModel="<%= true %>"
								modelVar="ddmStructureId"
							>
