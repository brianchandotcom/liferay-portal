													...
													<ul class="lfr-tree unstyled">
														<li class="tree-item">
															...
															<c:if test="<%= modelDeletionCount != 0 %>">
																...
																<%
																String exportDeletionsLabel = LanguageUtil.get(pageContext, "deletions") + (modelDeletionCount > 0 ? " (" + modelDeletionCount + ")" : StringPool.BLANK);
																%>

																<aui:input data-name="<%= exportDeletionsLabel %>" helpMessage="deletions-help" label="<%= exportDeletionsLabel %>" name="<%= PortletDataHandlerKeys.EXPORT_DELETIONS %>" type="checkbox" />
															</c:if>
														</li>
													</ul>
												</div>
