...
if (rankOnParticipation) {
	rankingCounters.add("participation");
}

if (rankOnContribution) {
	rankingCounters.add("contribution");
}

List<String> selectedCounters = new ArrayList<String>();

selectedCounters.add("contribution");
selectedCounters.add("participation");
...
if (rankingCounters.size() > 0) {
	...
	int total = SocialActivityStatsEntryLocalServiceUtil.getUserStatsByCountersCount(scopeGroupId, rankingCounters.toArray(new String[rankingCounters.size()]));

	searchContainer.setTotal(total);

	List<SocialActivityUserStats> results = SocialActivityStatsEntryLocalServiceUtil.getUserStatsByCounters(scopeGroupId, rankingCounters.toArray(new String[rankingCounters.size()]), selectedCounters.toArray(new String[selectedCounters.size()]), searchContainer.getStart(), searchContainer.getEnd());
	...
	for (int i = 0; i < results.size(); i++) {
		SocialActivityUserStats socialActivityUserStats = results.get(i);

		ResultRow row = new ResultRow(socialActivityUserStats, socialActivityUserStats.getUserId(), i);
