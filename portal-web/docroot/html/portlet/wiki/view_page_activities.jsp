	...
	<liferay-ui:search-container
		...
		<liferay-ui:search-container-row
				...
				<c:choose>
						...
						<liferay-util:buffer var="attachmentTitle">
							<c:choose>
								<c:when test="<%= fileVersion != null %>">
									<portlet:actionURL
											var="getPateAttachmentURL"
											windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
										<portlet:param name="struts_action"
													   value="/wiki/get_page_attachment"/>
										<portlet:param name="redirect"
													   value="<%= currentURL %>"/>
										<portlet:param name="nodeId"
													   value="<%= String.valueOf(node.getNodeId()) %>"/>
										<portlet:param name="title"
													   value="<%= wikiPage.getTitle() %>"/>
										<portlet:param name="fileName"
													   value="<%= fileEntry.getTitle() %>"/>
										<portlet:param name="status"
													   value="<%= String.valueOf(fileVersion.getStatus()) %>"/>
									</portlet:actionURL>

									<aui:a href="<%= getPateAttachmentURL %>"><%= title %>
									</aui:a>
								</c:when>
								<c:otherwise>
									<%= title %>
					...
					<c:when test="<%= (socialActivity.getType() == WikiActivityKeys.ADD_PAGE) || (socialActivity.getType() == WikiActivityKeys.UPDATE_PAGE) %>">
						...
						<portlet:renderURL var="viewPageURL">
							<portlet:param name="struts_action"
										   value="/wiki/view"/>
							<portlet:param name="nodeName"
										   value="<%= node.getName() %>"/>
							<portlet:param name="title"
										   value="<%= socialActivityWikiPage.getTitle() %>"/>
						...
						<c:choose>
							<c:when test="<%= socialActivity.getType() == WikiActivityKeys.ADD_PAGE %>">
								<liferay-util:buffer var="pageTitleLink">
									<aui:a href="<%= viewPageURL.toString() %>"><%= socialActivityWikiPage.getTitle() %>
									</aui:a>
								</liferay-util:buffer>
							...
							<c:when test="<%= socialActivity.getType() == WikiActivityKeys.UPDATE_PAGE %>">
								<liferay-util:buffer var="pageTitleLink">
									<aui:a href="<%= viewPageURL.toString() %>"><%= version %>
									</aui:a>
								</liferay-util:buffer>
								...
								<c:if test="<%= socialActivityWikiPage.getStatus() != WorkflowConstants.STATUS_APPROVED %>">
									<em><liferay-ui:message
											key="<%= WorkflowConstants.toLabel(socialActivityWikiPage.getStatus()) %>"/></em>
								</c:if>

								<c:if test="<%= Validator.isNotNull(socialActivityWikiPage.getSummary()) %>">
									<em class="activity-summary"><%= StringPool.QUOTE + socialActivityWikiPage.getSummary() + StringPool.QUOTE %>
									</em>
								</c:if>
							</c:when>
						</c:choose>
			...
			<liferay-ui:search-container-column-text
					...
					>
				<liferay-ui:message
						arguments="<%= LanguageUtil.getTimeDescription(pageContext, System.currentTimeMillis() - socialActivity.getCreateDate(), true) %>"
						key="x-ago"/>
			</liferay-ui:search-container-column-text>
