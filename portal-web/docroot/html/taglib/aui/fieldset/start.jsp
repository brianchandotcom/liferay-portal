...
<div aria-labelledby="<%= panelId %>Title" class="panel panel-default <%= cssClass %>" <%= Validator.isNotNull(id) ? "id=\"" + namespace + id + "\"" : StringPool.BLANK %> <%= InlineUtil.buildDynamicAttributes(dynamicAttributes) %> role="group">
	...
	<div aria-labelledby="<%= panelId %>Header" class="panel-collapse collapse <%= collapsed ? StringPool.BLANK : "in" %> <%= column ? "row-fluid" : StringPool.BLANK %>" id="<%= panelId %>Content" role="presentation">
