...
String disabledToggler = GetterUtil.getString((String) request.getAttribute("liferay-ui:input-field:disabledToggler"));
...
<c:if test="<%= type != null %>">
	<c:choose>
		...
		<c:when test='<%= type.equals("Date") %>'>
			...
			<c:if test="<%= Validator.isNotNull(disabledToggler) %>">
				<div class="clearfix">
					<aui:input id="<%= formName + fieldParam %>" name="<%= disabledToggler %>" type="checkbox" value="<%= disabled %>" />
				</div>
