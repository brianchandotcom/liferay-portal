...
public class SPIClassPathContextListener implements ServletContextListener {
	...
	public void contextDestroyed(ServletContextEvent servletContextEvent) {
		SPIProvider spiProvider = _spiProviderReference.getAndSet(null);

		if (spiProvider != null) {
			MPIUtil.unregisterSPIProvider(spiProvider);
		}
	}
	...
	public void contextInitialized(ServletContextEvent servletContextEvent) {
		...
		if (_log.isDebugEnabled()) {
			_log.debug("Generated SPI classpath : " + spiClassPath);
		}

		SPI_CLASSPATH = spiClassPath;
		...
		try {
			...
			boolean result = _spiProviderReference.compareAndSet(
				null, spiProvider);

			if (!result) {
				_log.error(
					"Skip register generate SPIProvider " + spiProvider +
						". There is already a SPIProvider registered in this " +
							"ServletContext " + servletContext +
								" with same name");
			}
			else {
				MPIUtil.registerSPIProvider(spiProvider);
			}
		}
		catch (Exception e) {
			_log.error(
				"Failed to create SPIProvide with name " + spiProviderClassName,
				e);
		}
	}
	 ...
	 * Basically it allows a SPI to create its own SPI instances.
	 ...
	 */
	private static Class<?> loadClassDirectly(
	...
	private static final AtomicReference<SPIProvider> _spiProviderReference =
