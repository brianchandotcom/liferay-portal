...
export class CreateFoldersComponent implements OnInit {
	...
	constructor(
		private templateServices: TemplateServicesService,
		private fb: FormBuilder,
		@Inject(MAT_DIALOG_DATA) public data: any | null = null,
		public dialogRef: MatDialogRef<CreateFoldersComponent> | null,
	...
	async save() {
		try {
			const result = await this.templateServices.createFolder(
				this.templateId,
				this.selectedContainerFolder.id,
				this.templateForm.value.folderName
			);
			if (result) {
