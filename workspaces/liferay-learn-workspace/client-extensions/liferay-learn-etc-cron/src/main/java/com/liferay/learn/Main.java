...
public class Main {
	...
	public static void main(String[] arguments) throws Exception {
		...
		String rootDir = System.getenv("LIFERAY_LEARN_ETC_CRON_ROOT_DIR");

		if (rootDir == null) {
			rootDir = "~/liferay-learn";
		}

		File rootDirFile = new File(rootDir);

		Main main = new Main(
			liferayDataDefinitionKey, liferayLearnResourcesDomain,
			...
			liferaySiteFriendlyUrlPath, new URL(liferayUrl), rootDirFile,
	...
	public Main(
			...
			String liferaySiteFriendlyUrlPath, URL liferayURL, File rootDir,
			boolean offline)
		throws Exception {
		...
		_rootDirName = rootDir.getCanonicalPath();

		_docsDirName = _rootDirName + "/docs";

		System.out.println("Liferay URL: " + _liferayURL);

		_readHashFromFile(rootDir);
		...
		for (String fileName : _fileNames) {
			...
			try {
				...
				if (externalReferenceCodeStructuredContents.containsKey(
					...
					if (!_diffFileNames.isEmpty() && !_diffFileNames.contains(fileName.replace(_rootDirName, ""))) {
						System.out.println(
			...
			_saveHashToFile(new File(_rootDirName), _newHash);
		}
	...
	private void _readHashFromFile(File dir) throws IOException {
		File hashFile = new File(dir, _hashFileName);
	...
	private void _saveHashToFile(File dir, String hash) throws IOException {
		File hashFile = new File(dir, _hashFileName);
	...
	private String _getHTML(File file) throws Exception {
		String htmlFilePath = file.getCanonicalPath(
			...
			_docsDirName, _rootDirName + "/site"
		).replaceFirst(
			"\\.md", ".html"
		);
	...
	private static final String _hashFileName = ".latest-hash";
	...
	private final String _rootDirName;
