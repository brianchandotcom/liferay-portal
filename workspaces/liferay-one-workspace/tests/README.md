...
yarn bootstrap:tests      # create tests/.env, install deps, fetch chromium, pull OAuth credentials
```
...
Integration tests authenticate via the `oneApi` fixture:

- If `ONE_OAUTH_CLIENT_ID` and `ONE_OAUTH_CLIENT_SECRET` are set, the helper fetches a bearer token from `/o/oauth2/token`.
- Otherwise it falls back to basic auth with `ONE_ADMIN_EMAIL` / `ONE_ADMIN_PASSWORD` (defaults: `test@liferay.com` / `test`).

Today, the default basic-auth path is enough for the sanity-level integration specs — the seed admin user created by `quickstart/run.sh` works out of the box.

### Upgrading to OAuth2 client-credentials

When a spec needs scoped auth or to test OAuth2-protected endpoints:

1. Register an `oAuthApplicationHeadlessServer` entry in `client-extensions/liferay-one-site-initializer/client-extension.yaml`

1. `./gradlew :client-extensions:liferay-one-site-initializer:deploy` and wait for Liferay to register.

1. From the workspace root, run `yarn fetch:oauth` — this reads the generated credentials from the container's routes volume and writes them to `tests/.env`.
...
- **Secrets** come from `.env` (gitignored) or environment. Never commit a real OAuth secret.