...
1. **Object definition (batch type)**
   Create the initial Object definition using a **batch** client extension so the form has a target Object and API to call. The batch type defines the Object (for example, a custom object containing fields the form collects).
...
- Deploy command: `blade gw deploy`. Automatically copies the zip file to the Liferay server in `osgi/client-extensions`. Run this for the user as part of the one-shot generation.
