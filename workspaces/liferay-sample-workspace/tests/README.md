...
| E2E | Playwright (browser) | `tests/e2e/` | Whole flows through the UI. Last resort — push behaviour down to integration when possible. |
...
> `test` is intentionally not wired as a script in this package — `./gradlew build` auto-invokes any `yarn test` it finds, and running Playwright during a gradle build would require a running Liferay instance. Vitest owns the build-time `test` slot in each client extension; Playwright runs on demand via these scripts.
