...
async function liferayStreamHubConnect() {
    try {
        const oAuthApplication = Liferay.OAuth2Client.FromUserAgentApplication(
            "liferay-stream-hub-etc-spring-boot-oauth-application-user-agent"
        );

        const [protocol, hostname] = oAuthApplication.homePageURL.split("//");

        const token =Liferay.ThemeDisplay.isSignedIn()? await oAuthApplication._getOrRequestToken():"GUEST";

        const socket = new WebSocket(`ws://${hostname}/server`, token.access_token);
        ...
        socket.onclose = (event) => {
            console.warn("Disconnected from server, trying to reconnect...", event.reason);
            retryWithBackoff();
        };
    } catch (error) {
