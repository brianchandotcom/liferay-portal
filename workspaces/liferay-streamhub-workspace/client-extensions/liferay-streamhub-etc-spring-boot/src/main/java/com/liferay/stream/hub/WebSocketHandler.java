...
public class WebSocketHandler extends TextWebSocketHandler {
	...
	@Override
	@SuppressWarnings("ListUtilCheck")
	...
	public <T> void sendMessage(
			T data, String name, List<String> clientIds,
			Boolean enableOfflineMessageQueue)
		throws JsonProcessingException {

		String json = _objectMapper.writeValueAsString(
		...
		clientIds.forEach(
			clientId -> {
				...
				if (!isOnline.get() && enableOfflineMessageQueue) {
					_pendingEvents.insert(clientId, "Event", name, data);
				}
			});
	}
